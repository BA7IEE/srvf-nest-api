...
- **招新/入队十三项收口 PR-3(刀F/G:申请人身份链 + 证书审核闭环)** —— ⚠️ **契约收紧**:公开 submit 的 `phoneVerificationToken` 对 H5/小程序统一必填,小程序仅传 `wechatCode` 现一律 40000;前端须先走既有 `identity/send-code` + `verify-code`,可另附 `wechatCode` 绑定 openid。**证书审核闭环**:新 `POST admin/v1/recruitment/applications/:id/certificates/:category/review`(+权限码 `recruitment-application.review.certificate`,全绑 biz-admin):通过自动标对应门槛(first_aid→redCross / bsafe→bsafe),驳回清图 + 退门槛并向申请人进度回显驳回态/note,重传复位为 uploaded;直接/批量标 redCross/bsafe 新增「对应类别已有图」前置,缺图 `28053`。第二条/最终授权 migration 合并刀G+刀H additive 列:`certificateReviewStatus Json?` + 入队轮 `openOrganizationIds Json?`/`maxTargetOrgs Int?`(后两列由 PR-4 消费),migration 46→**47**。计数:权限码 204→**205** / biz-admin 80→**81** / EXPECTED_ROUTES 335→**336** / controller 66 / org-admin 60 / ops-admin 96;新增 `recruitment-application.certificate-review` audit,无 PII。
