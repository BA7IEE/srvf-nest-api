...
- **NEXT_TASKS 熵清理(Harness 3.0 P7)**:`docs/ai-harness/NEXT_TASKS.md` **41,182 → 15,527 字节(−62%)**。台账没有退场机制 —— 搬出时 15 条活跃条目里 **7 条已完成或已判定不做**,另有一个 24.5KB 的「已完成项归档区」压在同一文件内(占全文件 60%),读的人每次都要先把它们过滤掉才能看见真正在推进的事。已收口项整体移至 `docs/archive/ai-harness/next-tasks-completed.md`(冻结不再增长),**P0/P1/P2 分类与全部措辞逐字未改**(第一版实现顺手把分类法换成了「在推进 / 休眠」,自查发现后回退重做 —— 那是维护者的台账结构,不是清理者可以顺带改的)。搬运做了逐行对账:非空行零丢失、零重复。
