...
// seed 事实闭包必须由三个独立解析器精确具名；漏掉权限目录时全部 237 条码必须立刻显形。
{
  ...
  // 搬家后**全部 269 条都在 permission-catalog.ts**,`prisma/seed.ts` 里一条都不剩 ——
  ...
  // 所以它必须与上一条(完整闭包恰 269)成对读:一条钉住总量非空,一条钉住分布只有一处。
  const CLOSURE_PERMISSION_CODE_COUNT = 269; // E1-3 +2 activity contribution-policy read/select；已批准的计数联动
  ...
  // 正向:权限目录**独自**就装着全部 269 条 —— 「单一事实源」的正面形式。
  checkEq(
...
void (async (): Promise<void> => {
  ...
  // ────────────────────────────────────────────────────────────────────────
  {
    ...
    checkEq(
      ...
      verdictOf(basePolicy({ mode: 'LOGIN_ONLY', codes: [], engine: null }), serviceOnly),
      'INCOMPARABLE',
    );
    checkEq(
