...
// seed 事实闭包必须由三个独立解析器精确具名；漏掉权限目录时全部 237 条码必须立刻显形。
{
  ...
  // 搬家后**全部 267 条都在 permission-catalog.ts**,`prisma/seed.ts` 里一条都不剩 ——
  ...
  // 所以它必须与上一条(完整闭包恰 267)成对读:一条钉住总量非空,一条钉住分布只有一处。
  const CLOSURE_PERMISSION_CODE_COUNT = 267; // E1-2 +2 contribution-policy read/manage；已批准的计数联动
  ...
  // 正向:权限目录**独自**就装着全部 267 条 —— 「单一事实源」的正面形式。
  checkEq(
