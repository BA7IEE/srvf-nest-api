...
- `migrations/` 累计 27 个 migration(2026-05-02 `init` → 2026-06-25 统一通知模块 S1:`notifications` + `notification_reads` 两表〔含统一形状列 audienceType/sourceType/channels;`notification_reads` plain unique [notificationId, memberId] 无软删;FK 全 Restrict;全 additive,无 enum、无不可逆、无回填〕)。
