...
V2 第一阶段开发已完成,等待维护者按需 release / tag。新增 44 个接口,**v1 14 接口契约严格 zero drift**(`LoginDto` / `UserResponseDto` 不漂移)。
...
`member_profiles` / `emergency_contacts`(批次 1)+ `certificates`(批次 2)已落地(详见上表)。V2.x 复活路径仍延后(不在本阶段):`attachments` / `audit_logs` / `events` / `event_participants`。
