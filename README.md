...
V2 第一阶段开发已完成,等待维护者按需 release / tag。新增 61 个接口,**v1 14 接口契约严格 zero drift**(`LoginDto` / `UserResponseDto` 不漂移)。
...
`member_profiles` / `emergency_contacts`(批次 1)+ `certificates`(批次 2)+ `activities` / `activity_registrations`(批次 3A)已落地(详见上表)。批次 3 schema 含 `attendance_sheets` / `attendance_records` 4 model 已同步入库(commit `31c8187`),但 `attendances` API 模块作为**批次 3B 独立 PR**,等 3A 合并后单独提交。V2.x 复活路径仍延后(不在本阶段):`attachments` / `audit_logs` / `events` / `event_participants`。
