...
| D-7 | 六类职责边界(Presenter / QueryService / Policy / StateMachine / AuditRecorder / Effect)。附注: `AuthzDeclarationGuard` 仅验证路由已有结构化访问声明，不执行或替代 Service 层 `rbac.can()` / `authz.can()` 判权。 | eslint 禁 Presenter·Policy 碰 DB;归属判断留人 |
