...
> 4. **storage-settings(多实例 live-read；enabled 恢复闭环)**:[`storage-settings.service.ts`](../src/modules/storage/storage-settings.service.ts) 每次调用直读 PostgreSQL 当前已提交 singleton；写事务提交后任一实例的下一次读取无需 wait/restart/reload/invalidate，当前 Effect 只使用一份已解析参数快照。`enabled=false` 会拒绝下一次普通业务 put/delete/sign/head/read（含历史 pinned locator 与自动 worker），但 API/worker 控制面可重启并 WARN；仅经过人工复核的 manual maintenance 可绕过。production 运行期 settings 丢失/LOCAL/unknown 均 fail-closed，普通 PATCH 冻结 providerType/bucket/region。
...
```bash
docker build -t u-nest-api-starter:<release-tag> .
```
...
启动强校验在 `APP_ENV=production` 下会拒绝:`JWT_SECRET` 等于 `.env.example` 默认值、`APP_CORS_ORIGIN` 为空或 `*`、`APP_TRUSTED_PROXY_CIDRS` 缺失/空白/非法、`SUPER_ADMIN_PASSWORD` 等于默认值(seed 时)、`SUPER_ADMIN_USERNAME=admin`(seed 时)。Docker Smoke 现以真实 `APP_ENV=production` 执行 migration→seed→Storage bootstrap→容器 boot，并固定 trusted proxy 为 `none`；它不触发真实 COS Effect，不能替代现场 COS 验收。完整字段以 [`.env.example`](../.env.example) 为准。
...
每个 release candidate 在最终 SHA 上运行 `pnpm audit:production`；high/critical 阻断，moderate/low 逐条分析并登记。tag 推送还会触发 `Production dependency audit` workflow，当前 COS 传递链基线与接受边界见 [`ops/production-dependency-audit.md`](ops/production-dependency-audit.md)。
