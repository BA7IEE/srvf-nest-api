...
> 2. **10 个内存限流器(扩容前必须处理)**:`@nestjs/throttler` 内存 storage、不引 Redis([`throttle-options.ts`](../src/bootstrap/throttle-options.ts);login(`default`)/ password-change / refresh / sms-send / sms-verify / password-reset / login-sms / login-wechat(2026-06-12 微信 goal +)/ recruitment(2026-06-18 招新一期 +,IP 10/3600)/ content-public(2026-06-21 CMS 内容模块 +,IP 60/60)共 10 个实例,均 IP 维度计数)。多副本 → 各实例独立计数,有效阈值 ≈ 配置值 × 副本数放宽。注:SMS 发码另有 DB 层第二道防线(同号 60s 间隔 + 同号自然日上限,`SmsCodeService` 查库实现),天然多实例安全;被放宽的只是 IP 维度桶(login / sms-send 等)→ 扩容前换共享 storage(如 Redis throttler storage)。
