...
> 4. **storage-settings(多实例 live-read；enabled 生命周期已闭合)**:[`storage-settings.service.ts`](../src/modules/storage/storage-settings.service.ts) 每次调用直读 PostgreSQL 当前已提交 singleton；写事务提交后任一实例的下一次读取无需 wait/restart/reload/invalidate，即可获得新 provider/bucket/region/credentials，当前 Effect 只使用一份已解析参数快照。`enabled=false` 会拒绝下一次普通业务 put/delete/sign/head/read（含历史 pinned locator 与自动 worker），已开始的 Effect 使用原快照自然结束；仅经过人工复核的 manual maintenance 证据采集可使用当前凭证恢复历史对象。production PATCH 可关闭 enabled，但合并后的配置始终强制 COS + 非空 bucket/region + 可解密凭证。
