...
| P2 | service 单测仍偏少(`src/` 内 18 个 `*.spec.ts` / 213 个源文件 ≈ 8.5%;#251 合并后 19) | E2E 78 spec 覆盖回归;4 个 god-service service spec 已优先覆盖;large-service `certificates` spec 已于 #251 起出(待验收),`users` spec 仍不自动启动 |
