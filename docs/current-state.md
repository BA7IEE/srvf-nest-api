...
| 🔴 **P1** | **整批评审(N+R+W)未通过,2 新 P1 已复现**(上轮 5 P1 **全 PASS**,真触发见 #868):① Proposal **多岗位同时扩容重复消费父容量预算**(helper 每轮重数 PASS 而递补只写 PENDING ⇒ 双岗位各领全额,可超父余量);② `/* eslint srvf/…: "off" */` 配置注释关掉执法规则(扫描器只认 `eslint-disable`)。P2×2:企微可见范围 null 当缺席;多次锁等待可撞 7s 总预算。**一个 Q goal 收口**;见 [`NEXT_TASKS` P1-24](ai-harness/NEXT_TASKS.md) |
