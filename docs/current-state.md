...
| 项 | 当前值 |
|---|---|
| **发布边界** | 🔴 **代码 Release NO-GO**(证书标准库第三轮独立评审未通过,见 §4)——**禁止 bump / tag / Release / 生产部署**,修复并复审通过前不解除;production 本就未部署;活动责任、保险、Storage、外部通道、基础设施硬门仍开放(§4 / runbook) |
...
| 🔴 **P1** | **证书标准库第三轮评审未通过**(第二轮 4 条已修关闭;本轮 5 条已复现、**无 P0**):① `rejected` 报名下 `APPROVED` Claim 无人终结(冻死 + 清理扫不到 + 证据可签,与既有不变量矛盾);② `updateApplication` 终态语义与 canonical handoff 分叉(**待拍板**);③ 建 Standard 传 `null` 仍 500;④ 父子环零执法而注释称不可能;⑤ 多处注释描述已不存在的设计。逐条见 [`NEXT_TASKS` P1-24](ai-harness/NEXT_TASKS.md) |
