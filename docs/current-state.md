...
- **测试与契约**:e2e **89 suites / 1818 tests** + unit **37 spec / 1324 tests**(2026-06-12 微信 goal true-up:e2e +wechat-settings 11 / auth-wechat 13 / app-me-wechat 7〔seed 二档计数随授权 seed 变更同步〕;unit +wechat crypto 7 / dev-stub 2 / provider 14)+ contract **168 路由**白名单;CI 全链 + docker-smoke(含生日 cron 启动锚行 + `WECHAT_ENCRYPTION_KEY` 注入)
