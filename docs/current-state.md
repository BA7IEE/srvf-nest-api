...
- **通知 durable outbox**:PG lease/fence+generation/recipient/RBAC/quota；招新/入队及报名/活动/责任/考勤事务内写 intent；考勤达标认 capped 前后真值、每 application+门槛一次。provider 事务外 at-least-once；生产未deploy，切换须排空旧API/worker/intents、禁混档
