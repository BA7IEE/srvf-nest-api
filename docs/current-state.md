...
| 项 | 当前值 / 何处看 |
|---|---|
| 版本(六处一致) | **v0.55.0**(2026-07-16;package.json = Swagger = CHANGELOG = tag = Release = handoff OpenAPI;最新 handoff `archive/handoff/v0.55.0.md`) |
| main HEAD / open PR / Unreleased | 实时看 `gh pr list` · [`CHANGELOG.md`](../CHANGELOG.md) `## Unreleased` + `changelog.d/`;当前:Harness 2.0 系列进行中(T0 #653 起) |
