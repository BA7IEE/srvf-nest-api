...
| 🔴 **P1** | **整批评审(M+企微)未通过,5 P1 已复现**(M 运行时+企微 T1 **双 PASS,无 P0**):① 裁判只冻结棘轮 id,同 id 可换判据载体;② `srvf/*` 规则可被行内 disable 与 `CV['IsOptional']`/中转绕过;③ 企微 Settings 锁后不复读(S1 重生)可写矛盾开关;④ `agent/get` 缺字段默认当"正常"(errcode 缺=成功);⑤ Provider 单例存 settings 串配置。**拆 R(①②)/W(③④⑤)两 goal**;见 [`NEXT_TASKS` P1-24](ai-harness/NEXT_TASKS.md) |
