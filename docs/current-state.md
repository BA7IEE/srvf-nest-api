...
| `main` HEAD | **`431ea8c`** `test(attendances): add scoped service characterization spec (#247)`(post-v0.15.0 docs 治理 + 架构边界抽离串 + Governance-1A 串 + **P1-C controller 物理拆分收口**(attendances step 4 = #236 / 状态回填 #237)+ **CODEMAP drift check 脚本**(#238)/ **CODEMAP 首批 drift 修复**(#239)/ **`srvf-god-service-refactor` skill**(#240)/ **`activity-registrations.service.spec.ts`**(#241)/ **current-state + CODEMAP 回填**(#242)/ **`attachments.service.spec.ts` service-level characterization spec**(#243)/ **`srvf-release-closeout` skill**(#244)/ **current-state 回填**(#245)/ **`activities.service.spec.ts` service-level characterization spec**(#246)/ **`attendances.service.spec.ts` service-level characterization spec**(#247)均已落地;v0.15.0 release tag 仍指向 `089499d` = PR #163 handoff squash commit,2026-05-31 核对) |
...
| P2 | service 单测仍偏少(`src/` 内 18 个 `*.spec.ts` / 213 个源文件 ≈ 8.5%) | E2E 78 spec 覆盖回归;4 个 god-service service spec 已优先覆盖,large-service(certificates / users)spec 不自动启动 |
