...
| 🔴 **P1** | **整批评审(M+企微 T1/T2)未通过,5 P1 已复现**(M 并发运行时与企微 T1 schema **双 PASS,无 P0**):① 裁判只冻结棘轮 id,同 id 可换判据载体;② `srvf/*` 规则 controller 行内 disable 有效、`CV['IsOptional']`/中转不识别;③ 企微 Settings **锁后不复读**(S1 形状在新代码重生)可写出矛盾开关;④ `agent/get` 缺字段被默认值补成"正常"(errcode 缺=成功);⑤ Real Provider 单例存请求级 settings 可串配置。**修复拆 R(①②)/ W(③④⑤)两 goal**;逐条见 [`NEXT_TASKS` P1-24](ai-harness/NEXT_TASKS.md) |
