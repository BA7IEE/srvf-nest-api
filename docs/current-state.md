...
| 项 | 当前值 |
|---|---|
| **发布边界** | 🔴 **代码 Release NO-GO**(2026-07-30 证书标准库第二轮独立评审未通过,见 §4 P0)——**禁止 bump / tag / Release / 生产部署**,修复并复审通过前不解除;production 本就未部署;活动责任、保险、Storage、外部通道、基础设施硬门仍开放(§4 / runbook) |
...
| 🔴 **P0** | **证书标准库第二轮评审未通过**(4 条已复现属实)。根因:证书新写路径已统一用报名锁,**评定 / 换绑 / 后台改资料仍是旧写法**(无锁、无锁后复读、无 CAS)⇒ 已撤销报名可被覆盖回 `publicity` 并发出永久编号;脱敏后 PII 可被写回。逐条机制与修复范围见 [`NEXT_TASKS` P1-24](ai-harness/NEXT_TASKS.md) |
