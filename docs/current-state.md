...
- **通知 durable outbox**:同事务写 intent，provider 外 at-least-once；自助取消正文=`displayName（memberNo）`，无标签匿名且不露 `Member.id`，gate true=active owner/false=legacy publisher，outbox 键/目标不变；生产未deploy，切换须排空、禁混档
