...
| `main` HEAD | **`bf0d033`** `docs(harness): add srvf-release-closeout skill (#244)`(post-v0.15.0 docs 治理 + 架构边界抽离串 + Governance-1A 串 + **P1-C controller 物理拆分收口**(attendances step 4 = #236 / 状态回填 #237)+ **CODEMAP drift check 脚本**(#238)/ **CODEMAP 首批 drift 修复**(#239)/ **`srvf-god-service-refactor` skill**(#240)/ **`activity-registrations.service.spec.ts`**(#241)/ **current-state + CODEMAP 回填**(#242)/ **`attachments.service.spec.ts` service-level characterization spec**(#243)/ **`srvf-release-closeout` skill**(#244)均已落地;v0.15.0 release tag 仍指向 `089499d` = PR #163 handoff squash commit,2026-05-29 核对) |
...
| P2 | `attachments.service.ts` 826 行 / `activity-registrations.service.ts` 750 行 / `activities.service.ts` 607 行 | audit-recorder / state-machine 已抽离;`activity-registrations`(#241)+ `attachments`(#243)已补 service-level characterization spec,`activities` 仍缺 `*.service.spec.ts` 单测;建议先补 service spec,再做任何重构 |
| P2 | service 单测偏少(`src/` 内仅 16 个 `*.spec.ts` / 211 个源文件 ≈ 7.6%) | E2E 78 spec 覆盖回归,但单元层缺位;建议为 god-service 优先补 spec |
