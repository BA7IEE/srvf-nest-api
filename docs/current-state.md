...
- **P1-C(Mixed Controller 物理拆分)已被 Route B 全量迁移收口**:原 `controllers/*-legacy.controller.ts`(users / attachments / activity-registrations / attendances)已于 Phase 4 删除,队员自助流在 `/api/app/v1/me*` / `/api/app/v1/my/*`;`attendances.service.ts`(1157 行)等 god-service 拆分仍需单独立项
- **不**自动拆分 `attendances.service.ts`(1157 行)/ `attachments.service.ts`(827 行)/ `activity-registrations.service.ts`(750 行)等 god-service — characterization tests 已落地一批(沿 [`architecture-boundary.md §5`](architecture-boundary.md));拆 service 行为本身仍需单独立项
...
| P2 | `attachments.service.ts` 827 行 / `activity-registrations.service.ts` 750 行 / `activities.service.ts` 607 行 | audit-recorder / state-machine 已抽离;`activity-registrations`(#241)+ `attachments`(#243)+ `activities`(#246)均已补 service-level characterization spec;后续 Presenter / QueryService 边界拆分需单独立项,本期不动 |
| P2 | service 单测仍偏少(`src/` 内 **20** 个 `*.spec.ts` / 211 个 `.ts` 源文件 ≈ 9.5%) | E2E 72 spec 覆盖回归;4 个 god-service + 2 个 large-service(`certificates` #251 / `users` #253)service spec **均已覆盖** |
