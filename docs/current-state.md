...
- **活动业务改造(第 0+1 批 ✅ 2026-08-04;39 表 / 第 71-75 migration)**:**需求 = v1.1 四份 + [`AMENDMENTS-v1.1.1`](archive/reviews/activity-business-overhaul-v1.1/AMENDMENTS-v1.1.1.md) 合起来,冲突以后者为准**(五条合同缺口已接受裁定;⛔③离线两表未定义=第 6 批开工硬门,⏸①容量索引缺列归第 4 批,⏸⑤四处取值集待业务方);万人锁原型**时间预算通过**(P95 197.7ms);**万人统一生效恒串行**(2026-08-04 定,因两场并发越 12800 共享锁表保底而 `out of shared memory`;⚠️须带执行位不能只写成文字);bind 上限 32767 走 unnest;逐条见 `NEXT_TASKS` P1-28
