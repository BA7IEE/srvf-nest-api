...
| **现象** | 冻结的 T0 架构评审稿(终态 scoped-authz 序列的立项设计文档)自称数据来源为「真实任命公告《深圳市公益救援志愿者联合会 2026 年任命公告》」,文中系统性出现 10+ 个真实姓名(石欣 / 黄勇 / 赵强 / 金洋 / 陈媛 / 吴冰宁 / 卓晓玲 / 崔广庆 / 李翔 / 李美玲 / 姬颖 / 王志伟等),并与真实 `memberNo`(`#18130` / `#18131` / `#20065`)、真实部门 / 组织代号成对出现,用来验证设计对真实组织结构的适配性。`prisma/seed.ts:776` 注释里单独出现「赵强兼 SAMT 队长」一例,是同一违规的最小回声(单一姓名,无编号)。 |
