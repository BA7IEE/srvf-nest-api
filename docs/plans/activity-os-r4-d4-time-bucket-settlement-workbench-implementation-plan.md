...
## 17. 两个关联Draft PR的独立交付（2026-09-13）

草稿依赖已创建为 [Draft PR #1326](https://github.com/BA7IEE/srvf-nest-api/pull/1326)，base为main；D4以`codex/activity-settlement-draft-batch`为base创建关联Draft PR。1952个代码／测试／治理文件与D4验证候选逐字一致，集合SHA-256为`2cd60642bd9355707976a4e82c6799c31e00875f8fde5c0df2c7fb3a467e988b`；这些文件未因拆分改写。PR创建不代表CI、审批、Ready或合并完成。

维护者已批准保留全部现有改动，122路径内按依赖顺序独立验证、拆分提交推送及两个Draft PR。原109份改动拆分前逐一备份，第一提交精确27个blob与独立候选一致，原工作树109份内容在暂存动作前后均逐字保留。草稿依赖提交`1ded3e7e229f1564edf9b500d9943e0fd415079d`，tree `3f75568e92e9c7434cbe68b3fb6e6cd0c05d958a`；D4基于该依赖提交，不混入首个PR。

### 17.1 草稿依赖独立候选验收（2026-09-13）
...
- `d4-unit.json`：385组／8473通过／0失败／5既有todo，113.494秒；完整typecheck及build通过。原联合树lint、完整Harness及replay通过证据仍适用于相同代码，独立依赖另行执行并通过，没有以它代替D4结果。
...
- 大规模结算草稿补齐实施中：保留同步500人口上限，共用原生成算法，后台绑定封印与原执行人，使用既有worker与租约；段/结果改为批量写入，草稿和任务成功回执同事务。新增501人口成功/回放和拒绝分支真实数据库验证，旧草稿37项回归通过。
- D4的2000人口/10000有效段/50000切片满额冷库首跑现已通过：真实封印、HTTP入队、worker生成、prepare/page/submit完整链，prepare12177ms，总SQL340/41/811。封印检查按单方法追加授权等价拆分索引查询，保持5秒预算；D4内部来源校验消除冷态的全量聚合重复遍历，保持30秒预算和全部数据库约束。
- 完成状态限制：其余G1–G11矩阵及最终完整检查仍待补齐，当前结果不代表已发布、已部署或允许启用Gate。旧数据和旧行为继续保留。
- 补充普通成员的授权/身份锁等待拒绝、真实替代封印后新键恢复、HTTP送审后排队重生成拒绝；既有44项组合通过，后续G9六项组合通过，当前46项尚未完整组合复跑。运维SOP新增草稿action的安全恢复步骤，明确不沿用旧任务手工SQL恢复、不改旧payload、不自动换键重试，保留健康检查盲区与7c待重签边界。
- 最新验证覆盖上条历史时点：当前草稿套件56项完整通过，包含资格变化的开始前/锁等待时点矩阵与真实子进程故障恢复；既有11组229项单测及新增任务读面/重试/取消11项单测分别通过。交接统一为任务成功后刷新既有结算工作台，不新增结果指针字段。完整交付核对、签字及PR流程尚未完成。
- 按维护者追加授权，为过期草稿job→item同语句更新补固定SQL合同的受限围栏识别，保留旧断言并增加20项正反例，不作方法级无条件豁免、不改生产SQL。全仓单测385组、8473项通过，0失败，5项既有todo保留；此前围栏单测失败报告继续留存。签字与发布边界不变。
- 维护者确认后已登记D4的3b/4b及草稿worker runbook的7c重签，签字完整性与当前读数对拍通过。此项仅更新签字及证据，不代表提交、PR、合并、部署或启用Gate获准。
