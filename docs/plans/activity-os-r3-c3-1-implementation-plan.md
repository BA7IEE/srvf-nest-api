...
建议每命令一活动、最多 100 个绑定、2000 名参与成员、10000 个当前段；逐类多取一条检测越界，显式拒绝，禁止截断结果。10000 个段不是解锁万人档。探针沿已批准 30/500/2000 人档，额外检验边界外一条。上述新增段数预算仍待整体批准；查询计划仅完成下文的隔离库索引可用路径核验，不把建议数或该核验当成现有延迟合同。
...
| 现场补录创建新 identity | 真实 App HTTP writer 持根锁时，calculate 等待；提交后仅纳入已有完整服务段，刚创建但尚未到场的 identity 不得虚增人数。 |
| 发布后场次取消／改期 effects | candidate 持根锁时，真实取消／改期 effect 各自等待；取消后保留其他场次已完成历史来源，改期后只在完整新窗口下重算来源并完成二维码重签。 |
...
空集可用性、开放段／投影落后检查必须独立有界查询或存在性探针，不能把取不到有效行当成零，也不能为验证完整性无界加载报名表。2026-09-09 已在获批 `app_test_w98` 对生产形状等价的段／事件有界 SQL 跑 `EXPLAIN (FORMAT JSON)`，在事务内关闭顺序扫描后两条计划均存在 index path、无 Seq Scan；该测试证明索引可用，不证明默认代价选择、生产数据分布或延时预算。若发现所需查询无法在现有索引预算内完成，先列精确新增索引方案，不能顺手改旧 migration。
