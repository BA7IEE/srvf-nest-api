...
> **2026-09-09 合并收口**：本计划实施已随 [#1300](https://github.com/BA7IEE/srvf-nest-api/pull/1300) 合入 `af80fa8e`，PR 18 项检查及可信审批通过，3b/4b 已重签（117 migration；258 权限；165/160 审计）。#1300 main CI 的旧紧急创建用例出现 500；#1301 仅增加脱敏诊断、保留断言，合入 `36ff609c` 后 [main CI 34369385145](https://github.com/BA7IEE/srvf-nest-api/actions/runs/34369385145) 通过，根因仍未定位。以下授权、未做项和复现进展保留为各自历史时点，不再表示尚未提交/合并。整体复审、生产与 Gate 未完成；下一步仅起草 C4 文档。
