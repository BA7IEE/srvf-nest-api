...
> **D5 仓内交付已合并 / D6 仅起草（2026-09-14）**：[#1329](https://github.com/BA7IEE/srvf-nest-api/pull/1329) 已 Squash 合入 `2609856e00b8274abb40cbd0cf5687d22d652b46`。最终 PR HEAD `5d54d60e` 的 CI、Ready 后可信审批通过；[main CI 34795854470](https://github.com/BA7IEE/srvf-nest-api/actions/runs/34795854470) completed/success，五组 Contract + E2E 均成功（main 的 report-only 审批按工作流 skipped，不冒称执行通过）。CI 满额 2000 身份对账 638ms、32 次总查询；600 秒仅为整项夹具测试上限，业务 30 秒 / 120 次与全部断言保留。下方 Draft、未合并和待 CI 为历史时点。本轮获准更正 D5 台账及起草 [D6 评审与精确计划](activity-os-r4-d6-time-ledger-review-and-plan.md)，不实施、不操作数据库；提交推送、开 PR、合并须另行确认。D6–D8、整体复审、真实业务证据归档、前端发布和生产切换未完成，T0 保持 open。

> **CI 后授权及复验更新（2026-09-14）**：#1329 保持 Draft。维护者明确批准仅将 D5 满额测试整体时限 120→600 秒，与 D4 同类夹具一致；业务 30 秒、查询 120 次及全部断言不变。w98 六项完整回归通过，耗时 78.365 秒；1/100/2000 身份对账为 38/32/221 毫秒，总查询 31/31/32，业务及授权查询均 31。CODEMAP 与 ROUTE_AUTHZ 已按最终源码刷新且检查通过；定向 lint 通过。以下 120 秒测试时限及本地旧读数为历史记录。本次只提交推送上述修正及记录，新的远端 CI 仍需验证，不合并、不启用 Gate。

> **实现验证记录（2026-09-14）**：代码提交 `6b8dd907` 已完成 34 路径实现，本地最终检查通过，待提交本条验证记录并创建 Draft PR；未合并、未上线。以下起草及验证中状态均为历史时点。

## 11. 实施验证证据与未完成边界

- 实际 34 路径，清单外 0；提交后 preflight 工作树 clean、未落后 origin/main，通过。四项令牌仅限既定红区，没有自行发放授权。
- 全仓单元 386 集 / 8,507 项通过、5 项既有 todo；随后补齐旧侧人工认定独立指纹反例，最终比较器/查询/访问定向 101 项通过。全仓 cold lint（默认 4 GB 堆曾耗尽，临时以 8 GB 重跑）、typecheck、build 通过；未改 CI 或检查规则。
- `app_test_w98` 最终独立 E2E 六项全部通过，总耗时 66.959 秒。1/100/2000 身份真实对账分别 60/33/241 毫秒，总 SQL 31/31/32；2000 档多出的仅是连接空闲健康检查 `SELECT 1`，仍计入 120 次预算，业务及授权查询三档均 31。保留 30 秒事务上限和 120 秒测试时限。
- 最大档经真实封印、草稿生成任务、prepare/submit 得到 2000 身份、10000 分配修订、50000 切片、8000 桶、40000 来源。分页摘要和指纹一致，不从来源逐条反查。
- HTTP 覆盖负责人/合格历史审核者、普通成员拒绝、跨活动、跨组织范围、草稿拒绝、失效成员、停用账号、撤权及后续自创建版本不污染历史报告。零写证据比较全部 public 表的行数及有序内容摘要，并额外核验旧结果、新桶与审计不变。
- 曾在并行重型检查期间发生满额夹具准备超过 120 秒及后续零写断言失败；该轮作废，不作为成功证据。未放宽时限/断言，停止竞争任务后原测试串行通过。不能据此宣称所有环境下的历史间歇超时根因已消除。
- Contract 1062 项、两份 snapshot 通过。差异逐项核对：单一 shadow GET 与五个 DTO，旧路由/字段无删除；原 D4 路由闭集保留八项并加入本次批准的读接口。
- Harness 三组 548/138/68 项通过，已知缺口原样保留。counts/CODEMAP/RBAC_MAP/OpenAPI/13 客户端/authz/权限管辖面/边界元数据与债务/readtax/migration 计数及两份台账检查通过。签字登记对拍通过，无新增 migration/权限/审计，无需重签 3b/4b。
- 权限 `activity.time-settlement.read` 原说明仍准确覆盖冻结桶与证据，仅新增同资格派生报告；基线端点 5→6，显式逐项复核说明不变。全仓端点 634，migration 121、权限 265、审计 169 总计/164 活跃。

本次未做：PR 合并、生产操作、Gate 启用、业务数据删除、D6–D8、整体跨模型复审、前端页面及发布、真实业务影子验收。永久证据位置、访问主体和备份仍须维护者指定；临时测试输出不作为永久归档。全量 E2E 由 PR CI 继续核验，本地仅 w98 定向验证。
