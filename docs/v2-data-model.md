...
|---|---|---|---|---|
| **闭集**(状态 / 角色机)| `cert_status` / `activity_status` / `registration_status` / `attendance_sheet_status` / `attendance_status` / `attendance_role` / `gender_requirement` | 真实闭集值 | 业务状态机 / 活动报名性别闸依赖,长期契约 | ✅ 受保护 |
...
| **占位 / 开放分类** | `node_type`(**已内置 8 真实分类 + SRVF 根 / 15 部门**,2026-06-21 组织树 goal;items 仍开放可改)/ `work_nature`(本次未给值)/ `cert_type` / `cert_sub_type` / `content_type`(待运营细化) | node_type 真实值;余占位 demo / 初始集 | node_type 4×`professional-*` 长期契约(team-join 依赖) | ❌ 项可由运营增删改 |
...
- **`memberNo` 软删后唯一性**:**全局唯一,永不复用** — 新建 member 撞旧 memberNo 抛 `MEMBER_NO_ALREADY_EXISTS`(150xx 段位,409)。这是**有意选择**:救援队"编号即身份印记",历史溯源价值高于编号回收。
  ⚠️ **兑现它的是两条独立理由,不是一条**(2026-08-22 订正):① 软删 member 的 memberNo 仍占着 `Member.memberNo` 的全局唯一索引位;② `MemberNoReservation` 台账(**只增不删**)占着那些**曾经发出去、现在不挂在任何人身上**的号。
  ② 是补上的那一半:`correctIdentity` 是**原地 update**,`A001` 订正成 `A999` 之后 `Member` 表里**一行都不剩** —— 只看 ① 的话预检直接放行,旧号就被重新发出去了,而 `memberNo` 同时是登录识别锚(`auth.service` 按 memberNo 兜底查)。
  ⭐ **执法位是台账上的 DB 唯一约束,不是应用层预检** —— 招新发号(`recruitment-promotion`)**从不调** `assertMemberNoUnique`,它靠 P2002 兜底转 28042,同样被这条约束管住。
  刻意**不做**:台账无 `status` 列、无软删、无释放 / 恢复入口 —— 号烧了就是烧了;要释放另行立项。直接推论:**订正回不去**(`A→B` 之后再改回 `A` 会被拒,对本人也成立)
- **物理删除**:**禁止**(沿用 baseline §10)
...
- ℹ️ 非敏感分类字典取值(国标参照 gender / blood_type / … / ethnicity + 队内 member_grade 级别名 / activity_type 活动类别 + node_type 组织节点类别)已内置 `prisma/seed.ts` seed;**node_type 已内置 8 真实分类 + SRVF 根 / 15 部门**(2026-06-21 组织树 goal);work_nature 仍占位(本次未给值)
- ❌ 完整 Prisma `model {}` 块
- ❌ Migration SQL
