...
| Phase 4 removal | 🔄 进行中 | 4a #263 / 4b #264 / 4c PR | **4a auth+health + 4b system + 4c admin(62 路由,attachments 除外)老路径已删**(legacy mobile-like controller 暂留;contract 300 + full e2e 1794 绿)。余 **4d**(删 3 个 legacy mobile-like controller + 迁 `/api/users/me` 探针 spec + smoke)/ **4e**(attachments:flip admin + 建 `app/v1/my/attachments` + 删 orphan + 移除 deprecation 后处理 + 终态断言 + 注释脚手架 / 模块 CLAUDE.md 清理) |
