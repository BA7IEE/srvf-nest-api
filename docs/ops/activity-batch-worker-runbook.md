...
>
> 合同 §16.1 第 ⑦ 条的机器判据:`pnpm cutover:check`(`scripts/cutover-check.ts:996-1011`)。
>
...
(`scripts/cutover-check.ts:1004-1009`),本文不改口径。
...
合同 §16.1 第 ⑦ 条要求「有健康检查」,判据 7c 把它列为**待维护者确认**
(`scripts/cutover-check.ts:1004-1009`)。下面只列**事实与代价,不给推荐**;
拍板之后由另一刀实现,本文届时同步更新 §2。

| 选项 | 要做什么 | 代价 | 拍板后 §2 会变成 |
|---|---|---|---|
| **A. 接受由 lease 恢复代偿** | 零代码。把 §2.3 四个盲区作为**已知并接受的风险**记录在案 | 零成本。盲区 ①②③④ 全部保留:单实例部署时故障不可见也不可恢复;卡住的进程永不被发现 | §2 保持现状,措辞从「没有」改为「**明确不设**」 |
...
| §6 | `scripts/cutover-check.ts:1004-1009`(7c 原文) |
