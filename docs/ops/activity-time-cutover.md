...
本文说明 D8-1 代码提供的只读预检和单次切换命令，不是生产执行授权。代码合并、部署、
v1.1 Gate 启用、结算只读窗口和 D8 正式切换是五个分开的决策。AI 不得因为本文存在就执行
`--execute`，也不得替维护者开关、部署、生产迁移或删除收据。

D8-1 只建立切换事实、统一真相选择器和正式证明。D8-2 合入之前，现有官方统计、直方图和新关账尚未
接入该选择器，因此禁止提前写入切换收据。
...
D8-1 候选没有执行任何生产 migration、切换、部署、Gate 修改、历史回填、重分类或业务数据删除。
D8-2 官方消费者接线与 D8-OPS 真实窗口仍待后续独立交付。
