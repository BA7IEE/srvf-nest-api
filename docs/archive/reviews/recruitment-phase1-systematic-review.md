...
- **修法方向(设计级 → 见 §决策-1,R0 不自定)**:首选「人工 resolve 闸放开 `pending_verification`」——把 `:355` 的可解状态由 `{manual_review}` 扩为 `{manual_review, pending_verification}`,使卡死行变为 admin 可清(approve→发号 / reject;`:213` 审计已留 matched 证据供裁断)。**因触及状态机 surface,必须拍板。**
  - **【2026-06-19 二次收紧,见文末 §R1b】**:Option A 放开后发现 `pending_verification` **同时是核验在途态**(真腾讯云通道接上后每个大陆报名在调 verify 的数秒窗口都停此态),已收紧为「仅 `verifyOutcome` 已落的真卡死行可解、在途行不可碰、mismatch 卡死行只能 reject」。
...
| 项 | 改动 | file:line(改后) |
|---|---|---|
| **FM-A** | 人工 resolve 可解态 = `manual_review` ∪ `pending_verification`;审计 `before.statusCode` 改读实际前态;Swagger summary + 注释 true-up。**【2026-06-19 二次收紧,见 §R1b】** | `recruitment-applications.service.ts:361`(闸)/ `:396`(审计 before)/ admin controller summary |
...
## §R1b FM-A 二次收紧(2026-06-19 拍板后)

**背景**:R1 FM-A Option A 把人工 resolve 可解态扩为 `manual_review ∪ pending_verification`,理由是「`:213` 审计已留 matched 证据供 admin 裁断」。但 `pending_verification` **同时是实名核验在途态**——真腾讯云通道接上后,每个大陆报名在调 `verify` 的最长 8s 窗口都停此态。Option A 放开后,admin 在该窗口 resolve 会:① 抢提交流程里的自动发号(谁后写谁赢 + 烧号段);② 在核验出结果前 approve 发号 = 大陆报名**没过实名也能拿临时编号**(顺带绕容量闸)。审计虽留痕但不 gate 代码,且在途行与卡死行在库层不可分 → 不足以守住「拿临时编号必须有 matched 实名支撑」这条不变量。

**修法(B 档纯代码,零 schema,复用既有字段 `verifyOutcome`)**:
1. **核验结果前置落库**(`submit`):付费 `verify` 之后、发号 tx2 之前,先把 matched/mismatch 写入 `verifyOutcome`。在途行(verify 未出结果)`verifyOutcome` 仍空;核验已出结果但 tx2 失败的**真卡死行** `verifyOutcome` 已落 → 两者库层可分。
2. **resolve 闸收紧**(`resolveManual`):
   - 外籍 `manual_review`(`verifyOutcome=manual`):沿用现状,admin 自由裁决(approve 发号 / reject),不变;
   - `pending_verification`:仅当 `verifyOutcome` 已落才可解;在途行(`verifyOutcome` 空)→ `28040`(沿用既有码,不新增);
   - `verifyOutcome=mismatch` 的卡死行:resolve **只能 reject、不能 approve 发号**(不给绕开实名结果的口子;→`28040`);`matched` 卡死行可 approve。

**不变量恢复**:大陆报名拿到临时编号必须有一次 matched 实名核验支撑;admin 碰不到在途行。happy-path「核验→发号」对申请人结果不变。

**file:line(改后)**:`recruitment-applications.service.ts` submit 前置落库(verify 审计后、tx2 前)/ `resolveManual:379-391`(在途闸 + mismatch-approve 闸)/ admin controller resolve summary + 注释 true-up;`biz-code.constant.ts` 28040 注释扩。

**DoD 自证(2026-06-19 亲核)**:`pnpm typecheck` ✔ · `pnpm lint`(--max-warnings 0)✔;`pnpm test`(unit)**45 / 1477**(零变化——FM-B 单测在 tx1 即失败,不触前置落库);`pnpm test:e2e`(full)**95 suites / 1895**(1893→+2:recruitment 21→**23**,在途行 resolve approve/reject 均 28040 + mismatch 卡死行 approve 28040/reject 可;既有 FM-A 卡死恢复 ×2 + FM-C 容量 28031 三用例 true-up = 卡死行先具备 `verifyOutcome` 方可救/触达容量闸,断言不变);`pnpm test:contract` **337**(snapshot 仅 resolve summary 1 行 diff);`docs:rbacmap:check`(136 码)/ `docs:codemap:check` 0 FAIL(recruitment service 544L < 700)。**无 schema 变更,migration 仍 19;零新码/路由/模块**。

**业务语义零变化**:招新流程 / 状态机 happy-path / 两层身份(members 零增长)/ 各端点契约不变;既有 happy-path + 安全面 e2e 断言零改。

*R1 结束。系统性审查 + 统一修复闭环 + FM-A 二次收紧;报告冻结。*
