...
2. **副队长 副队长乙** — PA(SRVF, 副队长)+ SA(SECT,tree)+ SA(SSD,tree)。副队长 policy = 薄/无(R5)⇒ 头衔不赋管辖。看 SECT 队员 → 命中 supervision(SECT,tree) **ALLOW(只读、敏感 masked)**;看 SWRT → 无覆盖 **DENY(out_of_supervised_scope)**。✅ 验证"副队长只能分管范围内可见"。
