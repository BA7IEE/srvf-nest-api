...
>
> **2026-07-03 R13 前向修订:文中真实人名/编号已替换为化名与示例编号(review #484 G1,维护者拍板选项 a);结构语义不变;历史 commit 含原文,维护者知悉接受。**
>
> **2026-07-14 R5 定向取代(v0.49.0):** 经维护者 goal 明文授权，旧「副职零 policy」收敛为
> 「副职只自动派生对应正职角色的只读投影，scope=TREE，恒零写码/零 `*.read.sensitive`」。
> `vice-captain` / `dept-deputy` → `org-readonly`，`deputy-group-leader` →
> `group-readonly`；管辖写权仍只能来自正职、分管或显式 `RoleBinding`。本稿下文凡称
> 「副职无 policy / 仅分管可见」均按本条 supersession 解释；实施冻结件见
> [`department-data-scope-v0.49.0-review.md`](department-data-scope-v0.49.0-review.md)。
...
| 「X 在 Y 任 Z 职」 | 队长甲 在 SECT 任队长 | **OrganizationPositionAssignment**(任期 2026-07-01~2027-06-30) | 任职 = 人×组织×职务×任期 |
| 分管队长分工 | 副队长乙(副队长)分管 SECT、SSD | **OrganizationSupervisionAssignment**(`scopeMode=tree`) | **不混进副队长职务** |
...
>
> **R8 — `requireMembership` 按(nodeType, position)可配:** 总队级领导(`headquarters` 的队长/副队长)规则设 `requireMembership=false` —— 总队长/副队长**不必"归属"于根**(其 PRIMARY 归属在本队);组长/副组长规则设 `requireMembership=true`(应在本组织子树内有 active 归属)。这落实"归属 / 任职 / 分管三轴独立"(副队长乙、副队长甲、分管人甲:三者 org 各不相同)。
...
model OrganizationPositionAssignment {
  ...
  isConcurrent      Boolean        @default(false)    // R2:兼任标记(回填公告"（兼）",如副队长甲兼 SAMT 队长;不影响授权,两条 PA 都生效)
  note             String?
  createdAt        DateTime        @default(now())
  updatedAt        DateTime        @updatedAt
  deletedAt        DateTime?
...
**Partial unique(手写):** `... (supervisorMemberId, organizationId) WHERE deletedAt IS NULL AND status='ACTIVE';`(同人对同组织一条 active 分管)。**副队长乙分管 SECT、SSD = 两行;与他的"副队长"职务(Assignment)正交。**
...
>
> **🔴 R5 安全红线 —— 副职不推导正职管理角色:** `副队长 / 副部长 / 副组长` 的 PositionRolePolicy 仅映射 v0.49.0 新增的对应只读投影角色，恒零写码/零 `*.read.sensitive`。写管辖权**只能**来自正职、`SupervisionAssignment`(分管)或显式 `RoleBinding`。**严禁**让副职头衔自动推导管理角色 —— 否则总队 6 名副队长各得近全组织管理。seed 默认 policy 覆盖正职管理 + 副职只读；CI 断言副职 policy 与只读投影精确相等(见 §10.5)。
>
...
> 1. **副职不自动推导管理角色。** `vice-captain / dept-deputy / deputy-group-leader`(副队长/副部长/副组长)经 §3.7 policy 仅得到对应正职码集的只读投影；下方步骤 3b 的职务推导对副职恒零写 grant。
> 2. **分管独立于职务。** 副职头衔只给任职组织 TREE 内的非敏感只读可见性；额外管辖范围或任何写权只能来自正职、显式 `RoleBinding`(3a)或 `SupervisionAssignment`(3c)。分管仍不由副职头衔隐式推导。
...
explain(user, action, ref):
  ...
  ##     🔴 R5:副职(vice-captain/dept-deputy/deputy-group-leader)只命中只读投影 policy → 本步对副职不产出写/敏感 grant
...
- **PositionAssignment → 虚拟 RoleBinding:** 不落库,decision 时由 `PositionRolePolicy` 动态推导,scope = 任职组织 + policy.scopeMode。**🔴 R5:仅正职有管理 policy;副职只映射只读投影,不产出写/敏感 grant。** 正职默认映射(**BD-1**;同一 `team-leader` 职务按 `conditionJson.nodeTypeCode` 分流,避免每个队长都成 org-admin):`team-leader@headquarters → org-admin@TREE(root)`(全组织业务管理,**不含**平台/RBAC/`*.read.sensitive`)、`team-leader(非 root)/dept-leader → *-manager@TREE(本组织)`、`group-leader → group-manager@EXACT/TREE`。v0.49.0 追加 `vice-captain/dept-deputy → org-readonly@TREE`、`deputy-group-leader → group-readonly@TREE`。
...
### 场景 1 — 应急通讯队队长甲 能否管理 应急通讯队行动组 考勤?

- **入库:** `Org(SECT-行动组, parent=SECT, nodeType=group)`;`PositionAssignment(队长甲, SECT, team-leader, 2026-07-01~2027-06-30, ACTIVE)`;`PositionRolePolicy(team-leader, conditionJson={nodeTypeCode≠'headquarters'} → team-manager@TREE)`(非总队队长走 team-manager;总队队长见场景 6),`team-manager` 含 `attendance.read.sheet` / `attendance.approve.sheet`(一级)、**不含** `attendance.final-approve.sheet`。
...
### 场景 2 — 行动组组长甲 能否管理 行动组 考勤?

- **入库:** `PositionAssignment(组长甲, SECT-行动组, group-leader, ...)`;`PositionRolePolicy(group-leader → group-manager@EXACT)`,`group-manager` 含 `attendance.read/approve.sheet`。
...
### 场景 3 — 副队长乙(分管 SECT、SSD)能否查 SECT 数据?能否查 水上搜救队(SWRT)数据?

- **入库:** `PositionAssignment(副队长乙, SRVF/队部, vice-captain, ...)`;`SupervisionAssignment(副队长乙, SECT, TREE, ACTIVE)` + `SupervisionAssignment(副队长乙, SSD, TREE, ACTIVE)`。分管推导 → `org-supervisor`(只读)over `TREE(SECT)` ∪ `TREE(SSD)`。
...
- **结论:** 分管把"副队长"从"全队可见"收敛为"仅分管范围可见",且只读 + 不破敏感闸门 —— 正是 goal 要的语义。`org-supervisor` 默认码集见 **BD-3**(只读 6 码);副队长乙若要审批/终审 SECT 事务,须**另加显式 `RoleBinding`**,头衔与分管都不赋予写权。
...
- **入库:** `PositionAssignment(总队长, SRVF 根, team-leader)`(队长 = §3.2 六职务之一,**无单独 "captain" 职务**);`PositionRolePolicy(team-leader, conditionJson={nodeTypeCode:'headquarters'} → org-admin@TREE)`(BD-1:仅 root 队长映射 org-admin;非 root 队长走 team-manager,见场景 1);`org-admin` = 业务面广管理权限(≈ 现 biz-admin 集,但**排除**平台/RBAC/`*.read.sensitive`)。
...
- **人员锚定:** 姓名仅作展示,确认键 = `memberNo` 唯一命中 active member。**同名/带编号(组长庚 90001 vs 90002、组长辛 90003)**:仅当 staging 行带明确 `memberNo` 且唯一命中才可确认;姓名多义 → 阻断,标记"需人工指定 memberId"。
...
| 14 | 副队长头衔全队只读，分管/显式绑定仅叠加额外能力 | e2e | `vice-captain@root` 读允许、写拒绝；分管 grant 仍独立可解释 |
...
| 23 | **副职恒只读(R5 v0.49 红线)** | unit + e2e | 三副职 PositionRolePolicy 只指向对应只读投影；读本树 ALLOW，写/敏感 action → DENY |
| 24 | 兼任(R2)| e2e | 副队长甲 两条 active PA(SRVF 副队长 + SAMT 队长 `isConcurrent`)并存;授权按各自 scope 独立判,命中即记 matchedGrant |
...
- **🔴 R5 副职恒只读(v0.49 定向取代):** `副队长 / 副部长 / 副组长` 头衔仅派生对应正职的非敏感只读投影，恒零写；额外管辖仍只来自正职、分管或显式绑定，分管独立于职务(详见 §5.2 / §3.7)。
...
| **🔴 副职头衔自动得管理 scope(R5)** | 误给副队长/副部长/副组长配管理 policy 或给只读角色混入写/敏感码 | **副职只映射两个只读投影角色**；码集从正职动态过滤；**CI/seed 断言 policy 精确相等且零写/零敏感** |
...
| **PR7** | 职务→角色 policy | `OrganizationPositionRolePolicy` + seed 正职管理映射；v0.49.0 追加副职只读投影(**🔴 恒零写/零敏感,R5** + CI/seed 断言)+ `org-admin`(队长@root,BD-1,排除平台/敏感/RBAC 码)与 `org-supervisor`(BD-3 只读码集)角色 | 加配置(未接授权) |
...
|---|---|---|---|---|---|---|
| 总队 | SRVF / 深圳公益救援队 | 队长/副队长 | 队长 ×1(总队长)| 副队长 ×6 | —(组挂各队/部下)| — |
...
| 高空绳索技术队 | STRT / 高空救援队 ⚠名漂移 | 队长/副队长 | ×1 | ×3 | 训练/装备/文书/**飞辅**(含 组长庚#90001)| — |
| 医疗辅助队 | SAMT / 医疗辅助队 | 队长/副队长 | ×1 **副队长甲(兼)**| ×3 | 训练(含 组长庚#90002)/装备/文书/**心理**/**行动** | — |
| 应急通讯队 | SECT / 应急通讯队 | 队长/副队长 | ×1(队长甲)| ×3 | 行动(1+3)/装备(1+3)/文书(1+3)/训练(1+3)| — |
| 信息指挥中心 | ICC / 信息指挥中心 | **部长/副部长** | ×1 | ×1 | 统筹/技术/标准/文书/**无人机** | — |
| 特勤部 | SSD / 特勤部 ⚠分管段称"特殊勤务部" | 部长/副部长 | ×1 | ×3 | 物资(6)/交通(5)/文书(5)+ **炊事保障组(筹)** | 1 |
| 行政外联部 | APD / 行政外联部 | 部长/副部长 | ×1(部长甲)| ×3 | **文秘**/传媒/外联 | — |
| 志愿者管理部 | VOD / 志愿者组织部 ⚠名漂移 | 部长/副部长 | ×1 | ×1 | 招新/宣导/荣誉 | — |
| 技术培训部 | TTD / 技术培训部 | 部长/副部长 | ×1 | ×3 | 培训/文书(含 组长辛#90003)/**教委执行** | — |
| 少年辅助队 | STAT / 少辅队 ⚠名漂移 | **部长/副部长**(队却用部长)| ×1 | ×1(副部长甲)| 总务/导师/文书/项目 | — |

**分管(12 条,7 人):** 总队长→STAT;副队长丙→SURT、TTD;副队长乙→SECT、SSD(称"特殊勤务部");分管人甲→STRT(称"高空救援队")、ICC;分管人乙→SWRT;副队长甲→SAMT、SMRT;分管人丙→APD、VOD。
**关键观察:** 公告把"队长/副队长"列为总队头衔(seniority),又**单列"分管队长分工"把每个具体管辖责任分配出去**——连总队队长总队长都另有一条"分管 STAT"。这说明真实组织里**头衔(rank)与管辖范围(scope)是解耦的**,管辖权主要由"分管"承载,而非头衔自动赋予 → 直接验证 §5 的"职务不天然 = 管理员",并引出 R5。

### 12.2 公告文本 → 模型对象 映射表

| 公告文本(代表句)| Organization | Position | PositionAssignment | SupervisionAssignment | RoleBinding(经 policy/显式)| Membership |
|---|---|---|---|---|---|---|
| 队长:总队长(总队)| SRVF(已存)| 队长 | (SRVF, 队长, 总队长, 任期)| 另见分管(总队长→STAT)| policy→org-admin@tree(root) **或薄(R5)** | 总队长 PRIMARY 在其本队;root 不强制归属(R8)|
| 副队长:副队长丙…(总队 ×6)| SRVF | 副队长 | (SRVF, 副队长, 每人)×6 | 见分管段(各自分管)| **薄/无(R5)**;实际权来自分管 | 各自本队 |
| 山地救援队 队长:队长乙 | SMRT(已存,code 对齐)| 队长 | (SMRT, 队长, 队长乙)| — | team-manager@tree(SMRT)| 队长乙 PRIMARY=SMRT |
| 训练组组长:组长乙、组长丙、组长丁 | **SMRT-训练组(新建,group,parent=SMRT)** | 组长(allowMultiple)| (SMRT-训练组, 组长, ×3)| — | group-manager@tree(组)| 三人 membership 落 SMRT 子树 |
| 潜水组(筹)组长:组长戊 副组长:副组长乙 | **SWRT-潜水组(新建,group + provisional 标记,R1)** | 组长 / 副组长 | (…,组长,组长戊)/(…,副组长,副组长乙)| — | 组长 group-manager;副组长 薄(R5)| 子树 |
| 队长:副队长甲(兼)(SAMT)| SAMT | 队长 | (SAMT, 队长, 副队长甲, **isConcurrent=true**,R2)| 见分管(副队长甲→SAMT、SMRT)| team-manager@tree(SAMT)| 副队长甲 PRIMARY 可能在别处 |
| 飞辅组组长:组长己、组长庚(90001)| STRT-飞辅组(新建)| 组长 | (…,组长,组长己)/(…,组长,**member#90001**)| — | group-manager | 子树 |
| 训练组组长:…组长庚(90002)(SAMT)| SAMT-训练组(新建)| 组长 | (…,组长,**member#90002**)← 与 90001 异人 | — | group-manager | 子树 |
| 信息指挥中心 部长:部长乙 | ICC(已存)| **部长**(中心用部长)| (ICC, 部长, 部长乙)| — | dept-manager@tree(ICC)| PRIMARY=ICC |
| 少年辅助队 部长:部长丙 | STAT(已存,名漂移 R7)| **部长**(队却用部长,R6)| (STAT, 部长, 部长丙)| — | dept-manager@tree(STAT)| PRIMARY=STAT |
| 行政外联部 部长:部长甲 | APD | 部长 | (APD, 部长, 部长甲)| — | dept-manager@tree(APD) **+ 显式 attendance-final-reviewer@tree(root)**(APD 终审中枢)| PRIMARY=APD |
| 副队长乙分管:应急通讯队、特殊勤务部 | —(SECT、SSD 已存)| — | — | (副队长乙→SECT,tree)/(副队长乙→SSD,tree)×2;"特殊勤务部"=SSD(R7)| org-supervisor 只读 @这两树 | — |

### 12.3 真实样例端到端验证(12 个)

> 每例:数据如何入库 → AuthzService 如何命中 → 结论。

1. **总队队长 总队长** — PA(SRVF, 队长)。policy 把"队长"映射为 `org-admin@tree(own org)`;own org=SRVF 根 ⇒ scope=tree(root) 覆盖全组织。⇒ 在全组织业务面 **ALLOW**(matchedGrant: position, root, tree),但不含平台码 ⇒ 平台/凭证类 **DENY**(= 组织顶级管理员 ≠ SUPER_ADMIN,场景 6)。**注:** 是否让"队长@root"自动得全组织管理,是 policy 选择(R5);公告同时给总队长"分管 STAT"显示真实意图更偏"管辖经分管下放"。
2. **副队长 副队长乙** — PA(SRVF, 副队长)+ SA(SECT,tree)+ SA(SSD,tree)。v0.49.0 起副队长 policy → `org-readonly@TREE(root)`，看 SECT / SWRT 队员均 **ALLOW(只读、敏感 masked)**，任何写 action **DENY**；两条 supervision grant 仍独立保留并可在 explain 中命中。✅ 验证「副队长头衔只给全队只读，分管不被隐式替代」。
3. **专业队队长 队长乙(SMRT)** — PA(SMRT, 队长)→ team-manager@tree(SMRT)。管 SMRT-训练组考勤(组 ∈ SMRT 子树,closure)→ 一级 **ALLOW**;终审 → **DENY**(终审属 APD 中枢)。✅
4. **职能部门部长 部长甲(APD)** — PA(APD, 部长)→ dept-manager@tree(APD);**额外**显式 RoleBinding(principalType=POSITION_ASSIGNMENT, role=attendance-final-reviewer, scope=tree(root))。⇒ 可对**任意部门**考勤终审(场景 4),叠加自审约束(若自己是 submitter → DENY)。✅ **注:** "由 APD 承担全队终审"是 schema 现有注释口径;究竟哪个部门是终审中枢可换(模型用显式绑定,可改),非写死。
5. **组长 组长甲(SECT-行动组)** — PA(SECT-行动组, 组长)→ group-manager@tree(行动组,叶子=EXACT)。管本组考勤一级 **ALLOW**;终审 **DENY**。✅
6. **副组长 副组长甲(SECT-行动组)** — PA(SECT-行动组, 副组长)→ 薄/只读(R5)。读本组 **ALLOW**;写/终审 **DENY**。✅ 验证副职不自动得管理权。
7. **筹备组 潜水组(筹)组长戊(SWRT)** — Org(SWRT-潜水组, group + **provisional**,R1, parent=SWRT)+ PA(组长)。授权与普通组一致;转正 = 清 provisional 标记,**任命/历史不丢**(R1 优于"独立 nodeType")。✅
8. **分管队长 分管人甲** — SA(STRT,tree)+ SA(ICC,tree),在这两组织**无任何 position**。看 STRT/ICC 数据 → 命中 supervision **ALLOW(只读)**;看其它 → **DENY**。✅ 验证分管是独立授权源、可跨"专业队 + 职能部门"。
9. **同名带编号 组长庚(90001) vs 组长庚(90002)** — 两个 Member(memberNo 90001 / 90002),各落不同 PA:(STRT-飞辅组, 组长, #90001)与(SAMT-训练组, 组长, #90002)。导入**必须按 memberNo 锚定**,姓名"组长庚"绝不可自动落库。✅ 验证 §8.4 导入铁律;另 组长辛(90003)虽本公告只现一次也带号 ⇒ 说明编号是 membership 系统 memberNo、作者已预消歧。
10. **兼任 副队长甲(兼)** — 两条并存 PA:(SRVF, 副队长)+(SAMT, 队长, **isConcurrent=true**,R2);再加 SA(SAMT,tree)+ SA(SMRT,tree)。⇒ 同一人:1 个总队副职 + 1 个队正职(兼)+ 2 条分管。授权按每条独立判、命中即记 matchedGrant。✅ 验证 `allowConcurrent` + 职务/分管正交;`isConcurrent` 标记忠实回填"（兼）"。
11. **部长领衔的"队" 部长丙(STAT)** — STAT 是 rescue-team(队),却用"部长/副部长"。PA(STAT, 部长)。要求 OrganizationPositionRule(rescue-team) **同时允许 队长 与 部长**两种领导职务(R6),由实际任命择一。✅ 模型可表达,但 Rule 不能强制"一类组织唯一领导命名"。
12. **多人共职 SURT-训练组(6 组长)/ SSD-物资组(6 组长)** — Position(组长, allowMultiple=true, maxCount=null)。6 条 PA 并存合法。✅ 验证 allowMultiple + 不设上限默认。

**三轴独立的强验证(副队长乙):** 他的"归属(Membership=某本队)/ 任职(Position@SRVF 根)/ 分管(Supervision@SECT、SSD)"三者组织全不同 —— 模型把"在哪里属于、领导什么、监督什么"三轴彻底解耦,这是真实公告反复出现的形态(总队长、副队长甲、分管人甲同理)。✅

### 12.4 模型经受住的验证点(无需改)

- **code 稳定、name 可漂移** → 12 组织全部按 code 命中,3 处 name 漂移不影响模型(只影响导入,见 R7)。
- **scope 锚在"任职所在组织"** → 同一个"队长"职务,总队长@root 得全组织、队长乙@SMRT 只得 SMRT 子树,无需为"总队长"单设职务。
- **三轴解耦(归属/任职/分管)** → 副队长乙、副队长甲、分管人甲、总队长全部成立。
- **职务/分管正交** → 副队长甲既是 SAMT 队长又分管 SAMT(冗余但无冲突,两 grant 并存)。
- **allowMultiple / allowConcurrent / 任期 / 历史** → 6 组长、(兼)、任期 2026-07-01~2027-06-30 全部落位。
- **memberNo 消歧** → 组长庚 90001/90002、组长辛 90003 正是 §8.4 设计的用例。
...
| b | 副队长甲(兼)、潜在多兼任 | PositionAssignment 无"兼任"结构化标记,只能塞 `note` | 回填失真 → **R2** |
...
| h | 副队长乙/总队长 任职 org ≠ 归属 org;总队级领导是否"归属"于根 | requireMembership 对总队级领导若强制=true 会逼出无意义的 root 归属 | 规则细化 → **R8** |
...
- **R8〔改 §3.3 注 + §2〕requireMembership 按(nodeType, position)可配;总队级领导可设 false。** 总队长/副队长不必"归属"于根(其 PRIMARY 在本队);组长/副组长则应要求在本组织子树内有 active 归属(一致性校验)。把"归属/任职/分管三轴独立"在 §2 显式强调(副队长乙/副队长甲/分管人甲为证)。
