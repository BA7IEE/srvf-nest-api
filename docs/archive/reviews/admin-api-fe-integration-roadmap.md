...
> **归档戳:已全量落地:F1 #502 → F2 #503 → F3 #504 → F4 #505 → F5 #506(2026-07-04 五批连续 merge-on-green;§11 拍板全按推荐 + §6.2 两 ⚠️ 决策点由 goal 代决「preview 均复用 read 码不设新码」),2026-07-04 归档;实现以 src/prisma 为准,能力图见 [`docs/handoff/admin-web.md`](../../handoff/admin-web.md)。**
> 终值(实现期精算,取代 §0/§6.3 预估):权限码 **195**(+4:meta.resolve.label / authz.explain-batch.decision / authz.action-state.decision / membership.transfer.record)· ops-admin **94** · biz-admin **73** · org-admin **57**(派生自动继承)· EXPECTED_ROUTES **319**(+27)· controller **66**(+3:meta / action-state / memberships-admin)· 模块 **35**(+1 meta)· migration **39 不变** · 角色 **7 不变** · AuditLogEvent **+1**(membership.transfer,goal 显式预授权)· BizCode **+0**。
>
> **性质**:规划路线图 + 决策清单。**本文只做分析规划,不改任何 `src` / `prisma` / `seed` / 测试 / 端点**;实现另起 per-batch goal(process §7.1)。
> **状态**:🗄 **已归档**(原「冻结待拍板」→ 2026-07-04 §11 全按推荐拍板 → F1–F5 全量落地后按 scoped-authz 先例归档至本目录;正文自 §0 起为拍板时点冻结快照,不再更新)。
