...
> **状态：维护者已于 2026-09-03 确认 B2 方案 A；仍未授权任何 B2 实现。** 本决定只冻结 B2 的边界和后续授权预算；当前工作只允许维护本评审稿及 CI 强制的冻结稿台账分类，不授权 Prisma schema、migration、测试基础设施、运行时代码、接口或生产部署。
>
...
这项决定确认了上述数据合同、转换包络和“歧义不投影”规则，并允许完成这份 review PR 的收口；它不授权 B2 implementation。

实际 schema、migration、测试或运行时代码仍须在 review PR 独立合入后，按真实 diff 重新申请授权；第 107 条 migration SQL 逐行审查完成后，还必须单独确认：
