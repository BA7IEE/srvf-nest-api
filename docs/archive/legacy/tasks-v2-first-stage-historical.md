# TASKS.md V2 第一阶段与 V2.x 历史任务归档

> 本文件由根目录 [`TASKS.md`](../../../TASKS.md) 的 `§6` ~ `§10` 迁档而来。
>
> **归档日期**:2026-05-23
>
> **当前事实**以 [`docs/current-state.md`](../../current-state.md) 为准;**版本变更**以 [`CHANGELOG.md`](../../../CHANGELOG.md) 与 [`docs/archive/handoff/`](../handoff/) 为准。
>
> 根目录 [`TASKS.md`](../../../TASKS.md) 仍保留 `§6` / `§7` / `§8` / `§9` / `§10` 章节锚点,避免破坏历史 handoff、V2 红线、批次文档与设计期文档中的引用。
>
> 本文件是**历史快照**,不作为当前任务源;除非修复链接或明显笔误,不应继续维护其中的任务状态。
>
> **PR-D 归档边界**:`TASKS.md` 原 `§6` ~ `§10`(lines 70-915,从 commit `6088ef6` 起)完整迁出,**未做内容编辑**;原各 Step / 批次任务卡的"完成情况"事实块原样保留。`§0` 历史归档索引与 `§0.3` 章节编号说明继续保留在根目录 `TASKS.md`。
