# Architecture v1 Blueprint (Historical)

> **Status**: archived historical material.
> **Source**: `ARCHITECTURE.md §1-§10` + 附录(实施顺序) moved verbatim from [`ARCHITECTURE.md`](../../../ARCHITECTURE.md) at commit `d81c2ab` (PR-6 `docs: rewrite ARCHITECTURE.md as top-level entrypoint`, archived 2026-05-22).
> These sections are retained for traceability only and **are not the current execution authority**.
>
> **Active authorities承接 from these sections** (read these instead of the archived body):
> - **Long-term iron laws** (naming / response wrapping / Swagger / pagination / Guard / soft delete / role hierarchy / password / DTO separation / transaction / config attribution / env validation): [`AGENTS.md`](../../../AGENTS.md) §1-§14
> - **V2 baseline规范** (BizCode 段位 / 命名 / DTO 白名单 / Swagger / config / 日志屏蔽 / soft delete / v1 兼容 / 时区 / 验收门槛): [`docs/srvf-foundation-baseline.md`](../../srvf-foundation-baseline.md)
> - **V2 红线与复活路径** (A/B/C/D/E 五档 + Slow- 类清单): [`docs/V2红线与复活路径.md`](../../V2红线与复活路径.md)
> - **API surface 长期边界** (App / Admin / Root / Public 三前缀策略): [`docs/api-surface-policy.md`](../../api-surface-policy.md)
> - **架构边界 / 服务抽离决策** (Presenter / QueryService / PolicyService / StateMachine / AuditRecorder / Effect): [`docs/architecture-boundary.md`](../../architecture-boundary.md)
> - **当前事实** (版本 / open PR / 已发能力 / surface 状态): [`docs/current-state.md`](../../current-state.md)
> - **§9 升级路径表 是 active**: 完整原表保留进新版 [`ARCHITECTURE.md §3`](../../../ARCHITECTURE.md);本归档版本仅作历史记录,外部对 `ARCHITECTURE.md §9` 的引用继续指向新版顶层文档,不指向本文
>
> **Paths**: original sections used repo-root-relative paths (e.g. `docs/security.md`, `CLAUDE.md §X`). Those strings are preserved verbatim below — they no longer resolve as Markdown links from this nested archive location, but the textual references are still meaningful when read from repo root.
>
> **Original section anchors covered by this file**:
> - §1 设计原则 / §2 技术栈 / §3 项目结构 / §4 v1 范围 / §5 数据模型
> - §6 API 接口清单 / §7 命名与编码约定 (7.1-7.11) / §8 环境变量
> - §9 升级路径(原文保留,但 active 引用以新版 [`ARCHITECTURE.md §3`](../../../ARCHITECTURE.md) 为准)
> - §10 部署 / 附录 实施顺序
>
> **Excluded** (handled in separate archives):
> - §11 V1.1 Engineering Hardening → [`architecture-v1-1-hardening.md`](./architecture-v1-1-hardening.md)
> - §12 V2 派生项目方向 → [`../plans/architecture-v2-first-stage-blueprint.md`](../plans/architecture-v2-first-stage-blueprint.md)
