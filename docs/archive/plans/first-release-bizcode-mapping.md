...
- **第一版前端 BizCode 翻译表唯一权威源**:覆盖 `src/common/exceptions/biz-code.constant.ts` 中**全部 125 条** BizCode(P0-E PR-3 #127 新增 `REFRESH_TOKEN_INVALID=10007` 后实数;P0-D PR-3 #117 新增 `OLD_PASSWORD_INVALID=10005` / `NEW_PASSWORD_SAME_AS_OLD=10006` 前为 122 条,P0-G 撰写时为 122 条)
...
P0-G 撰写时(基于 `main` HEAD `a240e0a`)`biz-code.constant.ts` 实数为 **122 条**;P0-D PR-3(#117,`main` HEAD `8a70573`)新增 `OLD_PASSWORD_INVALID=10005` / `NEW_PASSWORD_SAME_AS_OLD=10006` 后实数为 **124 条**;P0-E PR-3(#127,`main` HEAD `25f03fb`,2026-05-18)新增 `REFRESH_TOKEN_INVALID=10007` 后实数为 **125 条**(精确点数命令 `grep -cE "^  [A-Z][A-Z0-9_]+: \{" src/common/exceptions/biz-code.constant.ts`)。本文 §4 全量翻译表对应 125 条;若实数与本文记数不一致,以源码为准。
...
> 本节按段位顺序列出全部 **125 条** BizCode(2026-05-18 `main` HEAD `25f03fb` 实数;P0-G 撰写时为 122 条 + P0-D PR-3 #117 新增 10005 / 10006 = 124 条 + P0-E PR-3 #127 新增 10007 = 125 条)。
>
> 表列说明:
...
### 4.2 `100xx + 101xx` users / auth(9 条)
...
| 10005 | `OLD_PASSWORD_INVALID` | 当前密码不正确 | 401 | PUT `/api/users/me/password` 时 `oldPassword` 与当前 `passwordHash` 不匹配(沿 [P0-D 评审稿 §5.3](first-release-p0d-change-my-password-review.md):本人改密无账号枚举攻击面,**不复用** 10004 的模糊语义) | 改密表单"当前密码"红框 + 提示"当前密码不正确";**不要**跳登录页(已登录态),**不要**当成 10004 LOGIN_FAILED 错误重登 | 起步 |
| 10006 | `NEW_PASSWORD_SAME_AS_OLD` | 新密码不能与当前密码相同 | 400 | PUT `/api/users/me/password` 时 `newPassword === oldPassword`(严格字符串 === 比较,无 trim/toLowerCase) | 改密表单"新密码"红框 + 提示"新密码须与当前密码不同";前端可在 submit 前预校验避免后端拒绝 | 起步 |
| 10007 | `REFRESH_TOKEN_INVALID` | refresh token 无效或已过期 | 401 | POST `/api/auth/refresh` 4 子原因统一返(token 不存在 / 已撤销 / 已过期 / 重放命中;沿 [P0-E 评审稿 D-6](first-release-p0e-refresh-token-review.md) + v1 §8 防账号枚举铁律,**不**拆 EXPIRED / REVOKED / REPLAY)| **清本地 accessToken + refreshToken + 跳登录页**;**不重试** refresh(refresh 失败 4 子原因前端不应也不能分辨);沿 [frontend-scope §3.2.1 token 生命周期](first-release-frontend-scope.md) | 起步 |
...
理论上不应该出现 §4 未列出的 code(已覆盖 125 条全部);但仍有几种边界场景:
