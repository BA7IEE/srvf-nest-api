...
2. **App 可见状态白名单严格 1 态**:仅 `statusCode='published'`(沿 §3 决议 D-P2-4-1)。`draft` / `cancelled` / `completed` / `deletedAt!=null` 一律对 App 不可见;`/:id` 命中不可见 → **404**(沿现有 v2 USER `findOne` Q-A7 范式,避免存在性泄漏)。
...
4. **DTO 严格反 Admin 复用**(沿 D-7 + Phase 0.7 §2.2):新建 `AppAvailableActivityListItemDto` / `AppActivityDetailDto`,**禁止** `extends` / `Pick` / `Omit` / `IntersectionType` / `PartialType` / `OmitType` 任一 Admin DTO(`ActivityListItemDto` / `ActivityResponseDto`)。
...
10. **零变更 / api/v2/activities***:旧 7 个 admin endpoint 行为**逐字不变**(沿 phase-2-review.md §3.2 + migration-plan §5 方案 C);P2-4 新增 2 path **不**改老 Controller / DTO / Service / 权限 / Tag。
...
### 2.3 决议 D-P2-4-1(待用户拍板)

**推荐方案 A**:`/available` 与 `/:id` 一律仅 `published` 可见;`draft` / `cancelled` / `completed` / `deletedAt!=null` 在 App 端表现为"不存在"。

**理由**:
- `/available` 字面语义 = "我现在可参加的活动",`completed` / `cancelled` 已不可报名,语义上不属于"可参加"
- 单一可见集合简化前端心智模型(列表 = 详情可见集 = `published`)
- 历史 / 已结束活动若有展示需求,走 `/my/activities`(P2-5+ 评估)+ `/my/registrations`(P2-5+),按本人 member 数据拉取
...
**备选方案 C**(若用户认为应与 v2 USER 字面对齐):**仅**改 D-P2-4-1 决议,**不**改 §1 endpoint 设计 / DTO / Service / e2e 范式;`AppActivitiesService` `where.statusCode` 由单态 `'published'` 改为 `{ in: ['published', 'completed'] }`,e2e 反向断言改 1 处。
...
| 字段 | 类型 | 必返 / 可空 | 来源 | App 必要性 | 备注 |
|---|---|---|---|---|---|
| `id` | `string` | 必返 | `Activity.id` | ✅ 详情跳转必需 | cuid |
| `title` | `string` | 必返 | `Activity.title` | ✅ 列表展示 | |
| `activityTypeCode` | `string` | 必返 | `Activity.activityTypeCode` | ✅ 类型筛选展示 | 字典 code;label 由前端字典 map(沿 v2 范式) |
| `organizationId` | `string` | 必返 | `Activity.organizationId` | ✅ 承办组织展示 | id;name 由前端 org tree map(沿 v2 范式) |
| `startAt` | `Date` | 必返 | `Activity.startAt` | ✅ 时间展示 + 排序 | |
| `endAt` | `Date` | 必返 | `Activity.endAt` | ✅ 时间展示 | |
| `location` | `string` | 必返 | `Activity.location` | ✅ 地点展示 | 自由文本 |
| `description` | `string \| null` | 可空 | `Activity.description` | ✅ 列表摘要 | 沿 v0.5 Q-D14 可空 |
| `capacity` | `number \| null` | 可空 | `Activity.capacity` | ✅ 名额展示 | null = 不限名额 |
| `genderRequirementCode` | `string \| null` | 可空 | `Activity.genderRequirementCode` | ✅ 性别要求展示 | 字典 code |
| `registrationDeadline` | `Date \| null` | 可空 | `Activity.registrationDeadline` | ✅ 报名截止展示 | null = 不限 |
| `statusCode` | `string` | 必返 | `Activity.statusCode` | ✅ 状态展示 | App 端永远 = `'published'`(沿 D-P2-4-1 推荐方案 A);但保留字段便于前端泛化处理 + 后续候选方案 C 切换零 DTO 变更 |
| `isPublicRegistration` | `boolean` | 必返 | `Activity.isPublicRegistration` | ✅ 是否公开报名 | |
| `coverImageUrl` | `string \| null` | 可空 | `Activity.coverImageUrl` | ✅ 列表封面 | 裸 URL 字符串(沿 schema:**非** signed URL,**非 L3**) |
...
| `registrationSchema` | `Activity.registrationSchema` | 列表精简;详情返 |
| `galleryImageUrls` | `Activity.galleryImageUrls` | 列表精简;详情返 |
| `content` | `Activity.content` | 列表精简;详情返 |
| `locationLongitude` | `Activity.locationLongitude` | 列表精简(沿 v2 `activityListItemSelect` 返,P2-4 评估后认为列表暂无地图展示需求,**收窄不返**);详情返 |
| `locationLatitude` | `Activity.locationLatitude` | 同上 |
| `updatedAt` | `Activity.updatedAt` | 列表无展示价值;详情返 |

> **注**:与 v2 `activityListItemSelect` 差异:P2-4 列表**不返** `locationLongitude` / `locationLatitude`(沿 §4.2)。这是 App DTO 独立设计选择(D-7),**不是** v2 行为变更;v2 `/api/v2/activities` 仍返。
...
在 List DTO 15 项基础上**追加** 7 项详情专有字段;**不返** `cancelled*` / `publishedBy` 共 5 项(沿 §5.2)。
...
| `organizationId` | `string` | 必返 | `Activity.organizationId` | ✅ | |
| `startAt` | `Date` | 必返 | `Activity.startAt` | ✅ | |
| `endAt` | `Date` | 必返 | `Activity.endAt` | ✅ | |
| `location` | `string` | 必返 | `Activity.location` | ✅ | |
| `description` | `string \| null` | 可空 | `Activity.description` | ✅ | |
| `capacity` | `number \| null` | 可空 | `Activity.capacity` | ✅ | |
| `genderRequirementCode` | `string \| null` | 可空 | `Activity.genderRequirementCode` | ✅ | |
| `registrationDeadline` | `Date \| null` | 可空 | `Activity.registrationDeadline` | ✅ | |
| `registrationNotes` | `string \| null` | 可空 | `Activity.registrationNotes` | ❌ | ✅ |
| `statusCode` | `string` | 必返 | `Activity.statusCode` | ✅ | |
| `isPublicRegistration` | `boolean` | 必返 | `Activity.isPublicRegistration` | ✅ | |
| `registrationSchema` | `Record<string, unknown> \| null` | 可空 | `Activity.registrationSchema`(Json) | ❌ | ✅ |
| `coverImageUrl` | `string \| null` | 可空 | `Activity.coverImageUrl` | ✅ | |
| `galleryImageUrls` | `string[] \| null` | 可空 | `Activity.galleryImageUrls`(Json) | ❌ | ✅ |
| `content` | `Record<string, unknown> \| null` | 可空 | `Activity.content`(Json) | ❌ | ✅ |
| `locationLongitude` | `string \| null` | 可空 | `Activity.locationLongitude`(Decimal → string) | ❌ | ✅ |
| `locationLatitude` | `string \| null` | 可空 | `Activity.locationLatitude`(Decimal → string) | ❌ | ✅ |
| `createdAt` | `Date` | 必返 | `Activity.createdAt` | ✅ | |
| `updatedAt` | `Date` | 必返 | `Activity.updatedAt` | ❌ | ✅ |
...
| `cancelledBy` | `Activity.cancelledBy` | App 不见 cancelled |
| `cancelledAt` | `Activity.cancelledAt` | 同上 |
| `cancelReason` | `Activity.cancelReason` | 同上 |

> **注**:由于 D-P2-4-1 推荐方案 A 严格只允许 `published`,`cancelled*` 三字段在 App 不会出现非 null 值,但**依然显式排除**,避免未来若解锁 cancelled 可见时遗漏字段裁剪。

### 5.3 富文本 / XSS 安全

- `description` / `content` / `registrationNotes` / `registrationSchema` / `galleryImageUrls` / `coverImageUrl` 全部**原样返回**(沿 v2 `ActivityResponseDto` 范式;后端不解析 / 不渲染)
- **不**在 P2-4 引入 HTML sanitize / DOMPurify / 任何 XSS 防御层:沿 v2 已有范式 + App 端是原生 / 小程序客户端,渲染富文本时由前端各自处理 markdown / HTML / 富文本协议
- **不**对 URL 做 SSRF 校验:`coverImageUrl` / `galleryImageUrls` 是创建活动时管理员录入的可信内容(沿 v2 `CreateActivityDto.coverImageUrl` 仅 `@IsString() + @MaxLength(512)` 校验);若担心,在 P2-5+ 评估后端 URL 白名单(图床域名)
- `registrationSchema` / `content` 是 Json,前端按约定 schema 解析(沿 v0.2 D12 + Q-A13)
...
- 注意:`memberId` 在 v0 实际**未参与 where 过滤**(沿 §6.5;published 活动池对全员相同);保留 `memberId` 入参为后续 P2-5+ 若引入"已报名活动从列表排除"留扩展槽 + 调用链显式传递语义意图
- where 子句:`notDeletedWhere({ statusCode: 'published' })`(沿 D-P2-4-1 方案 A;若 D-P2-4-1 决议为方案 C,改 `{ statusCode: { in: ['published', 'completed'] } }`)
- select:**新建** `appActivityListItemSelect`(字段集 = §4.1 锁定 15 项 - `id` 等显式列;**不**复用 `activityListItemSelect`)
...
private toListItemDto(row: AppActivityListRow): AppAvailableActivityListItemDto {
  return {
    id: row.id,
    title: row.title,
    // ...逐字段映射,Decimal 经 decimalToString,Json 经 jsonAsObject / jsonAsStringArray
  };
}
...
- Decimal / Json helper:**复制**而非 import(`ActivitiesService` 内是 private;沿 P2-3 `buildAuditMeta` 复制范式;第二次复用再抽 `common/prisma/`)
...
- **新建** `test/e2e/app-activities.e2e-spec.ts`(沿 P2-2 `test/e2e/app-me-profile.e2e-spec.ts` 命名范式;P2-2 文件不存在则沿 `app-me.e2e-spec.ts` 范式)
- **不**新建独立 `app-activities-available.e2e-spec.ts` + `app-activities-detail.e2e-spec.ts`(2 endpoint 强语义耦合;沿 P2-2 单文件覆盖 GET + PATCH 范式)

### 10.2 用例分类(估算 28 ~ 32 cases)

#### `/api/app/v1/activities/available`(列表)

| 类别 | it block | 估算数 |
|---|---|---|
| 正常流 | `200 + 仅 published + 字段集恰好 15` / `200 + items 默认 createdAt desc` / `200 + 分页 page=2 / pageSize=5` | 3 |
| 字段隔离 | `不返 deletedAt` / `不返 publishedBy` / `不返 cancelled* / cancelReason` / `不返 publishedAt` / `不返 locationLongitude / Latitude`(列表收窄) | 1-2(参数化) |
| 可见性反向 | `draft 活动不在列表` / `cancelled 活动不在列表` / `completed 活动不在列表` / `软删活动不在列表` | 1(参数化 4 态) |
| 鉴权 | `401 不带 token` / `401 无效 token` / `403 canUseApp=false(memberId=null)` / `403 member inactive` / `403 member 软删` / `403 admin without member` | 1-2(参数化 ≥ 3 case) |
| admin-as-member | `200 + admin 兼队员可见集合 = 普通 USER 兼队员可见集合(零差异)` | 1 |
| Query 反向 | `传 statusCode=draft → 400(forbidNonWhitelisted)` / `传 activityTypeCode → 400` / `传 keyword → 400` / `pageSize=101 → 400` | 1(参数化) |
| 旧路径稳定 | `/api/v2/activities` Admin GET list 行为逐字不变(snapshot)` / `/api/v2/activities` USER GET list 行为逐字不变` | 2 |

#### `/api/app/v1/activities/:id`(详情)

| 类别 | it block | 估算数 |
|---|---|---|
| 正常流 | `200 + published 详情字段集恰好 22` | 1 |
| 字段隔离 | `不返 publishedBy / publishedAt / cancelledBy / cancelledAt / cancelReason / deletedAt` | 1(参数化) |
| 可见性反向 | `draft → 404 + ACTIVITY_NOT_FOUND` / `cancelled → 404` / `completed → 404` / `软删 → 404` / `不存在 id → 404` | 1(参数化 5 态) |
| 鉴权 | `401 不带 token` / `401 无效 token` / `403 canUseApp=false` / `403 member inactive` / `403 admin without member` | 1(参数化 ≥ 3 case) |
| Param 反向 | `id 长度 < 8 → 400` / `id 长度 > 64 → 400` / `id 非字符串(数字) → 400` | 1(参数化) |
| 旧路径稳定 | `/api/v2/activities/:id` Admin GET draft 仍 200` / `/api/v2/activities/:id` USER GET draft 仍 404`(沿 v2 Q-A7) | 2 |
...
- 复用既有 `createTestActivity`(若不存在,P2-4 内联 helper 创 1 draft + 1 published + 1 cancelled + 1 completed + 1 软删 5 态;**禁止**新建跨 module fixture util)
- canUseApp=false 三态 fixture(沿 P2-2 `app-me-profile.e2e-spec.ts` 范式)

### 10.5 contract snapshot

- **新增 2 path**:`/api/app/v1/activities/available` GET + `/api/app/v1/activities/{id}` GET
- **新增 2 ~ 3 DTO schema**:`AppAvailableActivityListItemDto` + `AppActivityDetailDto`(+ 若 PageResultDto 在 OpenAPI 中按泛型展开,可能产生 `PageResultDto_AppAvailableActivityListItemDto` 派生 schema)
- 修改 [`test/contract/openapi.contract-spec.ts`](../test/contract/openapi.contract-spec.ts) `EXPECTED_ROUTES`:在 Phase 2 P2-3 段(line ~323)之后追加 2 行
- 修改同文件 components.schemas 期望段:追加 `AppAvailableActivityListItemDto` / `AppActivityDetailDto`
...
| 文件 | 估算行数 |
|---|---|
| `src/modules/activities/dto/app/app-available-activity-list-item.dto.ts` | 70-90 |
| `src/modules/activities/dto/app/app-activity-detail.dto.ts` | 110-140 |
| `src/modules/activities/controllers/app-activities.controller.ts` | 60-80 |
| `src/modules/activities/app-activities.service.ts` | 130-170 |
| `src/modules/activities/activities.module.ts` 改动 | +3-5 |
| `test/e2e/app-activities.e2e-spec.ts` | 280-340 |
| `test/contract/openapi.contract-spec.ts` 改动 | +6-10 |
| contract snapshot 自动更新 | +60-120 |
| **总计** | **~720-955** |
...
| 选项 | 描述 | 评估 |
|---|---|---|
| A | 单 PR(P2-4 整体) | ⚠️ 边界(单 PR 估算 720-955 行,超过 phase-2-review.md §8 锁定的 < 500 行阈值) |
| B | 拆 P2-4a(`/available` 列表 + AppActivitiesService.listAvailableForMember + AppAvailableActivityListItemDto + e2e 子集) + P2-4b(`/:id` 详情 + findVisibleByIdForMember + AppActivityDetailDto + e2e 子集) | ✅ **推荐**:两 endpoint 行为独立 + 各自 ~360-475 行,均在 < 500 阈值内;P2-4a 合入后 P2-4b 立项前 phase-2-review.md §11.3 增 1 行引用即可;且 P2-4a 自身可独立交付前端联调 |
| C | 拆 P2-4a(DTO + Service)+ P2-4b(Controller + e2e) | ❌ 不推荐:DTO + Service 没 controller 无法运行 e2e,P2-4a 单独无法验收 |

### 12.3 决议 D-P2-4-5(待用户拍板)

**推荐方案 B**:**拆 P2-4a + P2-4b 两个独立 PR**,各自走 C 档(沿 phase-2-review.md §8 决策 5)。

**理由**:
- 沿 phase-2-review.md §8 "每个 PR diff < 500 行(超 500 行必须拆)"硬铁律
- P2-4a(列表) + P2-4b(详情)是天然解耦的两 endpoint;不像 P2-2 GET + PATCH 是强语义对 + 共享 DTO,P2-4 list 与 detail DTO 是两个独立 class(虽字段集 List ⊂ Detail,但 §4.3 + §5.4 锁定不可继承)
- P2-4a 合入后 App 端已可独立联调"看可参加活动",前端不必等 P2-4b
- P2-4b 启动前可借 P2-4a 实际行数(可能因 e2e helper 复用)再校准估算;若 P2-4a 实际 < 350 行而 P2-4b 估算亦 < 350 行,**且**用户拍板可接受,允许 P2-4 合并回单 PR(由 P2-4b 启动前评审稿决议)

**备选方案 A**(若用户允许单 PR > 500 行):需在 PR description 显式声明"沿 phase-2-review.md §8 例外:P2-4 单 PR 行数 X,本 PR 评审稿明确拒绝拆 P2-4a/4b 因 ...";本评审稿**不**主动建议此路径。
...
| 编号 | 决议项 | v0 推荐 | 状态 |
|---|---|---|---|
| **D-P2-4-1** | App 可见状态白名单 | **方案 A**:仅 `published`(`/available` + `/:id` 一致;比 v2 USER 更严,去掉 completed) | ⏳ 待拍板;备选方案 C(对齐 v2 USER 含 completed) |
| **D-P2-4-2** | P2-4 是否返回报名状态 / hint / capabilities / registeredCount | **方案 A**:全部不返(纯浏览;报名相关属 P2-5) | ⏳ 待拍板 |
...
| **D-P2-4-5** | PR 拆分 | **拆 P2-4a(列表) + P2-4b(详情)** 两 PR;各自走 C 档 < 500 行 | ⏳ 待拍板;备选方案 A(单 PR > 500 行,需用户显式允许) |
...
- ❌ 生成 migration(沿 §0)
- ❌ 启动 P2-4 implementation(本评审稿是评审,实施需单独立项)
...
- ✅ D-P2-4-1 决议方案 C(对齐 v2 USER 含 completed)— 若运营 / 产品有"App 端历史可参加活动回顾"需求
- ✅ D-P2-4-2 改为返 minimal `canRegister` / `myRegistrationStatus` — 若 P2-5 立项前前端联调实测前端按钮状态高度依赖,可在 P2-4b 评审稿前重开
