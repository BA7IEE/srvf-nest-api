...
负责人页面读取 managed activity 的 `ending-workbench` 获取安全汇总；字段真相见 OpenAPI。指标未配置、不需要成果、尚无正式成果分别显示，不能互相当作零值。正式成果来自现行 confirmed 选择器，更正待处理和取消都不应隐藏旧正式成果。
提示 target 只是功能入口，不代表有写权限；录入、计算、确认、更正、结算与归档继续调用各自既有接口并独立判权。当前接口不返回 canSettle/canArchive，不返回成员名单、指标实际值或附件链接，不引入结束硬门。无权读成果的结算协作者仍沿原结算入口，不因本工作台缩减原权限。C4 前端页面尚未发布，Gate 未启用。
...
| 账号头像 | `GET /api/app/v1/me/avatar` · `POST me/avatar`(**multipart**,字段名 `file`)· `DELETE me/avatar` |
...
- **单人代理与批量任务**：`POST .../proxy-punch` 要求 `participationIdentityId`、`actionCode`、`eventKey`、`reason`；`POST .../bulk-punch-jobs` 要求 `operationKey`、`actionCode`、`reason`、可选完整坐标三元组，外加 `participationIdentityIds`(去重，≤500 条)与 `selection` **恰好二选一**(两个都给或都不给一律 400)。`selection` 形如 `{ mode?: 'session-all', statusCodes?, positionId? }`(`mode` 可省略，当前闭集只有这一个值，省略即取它；乱填仍 400)，由服务端把路径上那一场次展开成任务项 —— 500/2000/10000 人**不需要前端自行分批**，上限只约束 id 列表那一路。bulk 的同 key 同请求重放原任务，异请求稳定冲突；用 `GET .../onsite/bulk-punch-jobs/:jobId` 读取安全进度，不能把已创建任务视为所有 item 都已成功。
