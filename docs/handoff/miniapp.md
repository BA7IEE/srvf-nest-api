...
| **证书图上传 + 审核进度(F7 + 刀G)** | `POST /api/open/v1/recruitment/applications/certificates`(multipart;双通道二选一:`wechatCode` 或 `phone+code`;`category ∈ {first_aid, bsafe}` + `images` 1~3 张)→ `{category,imageCount}`。重传整类覆盖并把该类旧审核态复位为 `uploaded`;终态行 → `28041`。报名进度 DTO additive `certificates:[{category,status,imageCount,note?}]`,其中 `status ∈ none|uploaded|approved|rejected`;驳回 `note` 对申请人可见且图片已清空,前端应显示原因并提供重传入口。审核通过会自动完成对应门槛(first_aid→redCross / bsafe→bsafe),驳回会同步取消门槛标记 |
