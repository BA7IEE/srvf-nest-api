...
| **我负责的活动（Unreleased PR-7）** | 新 base `GET/POST /api/app/v1/my/managed-activities`，**不得复用**上行 `/my/activities`（后者仍只表示本人报名历史）。`GET organization-options` 返回 active membership / cross-org grant 可发起组织及 `pathLabel`；detail 返回 activity、initiator/owner/myResponsibility、latest publishReview、counts、closure。draft 可 PATCH/DELETE，并在独立 `/positions` 子资源 CRUD；初发用 `submit-publish-review` 或持 grant 的 `direct-publish`，pending 可 `withdraw-publish-review`。published 直改仍返 `20037`，owner 改走 `submit-change-review`：body `{activity,positions?}`，positions 若传必须是完整 proposal（现有项带 `activityPositionId`，新增项带 `clientRef`，遗漏现有项表示软删），审核通过后才更新 live 数据。职责面为 `responsibilities`、`collaborator-options`、`collaborators` add/end、`transfer-owner`；App DTO 独立且不返回 L3 字段。报名管理已新增 `/api/app/v1/my/managed-activities/:activityId/registrations`：GET list，PATCH `:registrationId/{approve,reject,cancel}`，POST `:registrationId/reopen`，PATCH `bulk-{approve,reject}`；owner 与 active 报名协办可用，考勤协办/无关用户统一 30100，协办被结束后下一请求立即失权。列表/动作出参不含 Admin 审核人 id。考勤管理继续等 PR-8，`declare-attendance-complete` 等 PR-9，当前客户端不要提前调用。 |
