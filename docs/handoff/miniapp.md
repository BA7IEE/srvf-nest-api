...
| **H5 报名提交(无账号;S4a)** | `POST /api/open/v1/recruitment/applications`(multipart;`payload` JSON 内 **`phoneVerificationToken`**〔H5〕或 `wechatCode`〔小程序〕**至少二选一**;`payload.phone` 须与验证手机一致;小程序链向后兼容不变)。**⚠️ F5 契约收紧(v0.41.0-pre)**:`payload` **必含 `privacyConsentAccepted: true`**(缺省/false → 40000,旧版本前端会全量 400——发版前必须同步升级);可选 `privacyConsentVersion`(同意文本版本号,建议随文案版本迭代);multipart 新增**可选**文件位 `signatureImage`(jpeg/png ≤5MB,申请人手写签名图;发号后随队员档案长期留存) |
