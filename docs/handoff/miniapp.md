...
| 我的报名(报名/查/取消) | `GET /api/app/v1/my/registrations` · `POST` 报名 body additive 接受 `activityPositionId`（活动有岗位时必填，缺失 21035；不存在/跨活动/已删 20002；同人报第二岗仍 21002；满员成功创建 `waitlisted`，排位按岗位队列）· `PATCH` 取消(候补可随时退出)。最终写入前后端会在事务内重验本人 Member 存在、未软删且 ACTIVE（竞态失效时 15001/17030）；活动已结束 `20125`、报名截止 `20123`、已有考勤不可取消 `21033` 等既有闸不变 |
