...
| **入队申请候选部门(刀H)** | `POST /api/app/v1/me/team-join/applications` 发起 / `GET .../applications/current` 查进度 / `PATCH .../applications/:id/targets` 改候选。轮次现可配置开放部门与候选上限:候选必须属于 `openOrganizationIds`(空=全部 ACTIVE)且去重后数量 ≤ `maxTargetOrgs`(默认 8),违例统一 `28242`;submit/current/update 成功响应 additive 回显**有效值** `openOrganizationIds:string[]`(空=全部)与 `maxTargetOrgs:number`。进度中的贡献值由后端按北京日封顶 3 实时计算,v0.48.0 起历史记录也按新上限重算,数字可能变大;前端直接展示、不要自行 SUM。候选选择器须按两字段禁用清单外部门并限制多选数;本刀不新增 pre-submit 轮次查询端点,首提交流程的部门数据来源沿现有产品配置/前端交付同步 |
