...
> D7-1 内部性能修复候选：追加函数迁移和前驱分录查询等价优化，接口、错误码、权限与客户端合同不变，前端无需调整。当前仅本地验证，未合并或部署；不得据此开启 Gate。上线仍须独立审查并执行正式部署流程，详见[精确修复计划](../plans/activity-os-r4-d7-1-sql-pairing-fix-plan.md)。
