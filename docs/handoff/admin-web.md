...
| 入队管理 / 入队申请 | `team-join/{cycles,applications}` | `team-join-cycle.read.record` · `team-join-application.read.record` | 开轮/编辑轮表单 additive `openOrganizationIds?:string[]\|null`(null/空=全部 ACTIVE)+`maxTargetOrgs?:number\|null`(null=默认 8,域 1..8),配置清单中的 org 必须存在且 ACTIVE;轮次 list/detail 回显原始配置。申请人发起/改候选由后端强制「候选 ⊆ 开放清单且去重后数量 ≤ 上限」,违例 `28242`;一键入队弹窗仍只能从申请人的候选中选最终部门 + 默认级别 L1 |
