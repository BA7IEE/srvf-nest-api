...
### 2.5 通知管理(站内信撰写 / 发布 + 微信订阅渠道)— ✅ S1+S2 后端就绪(本 PR,Unreleased)

统一通知模块 S1 站内信渠道 + S2 微信订阅 quota 渠道(GAP-005 前两切片;冻结评审稿 [`unified-notification-dispatcher-review.md`](../archive/reviews/unified-notification-dispatcher-review.md))。admin 撰写/发布,会员 app 拉取站内信 feed(未读红点);S2 起 admin 可勾微信渠道,发布时向已订阅会员机会式推送。
