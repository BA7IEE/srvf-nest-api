...
### 3.4 角色权限编辑器请改用 `PUT` 整集替换(P1-32 PR 4a,2026-08-23;**加值不破坏**)

针对的就是 §2.6 ② 那个「角色权限编辑器」。**旧的 `POST` / `DELETE` 请求响应形状一字未变,继续能用**;
但「勾一批 → 保存」这个交互,现在有了对得上的端点。
...
- 判权要 `rbac.role-permission.create` **与** `rbac.role-permission.delete` **两个**码 ——
  只有其中一个的账号调 `PUT` 会拿 `30100`(但它调 `POST` 或 `DELETE` 仍然正常)。

**为什么别再用「先 POST 几条、再 DELETE 几条」拼保存**:那是 N 次请求、N 个中间态,
