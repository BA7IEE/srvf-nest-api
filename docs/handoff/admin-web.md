...
### 2.2 队员 360(沿队员轴下钻)— 🟡 5 子资源就绪,3 跨轴查询缺
| tab | 端点 | 状态 |
|---|---|---|
...
| 活动履历 / 考勤记录 / 贡献值 | `GET .../members/:id/{registrations,attendance-records,contribution-summary}` | ⛔ 见 [GAP-002](#4-缺口台账-gap-ledger) |

### 2.3 审批工作台(跨活动横扫"待我处理")— ⛔ 后端缺扁平查询
按 `status=pending` 跨所有活动横扫报名/考勤。现有列表都锁死在 `:activityId` 路径段,**做不了跨活动**。见 [GAP-001](#4-缺口台账-gap-ledger)。
...
| # | 诉求(前端想做的任务) | 期望端点 | 状态 |
|---|---|---|---|
| **GAP-001** | 审批工作台:跨所有活动按 status 横扫报名/考勤 | `GET /api/admin/v1/registrations?status=` · `GET /api/admin/v1/attendance-sheets?status=` | 🟡 **草拟·未跑**(2026-06-23;Tier2 goal 已起草,**未下发 / 未合并**) |
| **GAP-002** | 队员 360:某队员的报名履历 / 考勤记录 / 贡献值生涯累计 | `GET .../members/:id/{registrations,attendance-records,contribution-summary}`(贡献值=实时算复用 team-join `computeContribution`,封顶 1.5) | 🟡 **草拟·未跑**(2026-06-23;Tier3 goal 已起草,**未下发 / 未合并**) |

> 备注:**活动作战室(Tier1)不是缺口**——后端全就绪,纯前端重组 IA(见 §2.1)。
> 这两条 goal 合并发布后,把状态改 `已发`,并把 §2.2/§2.3 的 ⛔ 改 ✅。
