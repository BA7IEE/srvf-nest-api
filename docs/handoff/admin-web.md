...
> ① **取消/完结活动禁批报名** —— 活动 `statusCode ∈ {cancelled, completed}` 时 `PATCH .../:rid/approve` 返 **`20124`**(`ACTIVITY_ENDED_OR_CANCELLED_APPROVE_FORBIDDEN`,「活动已取消或已完结,报名不可审批通过」);**但 `reject` / `cancel` 不受此闸**——活动取消/完结后仍可驳回/代取消,用来清理残留待审队列。前端审批工作台在活动已取消/完结时应**禁用"通过"按钮、保留"驳回/取消"**。
...
> ④ **活动已结束不可报名 + App 池过滤(v0.40.0)** —— 活动 `endAt < now` 时,代报名 / 自助报名(App)均返 **`20125`**(`ACTIVITY_ENDED_REGISTRATION_FORBIDDEN`,「活动已结束,不可报名」);且 App 可报名列表 `GET /api/app/v1/activities/available` **不再返回**已结束活动(`endAt >= now` 过滤)。活动**详情** `GET /api/app/v1/activities/:id` 口径不变(published 即可见,已报名者仍能回看已结束活动)。`registrationDeadline`(报名截止 `20123`)与 `endAt`(活动结束 `20125`)是两条独立闸:deadline 早于 endAt 时先撞 `20123`。
...
| 入队管理 / 入队申请 | `team-join/{cycles,applications}` | `team-join-cycle.read.record` · `team-join-application.read.record` | 开轮/编辑轮表单 additive `openOrganizationIds?:string[]\|null`(最多 64 项;null/空=全部 ACTIVE)+`maxTargetOrgs?:number\|null`(null=默认 8,域 1..8),配置清单中的 org 必须存在且 ACTIVE;轮次 list/detail 回显原始配置。申请人发起/改候选由后端强制「候选 ⊆ 开放清单且去重后数量 ≤ 上限」,违例 `28242`;一键入队弹窗仍只能从申请人的候选中选最终部门 + 默认级别 L1 |

> **招新/入队契约收口(Unreleased;C/D/E)**:
>
...
**✅ 2026-07-05 true-up**:首页数字卡片 `el-statistic` **现已可做**——[GAP-003](#4-缺口台账-gap-ledger) 已全量交付,`GET admin/v1/meta/dashboard-summary`(见 §2.7)一次性返回「待审报名数」`registrations.pending` + 「考勤待办」`attendanceSheets.{pending,pendingFinalReview}`(一级待审 + 待终审两数)+ 「进行中活动数」`activities.published` 三块。**块级权限裁剪**:前端渲染前按response 实际出现的 key 决定显示哪些卡片(无权限的块不出现,不是 0——区分"没权限看"和"数字是 0"两种语义,别把 `undefined` 渲染成 0);零 query 参数、零缓存(每次调用即时算,轻量聚合,可放心挂在首页/工作台加载时机)。**team-join / 证书待核 / 通知**等块刻意不在本端点内(诉求触发再 additive 加块,前端可先用各自列表分页 `total` 拼)。**招新域例外**:招新工作台已有专属 stats 端点(`recruitment/cycles/:id/stats`,五组聚合,GAP-006 S2),招新进度看板可直接做,不走本端点。
