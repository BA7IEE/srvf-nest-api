...
> 字段级真相 = live `/api/docs-json`;权限码 = [`RBAC_MAP.md`](../ai-harness/RBAC_MAP.md)(189)。
...
> ⚠️ **考勤终审判权收紧(终态 scoped-authz PR9,2026-07-02 起生效,考勤终审是全仓首个真正切到 scoped 判权的业务面)**:`final-approve` 新增两个专用错误码——**`22074` 自审拒绝**(提交人 == 终审人;**SUPER_ADMIN 也拒**,没有任何配置能放开)/ **`22075` 同人拒绝**(一级审核人 == 终审人;**默认禁止**,运维可设 env `ATTENDANCE_ALLOW_SAME_REVIEWER=true` 放开,严格字符串匹配)。**`final-reject` 不受这两条约束**——同一张单据,驳回操作没有自审/同人限制(不对称设计,e2e 已锁死这个差异,不是漏做)。前端终审按钮**必须单独处理 22074/22075**,给出对应文案(如「不能终审自己提交的考勤单据」/「一级审核人不得再终审同一张考勤单据」),不能笼统按旧的"权限不足"(`30100`)文案兜底——这两个码本质是"数据完整性约束"而不是"没权限",文案不该说"你没有权限"。**运维/部署注意**:单管理员部署下,终审链现在需要**至少两个不同的人**(提交人 / 一级审核人 / 终审人三者两两不同,才能走完一整条链);如果确实只有一人可用,可设置上述 env 放开"同人"限制,但"自审"限制无论如何都放不开。**权限来源新增一条 scoped 通路**:除 biz-admin 全局终审(不变,契约照旧)外,现在也可以给某人的组织任职挂一条 `attendance-final-reviewer` 角色的 role-binding,使其**不需要 biz-admin 也能终审**(换届撤任职即自动失权,零代码改动;详见 §2.6)。想确认"某人到底能不能终审某张单" → 用 §2.6 的「权限解释」端点(PR10)直接查,不用猜或翻权限表。
...
### 2.6 组织 · 职务 · 任职 · 分管 · 角色绑定 · 权限诊断(终态 scoped-authz PR1–PR10 已发 main)— ⚠️ 看清落地进度再造 UI

「组织职务 + 分管 + scoped RBAC + 统一鉴权」终态按 §11 序列逐刀落地(冻结稿 [`org-position-scoped-authz-terminal-design-review.md`](../reviews/org-position-scoped-authz-terminal-design-review.md))。**已发 10 刀(PR1–PR10),新增以下配置面**(队员**组织归属** memberships 属队员轴见 §2.2;**任职** position-assignments 亦双轴,队员轴一侧同见 §2.2):
...
> ⚠️ **scoped-authz 落地进度(2026-07-02 PR10 收口,序列 PR1–PR10 已全发 main;前端别超前造"判权已全面生效"的 UI,也别误以为"还没生效")**:
> - **判权现状一句话**:统一判权大脑 `AuthzService` **已上线**(PR8);**首个、也是当前唯一 scoped-live 的业务面 = 考勤终审**(`final-approve`/`final-reject`,PR9);**其余所有业务面仍只认 GLOBAL 角色绑定**(逐面迁移是 PR12,**尚未开始**)。
> - **🔴 关键语义(必读,替换旧版"PR8 前均不判权"的表述——PR8 已发,判权已发生,但只对一个面,且不是每个管理面单独都算数)**:**只有"任职(position-assignments,PR4)+ 一条显式指向该任职的角色绑定(role-bindings 绑 `attendance-final-reviewer`,PR6)"两步都做,才会对考勤终审(final-approve/final-reject)真实生效**(见 §2.1);只做任职不建绑定 = 不生效(§2.2 已有提示:任职记录的存在本身不代表拿到权限)。**职务定义 / 职务规则(PR3)** 是更上游的全局配置,只决定"哪个组织类别可以设哪个职务",本身不影响任何人的权限。**分管(supervision-assignments,PR5)** 推导出的 `org-supervisor` 角色 / **职务自动推导出的 `org-admin`/`group-manager`** 角色(PR7 职务→角色 policy)都**只含只读或一级审批码,均不含终审两码**——分管人 / 队长部长即使有任职或分管记录,**不会**因此自动获得终审权,必须额外单独给他们(或给他们的任职)显式绑 `attendance-final-reviewer`。**除考勤终审外的其它任何业务面**(报名审批、活动管理、证书核验、队员管理……)**这些管理面录入的数据目前一律只是"铺数据"**,因为那些业务代码还没接入 `AuthzService`(接入 = PR12,逐面单独出 goal)。前端在这些管理面的文案/交互上,**不要**笼统说"暂不生效"(对终审场景不准确),也**不要**说"设置即生效"(对绝大多数场景不准确)——role-bindings 页具体文案见上一段。
...
> - **未落地(先别做)**:**PR11 公告导入**(2026 任命 staging 双锚落库工具)/ **PR12 逐面迁移**(其余业务面 rbac→authz、届时摘 biz-admin 终审两码)——详见 §4 [GAP-007](#4-缺口台账-gap-ledger)。
...
8. **考勤终审(`final-approve`)自 2026-07-02 起真收紧**:新增 `22074`(自审拒,提交人==终审人,SUPER_ADMIN 也拒)/ `22075`(同人拒,一级审核人==终审人,默认禁止,env `ATTENDANCE_ALLOW_SAME_REVIEWER=true` 可放开)两个专用错误码;`final-reject` **不受影响**(不对称,别当成漏做)。终审按钮要单独处理这两码给专门文案,别混进通用 `30100` 权限不足文案。排查"某人为什么终审不了" → 用 `authz/explain` 端点直接查。详见 §2.1 / §2.6。
...
| 角色绑定(scoped) | `admin/v1/role-bindings` | `role-binding.read.record`(增/改/删另持 `create`/`update`/`delete`.record)| `PureTable` CRUD;建绑定选 **principalType**(USER/MEMBER/POSITION_ASSIGNMENT/SYSTEM)+ principal + 角色 + **scopeType**(GLOBAL/ORGANIZATION/ORGANIZATION_TREE/ACTIVITY/RESOURCE/SELF,决定对应 `scope*` 字段是否必填);**scopeType≠GLOBAL 时须在表单/列表显著提示「当前仅对考勤终审生效;其余业务面待 PR12 逐面接入」**(PR8/PR9 已发,取代旧的"暂不生效"绝对表述;§2.6 落地进度声明);删除=软删(`status=ENDED`+`deletedAt`,与任职/分管的"REVOKED 不物删"不同);GLOBAL 绑定与「角色与权限」页共享同一张底表,互相可见 |
