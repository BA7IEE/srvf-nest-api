...
> **C1 D2c（当前分支已实现，待 PR/CI/整体复审）**：新发布审核冻结为 V7，既有 Admin 工作台路径不变。详情的 `changeDiff.kind='proposal-v7'` 与 `v7Fields.changedFields` 只可展示受控字段名，绝不展示指标集名称、定义、before/after 值或快照全文；损坏 V7 只显示受控不可解析结果。审批时若显式新选引用已退役，提示 `20172` 后由发起人重新选择；没有新权限、目录初始化、成果值、生产部署或 Gate。
...
> **D83 资格 runtime（第 4 批⑪）**：报名审核 approve 在锁定活动、报名头/current revision 与全部 identity 后按当前 grade/profile/组织/证书/保险事实重新评估活动、场次和岗位资格；block 返 `409/21040`，配置漂移返 `409/21041`，两者都不得推进状态、容量、audit 或通知。旧 legacy pending 若没有 identity/preference、但后来出现 active 场次/岗位 RuleSet，Admin 与 managed approve 均返 `409/21038`，前端应引导使用 canonical 主链而不是猜测目标。managed 现场补录遇合法 active RuleSet 走同一 evaluator，block 同样零写，warn 可继续；只有 Form 仍未接通时维持 `21039`。前端不得尝试用报名历史 snapshot 推断审批资格，也不要把 `21040` 当作可通过前端改 payload 重试的参数错误。
...
跨所有活动按 `statusCode` 横扫报名/考勤,**脱离 `:activityId` 路径段**:`GET /api/admin/v1/registrations?statusCode=` · `GET /api/admin/v1/attendance-sheets?statusCode=`(均分页 + item 自带 activity 上下文 `activityId`/`activityTitle`)。见 [GAP-001](#4-缺口台账gap-ledger)。
...
|---|---|---|
| `GET admin/v1/registrations` | `q`(模糊命中 memberNo+memberRealName+activityTitle)/ `memberQ`(仅队员字段)/ `activityQ`(仅活动标题)/ `memberId` / `activityId`(精确过滤)/ `organizationId`(经活动 organizationId)/ `includeDescendants` | `member`(附 `{id,memberNo,realName,nickname,label,gradeCode}`)/ `activity`(附 `{id,title,startAt,organizationId}`) |
...
| 用户列表/详情反查"这个账号属于哪个队员"| `memberId`/`member:{memberNo,realName,nickname,label}\|null` 两字段 additive 出现在 `GET /api/admin/v1/users`(list)与 `GET /api/admin/v1/users/{id}`(detail)响应里 | `[rbac: user.read.account]` |
...
| **角色绑定 列表 / 建 / 改 / 软删**(带 scope 的角色绑定;PR6;2026-07-13 委派闸 + 任期/末位保护收口;v0.61.0 组织 scope 活跃闸)| `GET/POST /api/admin/v1/role-bindings` · `PATCH/DELETE .../:id` | `[rbac: role-binding.{read,create,update,delete}.record]`;非 SUPER_ADMIN 对特权角色的 create / reactivation / 任期扩张 → 30102;特权角色 = ops-admin 或含 `role-binding.*` / `rbac.*` / 6 条 SA-only 保留码;PATCH/DELETE 或用户管理的禁用/软删若使最后一个 active GLOBAL ops-admin 持有人离场 → 30101;ORGANIZATION/TREE create、preview、batch 及重新生效时要求 scope 组织 ACTIVE(停用 → 17031,软删/不存在 → 11001);PATCH 成功写 `role-binding.update` before/after audit |
...
> **v0.61.0 组织 scope 活跃闸**:`ORGANIZATION` / `ORGANIZATION_TREE` 新建、预检、批量建及 PATCH 重新置 ACTIVE / 延长已到期任期时，scope 组织必须存在、未软删且 `status=ACTIVE`；停用组织返 `ORGANIZATION_INACTIVE(17031)`，软删或不存在返 `ORGANIZATION_NOT_FOUND(11001)`。组织后来停用不会自动删除历史绑定，纯 `note` 修改仍允许；所有 RoleBinding 读取响应新增必有布尔 `scopeInactive`，前端为 `true` 时应显示“范围已失效”并禁用恢复生效操作。
...
7. **signed URL / 敏感字段**有可见级与时效;附件走 `upload-url` / `confirm-upload` 通用链路,别假设直链。SVG/HTML/XHTML 恒拒;confirm 与保留兼容的 legacy `POST /api/admin/v1/attachments` create 都会读取对象并核对实际大小、jpg/png/webp/gif/pdf 前缀魔数,声明 MIME 与内容不符返 13016,前端须保留失败提示与重选文件入口。`expireAt <= now` 的附件不再返回 `accessUrl`(公开内容附件会直接移除过期行、过期封面 URL 为 null);未来时间/null 仍按原行为签发。**内容附件删除 PR-G**:仅 draft 可删，且操作者须同时持 `content.update.record` 与对应 `attachment.delete.content-*`；当前封面或正文 `attachment:<id>` 仍引用时返 `29031`，published/archived 返 `29030`，存储账本处理中返 `13034`。前端必须先显式清封面/移除正文引用再重试，后端不会静默改正文或自动清封面；通用附件删除路由也不能绕过这些限制。
...
### 3.4 角色权限编辑器**必须**用 `PUT` 整集替换(P1-32 PR 4a,2026-08-23)

> 🔴 **2026-08-24 更新(P1-32 PR 8)—— 这一节从「建议」变成「唯一做法」,且是对外契约破坏。**
>
> 下面两条旧增量端点**已删除,调用会 404**:
>
> | 已删 | 替代做法 |
> |---|---|
> | `POST /api/system/v1/roles/{id}/permissions`(加几条) | `GET` 取现状 → 目标集 = 现状 **∪** 要加的码 → `PUT` |
> | `DELETE /api/system/v1/roles/{id}/permissions/{permissionId}`(撤一条) | `GET` 取现状 → 目标集 = 现状 **\\** 要撤的码 → `PUT` |
>
> **四处必须一起改**:
> 1. **成功码 201 → 200**(`PUT` 的契约)。
> 2. **撤权按 `code` 不再按 `permission.id`** —— `GET /roles/{id}/permissions` 直接给 `permissionCodes[]`,
>    前端**不必再维护 id ↔ code 映射**。
> 3. **判权要两条码**:`rbac.role-permission.create` **与** `rbac.role-permission.delete`。
>    只持一条的账号从此**什么都改不了**(返 `30100`)—— 旧接口时代「只有 create 也能加码」的路没了。
> 4. **错误码 `30011` 不会再出现**:旧 `DELETE` 对「撤一条本来就不存在的映射」返 `30011`;
>    新语义下这是 **no-op 200**。靠 `30011` 判断「这条本来就没有」的分支要改成先看 `GET` 的
>    `permissionCodes[]` 里有没有它。
>
> ⭐ **强烈建议保存前先打一次 `POST …/permissions/preview`**(同参、同判定、零写入):
> 它把「这次会加哪些、撤哪些、影响多少条授予」先给人看,也会提前告诉你要不要二次验证(`30112`)。

针对的就是 §2.6 ② 那个「角色权限编辑器」。「勾一批 → 保存」这个交互,现在有且只有一个对得上的端点。

**① 新端点:`PUT /api/system/v1/roles/{id}/permissions`**

```jsonc
// 请求
{ "permissionCodes": ["a.b.c", "d.e.f"], "expectedRevision": 3 }
// 出参与 POST / DELETE 完全一致:RbacRoleDetailResponseDto
```

- 提交后该角色的权限**恰好**是 `permissionCodes`;传 `[]` = 清空。
- `expectedRevision` **必填**,没有默认值。
- 判权要 `rbac.role-permission.create` **与** `rbac.role-permission.delete` **两个**码 ——
  只有其中一个的账号调 `PUT` 会拿 `30100`。
  ⚠️ PR 8 之前它还能退回 `POST` / `DELETE` 各用一半的码,**现在没有这条退路了**。

**为什么当年就不该用「先 POST 几条、再 DELETE 几条」拼保存**:那是 N 次请求、N 个中间态,
中途失败会把角色停在一个谁也没想要的权限集上。`PUT` 是一次事务:要么整批生效,要么一个字节都不写。
(PR 8 已把那两条端点删掉,这条理由从「建议」变成了「事实」。)

**② 新增响应字段 `permissionRevision`(整数,从 0 起)**

出现在角色的**列表 / 详情 / 建 / 改 / 删 / 授权 / 撤权**七处响应里(纯新增,老代码不受影响)。
用法就是乐观并发的标准三步:**取回它 → 用户编辑 → 带着它提交**。

⚠️ 它**不是** `updatedAt`:改角色显示名会动 `updatedAt` 而不动它;权限集空转两者都不动。
别用 `updatedAt` 当版本号。

**③ 新错误码 `30111`(HTTP 409)= 「这份权限集已经被别人改过了」**

⚠️ **前端必须专门处理这一个** —— 它不是校验失败,重试也没用。
正确处理是:**重新拉一次角色详情**(拿到新的 `permissionRevision` 与新的权限集),
把差异呈现给用户,让他基于最新状态重新确认再提交。**不要**自动拿新版本号原样重发 ——
那等于把对方的改动悄悄覆盖掉,正是这道闸要拦的事。

**④ 一个刻意的限制,别当 bug 报**

非 `SUPER_ADMIN` 对**已经含有控制面码**(`rbac.*` / `role-binding.*` / 7 条 SA-only 保留码)
的自定义角色**用不了 `PUT`**:保留那条码会触 `30103`,去掉它同样触 `30103`。
这类角色只可能由 SUPER_ADMIN 亲手造出来、极少。遇到时退回逐条 `POST` / `DELETE` 即可。

**⑤ 一个不影响 UI 的行为变化**:`POST` 重复授权(提交的码全都已经在了)现在是纯空转 ——
不写库、不推进版本号、也不再产生审计行。响应仍是 `201` + 角色详情,**形状一字未变**。
...
- `rolePermissionSet` 三项**必填**:
  ...
  - `payloadHash` —— 目标权限码集合的指纹,算法:
    **去重 → 升序 → `JSON.stringify(array)` → sha256 → base64url**。
    ...
    ```
- 🔴 **撤码 / 清空也算高风险变更**:判定看的是**差集**(added ∪ removed),不是只看新增。
  「把某个角色的 `rbac.*` 码撤掉」「清空一个含 CRITICAL 码的角色」**同样要二次验证** ——
  冻结稿 §12.1 第一条逐字是「增加或移除」。UI 上别只在「新增高风险码」时才准备验证框。
...
**③ 那条限制已经解除(2026-08-24,P1-32 PR 8)**

PR 5 交付时这里写着一条如实说的限制:**旧的 `POST` / `DELETE /roles/:id/permissions` 不受二次验证管辖**,
所以「高风险变更必须二次验证」这句话当时**只对 `PUT` 成立**。

✅ **那两条端点已于 2026-08-24 删除**(见本节开头的 §3.4 破坏性变更块)⇒
角色权限的写面只剩 `PUT`,而 `PUT` 与 `preview` 都在闸内。
**「高风险变更必须二次验证」现在是无条件为真的一句话** —— 不存在绕过它的写路径,
后端已把这条做成禁止型判据(新增写入口若绕过闸会当场红),不是靠约定。
...
> **分组可演进**:① "系统管理" 一拥挤就拆「基础数据」(字典 / 组织 / 职务定义 / 职务规则 / 贡献值规则 / 附件配置)+「系统与权限」(用户 / 角色 / 审计 / 短信日志 / 各设置),按使用频率 + 权限层级分;② 审批工作台**只做日常高频**(活动发布 / 报名 / 考勤);招新报名、入队申请的待处理队列是季节性的,**留在各自模块**别塞进工作台(要"全局待办数"等 [GAP-003](#4-缺口台账gap-ledger) 的 stats 端点统一出)。
...
| 角色与权限 | `system/v1/{roles,permissions,user-roles}` | `rbac.role.read` / `rbac.permission.read` | 角色授权 `el-tree`/`el-transfer`;仅 USER+GLOBAL 简单授权,契约不变;scoped 场景改用下方「角色绑定(scoped)」页。删除自定义角色时，若仍有未撤销的服务主体绑定，后端返回 `30113`/409；前端应先跳转或引导到角色绑定页显式撤销，不能假设后端级联撤权。**⚠️ 权限点列表页的「新建」与「删除」两个按钮恒不可用** —— 见表后「权限目录护栏」说明 |
...
| 角色绑定(scoped) | `admin/v1/role-bindings` | `role-binding.read.record`(增/改/删另持 `create`/`update`/`delete`.record)| `PureTable` CRUD;建绑定选 **principalType**(USER/MEMBER/POSITION_ASSIGNMENT/SYSTEM/SERVICE_PRINCIPAL)+ principal + 角色 + **scopeType**(GLOBAL/ORGANIZATION/ORGANIZATION_TREE/ACTIVITY/RESOURCE/SELF,决定对应 `scope*` 字段是否必填);**scopeType≠GLOBAL 时须在表单/列表显著提示「当前对考勤终审 + 活动/报名/考勤的单点动作生效(扁平跨轴列表与新建活动仍 GLOBAL-only);其余业务面待后续批」**(PR8/PR9/PR12 已发;§2.6 落地进度声明);删除=软删(`status=ENDED`+`deletedAt`,与任职/分管的"REVOKED 不物删"不同)。存在未软删服务主体 Binding（含 SUSPENDED、未来、过期）时，关联 Role 不可删，须先在这里显式撤销；GLOBAL 绑定与「角色与权限」页共享同一张底表,互相可见 |
...
| 短信日志 | `system/v1/sms-send-logs` | `sms-send-log.read.list` | 只读 `PureTable`(手机号**掩码**);独立页,别折进系统设置。🔴 **`status=SENT` = 已提交 Provider,不代表终端已送达** —— 页面须把这句话做成常驻说明(表头旁 tooltip 或列表上方 `el-alert`),别让运营把 `SENT` 读成「用户收到了」;真送达要凭 `providerMsgId` 去腾讯云控制台查回执 |
