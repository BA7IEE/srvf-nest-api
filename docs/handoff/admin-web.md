...
| **组织轴归属 / 队员下拉**(F4;组织管理页配套)| `GET /api/admin/v1/organizations/:orgId/memberships`(分页,`includeDescendants`)· `GET .../organizations/:orgId/members/options`(该组织±后代可选队员,F1 投影)| `[rbac: membership.list.record]` / `[rbac: member.read.record]` |
...
**`includeDescendants`**:配合 `organizationId` 使用,展开该组织 + 全部后代组织再过滤(经组织闭包表;**closure 展开只用于列表数据过滤,与 scoped-authz 判权无关**——即便某人被判权限制在某组织树内,`includeDescendants` 本身不做任何权限收紧或放宽,纯数据范围参数)。
