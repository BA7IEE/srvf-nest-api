...
> **仍待下一刀**：资格配置/发布激活、活动到点 expiry 与既有整单取消/legacy waitlist writer 的 lifecycle 收口；ADV-014 按激进安全版归第 6 批导入刀。
