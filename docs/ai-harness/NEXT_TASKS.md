...
- **C1 评审已起草，尚未实施**:[指标定义与指标集版本评审及授权清单](../archive/reviews/activity-os-r3-c1-metric-definition-set-review.md)推荐 C1 内部 D1 数据地基 → D2 目录维护与活动选用接入，方案与 implementation 均待独立确认；D2 必须另补外部契约评审，新增 v7 而不回改 v2–v6，未接入前不宣称 C1 完成、不自动进入 C2。此 D1/D2 不指 Release 4 时长阶段；本次仅文档，不授权 schema/migration/权限/Gate 或生产动作。
