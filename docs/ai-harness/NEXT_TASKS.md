...
>
> **第 4 批⑧（managed 现场临时参加 + 容量 caller）**：新增 `POST /api/app/v1/my/managed-activities/:activityId/onsite-participations`，只走 D-5 + `activity-registration.create.record` + 当前活动 active `canManageRegistrations=true` 三重门；固定锁序为 Activity → 报名头 → member/activity 全部 identity → session/position/requirements/保险 → 三层容量 → revision/pointer → population → audit。新头或复用 canonical active 头均追加 `sourceCode='onsite'` immutable revision；永久 identity 只复用，明确的非最终状态转 pass，最终状态或跨历史头一律 `21030`。同 Activity 锁内，新 key 在 `endAt <= now`、`currentPostedVersion != null` 或存在 active closure 时一律 `21030`；精确同 key+hash 的旧成功回执先返回，即使其后已 posted/closed 也零新增。Form 含字段、适用 active RuleSet、岗位显式 RuleSet 均 `21039`，零猜 evaluator；首头 evidence 恰一份、复用头重验不重复，容量不足 `21032`、漂移 `20147`，零 capacity pointer/outbox/通知。同 key+hash 20 并发只落一份事实、异 hash `21003`。**缺口 #26**：现场补录的 Form/资格覆盖、跨历史报名头处理及精确 wire 未定义，继续 fail-closed；AC-018、019、022、023 保持原状态。
>
