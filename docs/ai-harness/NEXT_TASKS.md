...
> **D2 精确实施计划 PR 已创建（2026-09-11）**：D2 方向评审 [#1317](https://github.com/BA7IEE/srvf-nest-api/pull/1317) 已合入 main `2af40878090e44b33c8dce7f68a1ff78c7c18fb9`，合并后 [main CI 34584472085](https://github.com/BA7IEE/srvf-nest-api/actions/runs/34584472085) completed/success。本轮仅起草 [D2 精确实施计划](../plans/activity-os-r4-d2-participation-segment-facade-implementation-plan.md)，并按维护者授权创建 [计划 PR #1318](https://github.com/BA7IEE/srvf-nest-api/pull/1318)（head `6d51cb2a7b5359a87cd4d7b9b989c5817f08456b`，CI 运行中）：未来最小实现是考勤域内部、显式 tx、只读的 `ParticipationSegmentFacade`，读取现有 current 服务段并做来源同链校验；没有第二张服务段表、双写、迁移、API、DTO、权限、审计或 Gate 变更。该 PR 尚未合并；不实施、不操作数据库、不启用 Gate，D2–D8、生产部署、Gate 和整体跨模型复审仍未完成。
