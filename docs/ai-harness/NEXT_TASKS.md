...
- **C1 方案 A 已拍板，D1 implementation 实施中**:[指标定义与指标集版本评审及授权清单](../archive/reviews/activity-os-r3-c1-metric-definition-set-review.md)已随 #1277 合入；维护者随后确认 D1 implementation 及指定隔离测试库验证/重建。当前分支新增指标定义、指标集版本、关联项三表及第 110 条 migration，提供五类强类型校验/hash、DB 冻结与并发约束；验证、3b 重签和 PR 合入分别验收，本行不宣称已合入或已部署。C1 内部 D2 目录维护与活动选用须另补外部契约评审，新增 v7 而不回改 v2–v6；未接入前不宣称 C1 完成、不自动进入 C2。此 D1/D2 不指 Release 4 时长阶段；不新增权限或 Gate，不授权生产动作。
