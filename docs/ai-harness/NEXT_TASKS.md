...
> **五条均不阻塞第 2 批。**
> **待折进下一版修订件的已知合同缺口(#6–#18；#14、#16–#18 已裁定，其余未裁定)**:#6 `workflowRevision` 来源未定义 ·
...
> **#18（同上）**：报名附件 MIME / 大小 / 单会话附件数未定义；固定 JPEG/PNG/WebP/PDF、单文件 10 MiB、每会话最多一个完成附件。第 79 migration 本次只落实 `registration-upload-session` 的单附件 partial unique；MIME / 10 MiB seed 与 runtime 归下一把 Form 行为刀。
