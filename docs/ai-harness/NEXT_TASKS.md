...
- 🔴 **摸底新发现(只登记不删,待维护者拍板;2026-08-27 git 考古已定位成因)**:
  3 个 union 成员零产出且注释未标注 ——
  `recruitment-application.certificate-upload` / `.certificate-review`
  (**生产者随 #830 / PR-4a-2「删旧 category 端点」消失,2026-07-30**;该刀对已发布
  错误码明确「保留不删」,union 词条同样保留但漏了标注)、
  `member.official-portrait.purge`(**预留事件**:#1106 / T1 刻意未接,service 注释写明
  「合规清理走 issue §5.2 的 purge 流程,不在本刀」)。
  登记表备注已改为带出处的「零产出:…」,D6 靠「零产出」标注兜住不红;
  维护者拍板后:真退役 ⇒ union 注释补〔已退役〕或删词条(需同步登记表);
  purge 预留 ⇒ 接通时本表计数自动跟着走;还该有产出 ⇒ 这是**漏写审计**的 bug,另立项。
