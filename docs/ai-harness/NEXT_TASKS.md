...
全仓**无任何送达回执 / 状态回调链路**;`SmsSendLogItemDto.status` 的描述只写「发送状态」,
**没说清它是提交态**。

**待做(按代价排序,不必一次做完)**:
1. **最小改动(A 档,先做)**:把 DTO 描述、后台 UI 文案、运维文档统一改成
...
**为什么登记而不是当场修**:第 1 项落在 `src/`,与在飞的 #1055 撞 ROUTE_AUTHZ digest(同 P2-8/P2-9);
第 2 项本就该单独评审。
...
### P2-15 `description` 漂移还有第二条路 —— 关掉 `PATCH` 只堵了一半 — 2026-08-23 由 PR 3b 实施方逼出
