...
#### 验收编号分拣(2026-08-24;§16.1 第 ⑨ 条)—— **分拣不是清零**

`cutover:check` 的 A 类唯一硬失败是 9a(还有 `it.todo`)。本刀把「32 条待办」这个
不可行动的数字,变成 **A 14 / B 4 / C 14 且逐条有依据**;⚠️ **9a 仍红,这是预期的** ——
32 → **30** 条 todo,只降了 2。**数量不是目标,别为了让它变绿而硬写用例。**
...
**A · 能写且不重(14)** —— 其中只有 2 条是「证据已在、只差接线」,本刀交付;其余 12 条要**新写常规规模用例**,
必须连库跑过才算交付,本刀不写。

| 编号 | 状态 | 一句话依据 |
|---|---|---|
| AC-012 | ✅ 本刀接通 | 同一夹具里两格都有:未受邀活动不进目录(有邀请的那条**在**列表里,判据非恒真)+ 直接拿 id 请求详情得 `ACTIVITY_NOT_FOUND` |
| AC-023 | ✅ 本刀接通 | 100 条真 HTTP 并发争 capacity=1:恰 1 成功 / 99 条容量码,桶停在 `occupied=1`,不超卖不负数另有 DB CHECK 双向反例 |
| AC-003 | 待写用例 | clone 已交付(`#955`);9 类历史里邀请 / 二维码 / 关闭 / 更正 / 通知**不在事实表 spy 集内** |
| AC-014 | 待写用例 | 现场事实闸已由第 5 批交付;缺「**有效**」限定的 HTTP 证据、「改走终止」与「并结算」的续链 |
| AC-019 | 待写用例 | accept 已复用 canonical 四闸;缺 accept **入口自己**的资格 / 保险 / 必要表单三格断言 |
| AC-022 | 待写用例 | HTTP / 状态写 / 分配 caller 三样都已落;缺 HTTP 级 100×3 与「一人多场只占 1 个活动位」回读 |
| AC-049 | 待写用例 | 缺 absent 零时长结果 → 零 day 行 / 零分录 / 不进有效服务明细的端到端断言 |
| AC-056 | 待写用例 | 跨活动日上限**算法已按基线截到 3**;缺真第二场活动的夹具、读面 `cappedOut` 非零断言、稳定顺序的 e2e |
| AC-057 | 待写用例 | 拆日算法有单测;全套 e2e 夹具**一律日内**,跨零点 + 逐日上限组合零覆盖 |
| AC-060 | 待写用例 | 结果码 flip 走真更正链、同夹具断言人员/时长/分数/评价资格/关闭版本五格 |
| ADV-012 | 待写用例 | 与 AC-060 **共用同一条**待写用例 |
| AC-063 | 待写用例 | 关账×终审、关账×更正;⚠️ 关账 spec 目前单实例,要先起第二个 app/pool |
| ADV-011 | 待写用例 | 同 target 两份更正申请的真并发;⚠️ 更正线全仓无 concurrency spec |
| ADV-019 | 待写用例 | 三轴已有读面证据;缺**停用**这一轴在第 3 批新目录路由上的断言,以及跨轴「组合」 |

**B · 能写但太重(4)—— 需专门的规模 / 耐久方案,不要顺手写进普通 e2e**
...
**C · 真做不了(14)** —— 卡的是**能力**不是测试;逐条卡点已写进
`src/modules/activities/activity-business-overhaul-acceptance.spec.ts` 的卡点表(缺哪一格写在那里)。

AC-004(无 archive 动作 / 状态列,亦无陈旧草稿工作台提示)· AC-009(计分规则全仓无按活动写接口,「无接口算不算满足」待裁定)·
...
### P1-32 RBAC 权限目录与角色权限管理终态 —— 9 个 PR,**PR 0/1/2/3a/3b/4a/4b/5 八刀已合(按 PR 编号完整落地 6/9),余 PR 6–8 待排**

**状态**:进行中(5/9 完整落地;PR 0/1/3a/3b/4a 已合 #1145 #1143 #1147 #1151 #1156,PR 2 已合 #1170,PR 4b 已合 #1171 —— **PR 4 两半齐全**;PR 5「影响预览与 Step-up」**已合 #1175**;余 PR 6–8,其中 PR 7 依赖前端投用)
