...
> **D2 implementation 已合并并完成主干验证（2026-09-12）**：D2 方向评审 [#1317](https://github.com/BA7IEE/srvf-nest-api/pull/1317) 与精确计划 [#1318](https://github.com/BA7IEE/srvf-nest-api/pull/1318) 均已合入；10 个精确路径的 implementation [#1319](https://github.com/BA7IEE/srvf-nest-api/pull/1319) 的 17 项 PR 检查与可信红区审批均成功，并已 squash 至 main `c0140c6efdfcbf1060b99ef8626b56ec8b65234c`。首次 main CI 只因过期日期棘轮基线失败；独立 [#1320](https://github.com/BA7IEE/srvf-nest-api/pull/1320) 删除陈旧基线后，含 D2 的 main `bcc297495c42f8a37ed06e946abe96db99d49d64` 在 [CI 34625317010](https://github.com/BA7IEE/srvf-nest-api/actions/runs/34625317010) completed/success。D2 新增的只是考勤域内部、显式 tx、只读的 `ParticipationSegmentFacade`；没有第二张服务段表、双写、迁移、API、DTO、权限、审计或 Gate 变更。维护者已确认 D3 方案 A，并授权起草精确实施计划与 docs-only PR；D3 implementation、D4-D8、生产部署、Gate 和整体跨模型复审仍未完成。
...
> **2026-09-10 C5 当前实施状态**：#1305 评审、#1306 精确计划已合并；本轮按维护者授权在40路径内实施两个正式成果报告接口，允许 w98 隔离验证及验证后提交推送开 PR，不合并。C4 已合并；C5 当前分支实现并进行验收，不再是“仅评审/未获实施批准”。单活动/最多20活动查询不跨活动求和、不返回证据或身份、不改变结算和归档；117迁移、258权限、165/160审计保持不变。满额及旧行为回归证据见 [C5实施计划](../plans/activity-os-r3-c5-implementation-plan.md)。PR CI、合并、前端发布、生产、Gate和整体复审尚未完成，下方旧时点表述以此为准。原紧急创建500仍未判定根因。
...
已完成真实满额 100 值/2000 引用、归档退队历史保留、59 项并发/回滚与存储状态验证；全仓 350 组单测（7859 项通过、5 todo）、quick 与 build 通过；新增诊断 E2E 93 项、旧行为诊断回归 56 项通过。验证范围仅 app_test_w98，不等同标准 CI 全量 E2E。
剩余：PR 创建及 CI、可信审批、合并、整体复审。C3-1 已合入 #1298；本节覆盖下方 C3-1/C3-2 的历史“未实施/仅文档”描述，不把 C3-2 登记为完成。
...
### P1-33 Activity OS 终态边界、数据所有权、Integration 安全与 AI 独立性 —— **T0-A / T0-B、Release 1 A1–A8、Release 2 B1–B7、Release 3 C1–C5、Release 4 D1-1/D1-2/D1-3/D2 已完成仓内交付；D3 方案 A 已确认、精确计划待 docs-only PR，implementation 仍待独立推进**

**状态**:进行中(T0-B #1236、A1 #1237、A2 #1239、A3 #1241、A4 #1244、A5 #1246、A6 #1248、A7 #1251、A8 #1254 与 B1 #1257、B2 #1259、B3 #1261、B4 #1264、B5 #1267、B6 D1 #1270 / D2 #1272、B7 #1275、Release 3 C1–C5、Release 4 D1-1 #1310 / D1-2 #1312 / D1-3 #1316 / D2 #1319 已完成相应 PR 与 main 验证；维护者已确认 D3 方案 A，本轮只起草精确实施计划并创建 docs-only PR，未获 implementation、数据库、测试库、合并或 Gate 授权；整体跨模型复审、前端页面发布、灰度人群、生产部署与 Gate 切换仍未执行，不宣称任何 Release 已上线)
...
- **B1 已交付并合入 #1257**:PlacePreset / ActivityPlace 存储地基和第 106 条 migration 纯 additive 落地；预设只作来源记录，活动 / 场次关联以复合锚物理同链。未接入地点 writer、地图、API、权限、Gate 或旧字段投影。
- **B2 已交付并合入 #1259**:坐标三值闭集、六条数据库约束、纯坐标投影策略与第 107 条 migration 已落地，并完成非空库 rehearsal；未接入地图、地点 writer、签到路径或旧字段写入。
- **B3 已交付并合入 #1261**:Definition V2 报名表蓝图复制、既有八种题型、受治理 Form canonical / managed 契约、v3–v5 Form 兼容及第 108 条 migration 已落地；敏感题目仍须逐题治理审批后启用。
...
- **C1 方案 A 的 D1 数据地基已交付并合入 #1278，D1–D2c 仓内实现已闭环**:[指标定义与指标集版本评审及授权清单](../archive/reviews/activity-os-r3-c1-metric-definition-set-review.md)已随 #1277 合入；D1 三表、五类强类型校验/hash、DB 冻结与并发约束及第 110 条 migration 已合入 `3b3e57aa`。本地验证、3b 重签、18 项 PR 检查和可信红区审批均通过；首次 B6 创建 HTTP 500 原样本地与最终 CI 未复现，根因未定位，不登记为已修复或误报，证据见 [#1278](https://github.com/BA7IEE/srvf-nest-api/pull/1278)。C1 内部 D2 外部契约方案 A 已批准，D2a、D2b、D2c 已分别随 #1280、#1282、#1284 合入；D2c 只新增 v7，不回改 v2–v6，并已完成 PR CI、可信审批和合并后 main CI。整体跨模型复审仍待统一完成；C2 尚未启动或授权。此 D1/D2 不指 Release 4 时长阶段；无生产 deploy、权限或 Gate 开启。
