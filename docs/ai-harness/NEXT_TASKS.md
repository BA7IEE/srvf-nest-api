...
> **D7-2 当前状态（2026-09-20）**：[#1335](https://github.com/BA7IEE/srvf-nest-api/pull/1335) 的方案 A / 第10–13节 / 124个精确路径已合入 `main`；[#1337](https://github.com/BA7IEE/srvf-nest-api/pull/1337) 保持 Draft。工作树新增第128条 `20260920110000_activity_os_r4_d7_2_correction_receipt_guard_set`：仅将已识别 correction receipt 的 immutable-proof 重复读取收敛为 AFTER INSERT 集合守护，保留 D3/D4 与错误 operationCode 的历史行守卫、父行/子项/proof/同活动锚点、锁序、拒绝优先级和7秒 fail-closed 边界。受控 `app_test_w98` 已完成第128条冷回放、127→128 非空升级、单身份与2,000身份 Human V3 链，并自动回收 w98；`pnpm test:contract` 另在本工作树受控 `app_test_*` 测试库应用第128条并通过1,072项契约，非生产库。实际 SQL 摘要待维护者第128条3b重签，之后才可更新签字登记、提交推送并由新 SHA PR CI裁决。13份固定其他 scratch 的旧迁移测试仍只由 PR CI冷跑；未 Ready、合并、操作生产或启用 Gate。
