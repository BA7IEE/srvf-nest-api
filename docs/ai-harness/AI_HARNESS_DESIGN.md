...
| 5 | feature(新 endpoint) | C | 目标模块 + dto + spec + snapshot | 跨模块 / schema / 权限码 | `agent:check:full` | ✅ 圈范围(已含于用户任务说明 / goal → 免二次确认) | 实现 PR(含 contract diff 解释) | snapshot 出现范围外 diff → 停 |
...
| 11 | release preparation | **E** | bump 3 文件 / handoff / current-state 回填 | 一切业务代码 | full + handoff 锚点 | ✅ 立项;tag/Release 由 AI 执行(gh),维护者亦可手动 | 沿 `srvf-release-closeout` | 任何阶段验收不过 |
...
| ④ 定档计划 | 任务 + [`PROJECT_RULES §3`](./PROJECT_RULES.md) | 档位 + 修改白名单 + 测试集([`TEST_MATRIX §2`](./TEST_MATRIX.md)) | 对照 process §3 档位归属规则(C 档范围已含于用户任务说明 / goal → 免二次确认) | 拿不准 → 按高档处理 |
...
4. 入口接线现状:`docs/README.md §1` 已登记本目录;根 `CLAUDE.md` §1 已增指向本目录的一行(P0-2,2026-06-10 用户拍板,PR #273);`AGENTS.md` 维持不加,非用户授权不动。
