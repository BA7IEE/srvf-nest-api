...
> 数据快照:2026-06-10,HEAD `18229ed`。✅ = 本次 Review 实测通过;🔶 = 需本地 Docker,本次未跑(daemon 未启动),CI 等价覆盖。
...
单测(20 个 `*.spec.ts`)关键资产——6 个 service-level characterization spec(attendances 716L / users 799L / attachments 850L / certificates 695L / activities 628L / activity-registrations 430L):纯构造注入 mock,不起 Nest 不连库,锁定 service 编排契约。**拆 service / 改内部编排前必须先跑;行为差异 = 停下报告,不是改断言。**
