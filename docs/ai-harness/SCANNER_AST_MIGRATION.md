...
1. ~~**给 §11.5 那 4 条「内存比对属主」端点加 e2e 行为锁**~~ —— **2026-08-14 开工后实测撤销：
   行为锁本就存在且绑得住，无工作可做**（变异对拍证据见 §11.5 的绿框）。立项前提
...
**第三类的静态可见性最差**：属主判定是一行**可删的 JS 比较**，删掉它**不会有任何结构性失败**
——没有 where 缺项、没有 SQL 变化、typecheck 照过、类型系统毫无察觉。前两类至少把属主
写进了查询条件。

> ### ✅ 已实测:行为层**已经锁住**,不需要补 e2e（2026-08-14 变异对拍）
>
> 上面那句「静态上看不见」成立，但**不能据此推出「删一行无人知」** —— 静态不可见 ≠ 行为
> 无人管。逐条变异实测（每次均校验变异命中数=1、还原残留=0）：
>
> | 端点 | 属主比较 | 变异后 | 抓住它的用例 |
> |---|---|---|---|
> | `GET /my/registrations/:id` | 1 处（`:1811`） | **红** | `app-my-registrations-read.e2e-spec.ts:508` |
> | `PATCH /my/registrations/:id/cancel` | **2 处**（`:1833` `:1838`） | 单删任一 → **全绿**；两道全删 → **红 2 条** | `app-my-registrations-write.e2e-spec.ts:695` `:793` |
> | `GET /notifications/:id` | 1 处（`:234`） | **红** | `notifications-directed.e2e-spec.ts:171` |
> | `POST /notifications/:id/read` | 同上（共用 `findVisibleOrThrow`） | **红**（同一条用例） | 同上 |
>
> 基线分别为 33 / 42 / 8 条全绿。**结论：四条端点的行为锁均已存在且绑得住，
> 「补 e2e 行为锁」一刀无工作可做，不得再写重复用例。**
>
> **残留（已知、不修）**：`cancelMy` 的两道属主检查是纵深防御，单删任一道 e2e 全绿。
> 这在 e2e 层**原理上不可修** —— 两道守卫的可观测行为完全相同（都返 404），黑盒测试
> 区分不了「一道」与「两道」。要钉住「必须有两道」只能上结构断言（lint 规则或源码级判据），
> 属另一种机制；且守卫被删到只剩一道时行为仍然正确，**判定为不值得**。
