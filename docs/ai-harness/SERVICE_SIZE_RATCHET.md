...
>
> **2026-08-21 复测:又回到 35,已越线。** 见 §3.3。

### 3.3 摩擦是**当期速率**,不是一次性成就(2026-08-21 复测)

| 时点 | revision | 严口径 | 判据线 |
|---|---|---|---|
| 立闸(2026-08-15) | — | 92 | 30 |
| 6-B 七刀后(2026-08-17) | `a2c11e02` | **27** ✅ | 30 |
| **复测(2026-08-21)** | `b82dfb34` | **35** ❌ | 30 |

**三天涨了 8。** 同期尺寸闸的读数:21 条基线里 **13 条变大,合计 +171 NCLOC**;
另有两个 6-B 亲手拆过的文件也在涨(`activity-publish-review` 908 → 969、
`attendance-punch-command` 1219 → 1257),它们因为还没超过基线值而不进 WARN 列表。

**为什么会这样,以及它不是意外**:

摩擦指标数的是「历史上有多少 PR 增长过**已超阈值**的基线文件」。它随每一次这样的增长**单调上升**,
而**只有让文件跌破 700 退出基线**才会下降 —— 单纯把文件改小(仍 ≥700)一格都不降。
6-B 七刀里只有 `attendance-offline-package` 真正跌破了 700,而它本来就不在基线里、贡献是 0。

所以 2026-08-17 那次 93 → 27 的降幅,**几乎全部来自基线重算时六个文件退出基线**,
不是来自"拆分本身"。而在 report 期,增长速度与拆分速度是两条独立曲线 ——
[[§3.2 末段]] 已经写过这句话,这里是它的第二次实测。

**这条读数的正确用法**:
- 它是**当期速率**的度量,**有效期到下一次 src 变更为止**。任何引用它的地方
  必须带上取数的 revision,否则读者无从判断是否过期。
- **转闸前必须重跑本文 §3 复测**,不得引用历史读数 —— 本节存在的直接原因就是
  §4 的「专属」条曾长期挂着一个 2026-08-17 的 ✅,而彼时实况已是 ❌。
- 想让它真正下降,只有一条路:**把基线文件拆到 700 以下并重算基线**。
  按当前归因,最划算的是 `users.service.ts`(897,只需 −198 就跌破,换 5 点)。

**已加机器守护**:`harness-guards.selftest.ts` 断言「只要尺寸闸报了 `service-size-ratchet`
WARN(即有基线文件当前值 > 基线值),§4 的专属条就不得写成 ✅」。
逻辑依据:基线文件当前值高于基线值 ⇒ 必定发生过至少一次「已超阈值文件被增长」
⇒ 摩擦必定较冻结时上升。这条守护挡不住"摩擦涨了但没超基线"的情形(如上面那两个文件),
但挡得住本次这种**文档 ✅ 与闸 WARN 公然矛盾**的形态。
...
| **专属** | **6-B 拆分已把摩擦压到可接受区间**(重跑本文 §3 复测) | ❌ **不成立(2026-08-21 复测)** —— 严口径 **35**,已越过判据线 30。2026-08-17 那次确实测到 27(逐刀实测见 §3.1),但那是**当时**的读数;见 §3.3。 |
