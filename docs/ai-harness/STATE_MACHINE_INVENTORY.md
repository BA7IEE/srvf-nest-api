...
## D7-1 当前实施增量（分支验证中，未提交）

新增 `ParticipationTimeCorrectionEntry.categoryCode` 与 `entryTypeCode` 两项 L1 inventory、
not-derived 配置。前者保留四种根分类，后者固定为 reversal / credit，零值也明确配对。
三张更正表永久追加，不引入生命周期或状态边，不提升 governed。检查器仅发现这两个精确
模型/字段对，其他同名字段不纳入。当前登记 74 项，其中 8 项 governed、66 项 inventory；
后续容量、迁移和全量验收尚未完成，下方记录保留原历史时点。
...
`transitions` 为 `not-derived`。它区分创建时的不可变修订类型，不存在 draft 行原地变为
submitted 的状态边；正式提交另外创建新行并保留草稿。第 121 条 migration 的复合 shape
CHECK、同链与完整性触发器约束证明、版本、桶及收据；不提升 governed，不新增生命周期。
当前共 71 项，其中 8 项 governed、63 项 inventory；下方阶段描述保留其历史时点。

## D3 当前实施增量（未合并）

`ParticipantTimeAllocationRevision.recognitionModeCode` 登记为 L1 inventory，闭集为
`automatic` / `manual`，`transitions` 为 `not-derived`。它记录创建时的认定方式，创建后
不可修改，不存在模式切换或独立生命周期。第 120 条 migration 的复合 shape CHECK 同时约束
模式与人工理由，append-only trigger 保留全部历史。此处只补登记，不新增状态边或提升
`governed`；当前登记为 70 项，其中 8 项 governed、62 项 inventory。下方旧阶段读数保留为历史。
...
**取数时点:2026-09-15(Activity OS R4 / D7-1 当前实施分支，未合并)**

| 项                                    |                               值 |
| ------------------------------------- | -------------------------------: |
| 总条目                                |                           **74** |
| `governed` / `inventory`              |                       **8 / 66** |
| 66 条 inventory 的分层                | L1 **14** · L2 **26** · L3 **26** |
| 已有机器可读边(`transitions` 是数组)※ |                               30 |
| `transitions: "not-derived"` ※        |                               31 |
| `transitions: "unconstrained"` ※      |                               13 |

> ※ 这三行按**全部 74 条**统计(30+31+13=74),不是按上一行那 66 条 inventory。
...
> 已知射程限制,不是本刀新引入的缺口。
>
> **④ A3 的当前读数变化**:`ActivityTemplate.statusCode` 没有新增登记条目，仍是 L2
> inventory；但它从「legacy 无闭集、无边、无写路径」变为「仅 `familyId IS NOT NULL` 的
> future Version 由 migration 条件 CHECK + trigger 定义 `draft → active → retired`」。因此
> `transitions` 数组 21→22、`not-derived` 26→25、`no-db-check` 24→23、
> `edges-not-derived` 21→20、`closed-set-undeclared` 6→5；新增
...
> **⑧ C1 D1 的当前读数变化**：新增 `ActivityMetricDefinition.statusCode` 与
> `ActivityMetricSetVersion.statusCode` 两条 L2 inventory；总条目 63→65、inventory 55→57、
> L2 22→24、数组边 24→26。数据库约束与触发器定义 draft → active → retired，
...
- ❌ **不改既有状态机或 `action-state-checks.ts`**：A3 的生命周期只由 migration trigger 与
  纯函数 canonicalization 定义，不把它冒充为已有业务状态机收口。
