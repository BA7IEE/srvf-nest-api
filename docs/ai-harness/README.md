...
| [`DICTIONARY_SEED_REGISTRY.md`](DICTIONARY_SEED_REGISTRY.md) | 字典 seed 逐条登记表(P2-23a);由 `src/modules/dictionaries/seed-dictionary-registry.spec.ts` **双向对拍**(漏登记 / 多登记 / label 漂移 / 新增写入点各自判红)。⭐ 它把合同 ④「字典对账」从「签字接受现状」变成机器可判 —— 此前只有 `seed-sha256-12` 整份摘要,**增删一个字典项不会让任何读数动一下**。 |
| [`AUDIT_EVENT_REGISTRY.md`](AUDIT_EVENT_REGISTRY.md) | 审计事件登记表(P2-23b):`AuditLogEvent` union **147 个事件**逐条点名(142 活跃 + 5 已退役/零产出);由 `src/modules/audit-logs/audit-event-registry.spec.ts` **双向对拍**(漏登记 / 多登记 / 出现次数漂移 / 静默死事件各自判红)。⭐ 合同 ④「Audit events 对账」半的落点 —— 事件名虽已被闭 union 收编(TS 静态锁三条写库漏斗),但 union 里有哪些事件此前无对外清单,④-b 签字只能「接受现状」。 |
