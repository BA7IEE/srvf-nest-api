...
| [`FROZEN_DRAFTS.md`](FROZEN_DRAFTS.md) | **冻结稿落地台账**:已拍板冻结的施工依据还欠多少 —— §1 逐项欠账 / §2 机器读数(生成块,手改即红)/ §3 归档评审稿全量四值分类。判据 `src/frozen-drafts-ledger.criteria.spec.ts` 守「不许有未分类项」+「读数不过期」;刷新读数 `pnpm exec tsx scripts/frozen-drafts-ledger.ts --write` |
