...
| [`DICTIONARY_SEED_REGISTRY.md`](DICTIONARY_SEED_REGISTRY.md) | 字典 seed 逐条登记表(P2-23a);判据本体在 `scripts/check-dictionary-seed-registry.ts`(红区)双向对拍(漏登记 / 多登记 / label 漂移 / 新增写入点各自判红),spec 侧是薄运行器 + 常驻变异对拍。⭐ 它把合同 ④「字典对账」从「签字接受现状」变成机器可判 —— 此前只有 `seed-sha256-12` 整份摘要,**增删一个字典项不会让任何读数动一下**。 |
| [`AUDIT_EVENT_REGISTRY.md`](AUDIT_EVENT_REGISTRY.md) | 审计事件登记表(P2-23b):`AuditLogEvent` union **156 个事件**逐条点名(151 活跃 + 5 已退役/零产出);判据本体在 `scripts/check-audit-event-registry.ts`(红区)双向对拍(漏登记 / 多登记 / 出现次数漂移 / 静默死事件各自判红),spec 侧是薄运行器 + 常驻变异对拍。⭐ 合同 ④「Audit events 对账」半的落点 —— 事件名虽已被闭 union 收编(TS 静态锁三条写库漏斗),但 union 里有哪些事件此前无对外清单,④-b 签字只能「接受现状」。 |
...
| [`LEGACY_LEDGER_CONVERSION_DRAFT.md`](LEGACY_LEDGER_CONVERSION_DRAFT.md) | 「存量考勤账本化」转换刀**设计稿(已签收,施工依据)** —— 第 7 批②账本桥 A 案(2026-08-27 拍板):锚点证据、D1/D2 业务拍板记录、D3 复核点、D4–D13 保守工程裁定、DoD、§16.3 SOP 集成;实施收口时随刀归档冻结入 `archive/reviews/` |
