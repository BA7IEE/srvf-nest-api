...
| [`AUDIT_EVENT_REGISTRY.md`](AUDIT_EVENT_REGISTRY.md) | 审计事件登记表(P2-23b):`AuditLogEvent` union **147 个事件**逐条点名(142 活跃 + 5 已退役/零产出);判据本体在 `scripts/check-audit-event-registry.ts`(红区)双向对拍(漏登记 / 多登记 / 出现次数漂移 / 静默死事件各自判红),spec 侧是薄运行器 + 常驻变异对拍。⭐ 合同 ④「Audit events 对账」半的落点 —— 事件名虽已被闭 union 收编(TS 静态锁三条写库漏斗),但 union 里有哪些事件此前无对外清单,④-b 签字只能「接受现状」。 |
