...
> **D7-2 当前状态（2026-09-20）**：[#1335](https://github.com/BA7IEE/srvf-nest-api/pull/1335) 的方案 A / 第10–13节 / 124个精确路径已合入 `main`；[#1337](https://github.com/BA7IEE/srvf-nest-api/pull/1337) 保持 Draft。工作树新增第128条 `20260920110000_activity_os_r4_d7_2_correction_receipt_guard_set`：只把已识别 correction receipt 从旧逐行 immutable-proof 分支迁为 AFTER INSERT 集合守护，D3/D4、非 correction receipt 与错误 operationCode 仍走历史行守卫；父行、子项、proof、同活动锚点、锁序、拒绝优先级、7秒预算及 fail-closed 边界不变，不新增表、列、权限、DML、回填或删除。受控 `app_test_w98` 的第128条冷回放、127→128 非空升级、单身份及2,000身份 Human V3 链已通过并回收 w98；`pnpm test:contract` 另在本工作树受控 `app_test_*` 测试库应用第128条并通过1,072项契约，非生产库。第128条实际 SQL 摘要尚待维护者3b重签，签字前不更新签字登记或推送。其他 scratch 的13份旧迁移测试只由新 SHA PR CI冷跑；上述本地结果不替代 CI，未 Ready、合并、操作生产或启用 Gate。
