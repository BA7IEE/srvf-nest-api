...
> **D7-2 当前状态（2026-09-20）**：[#1335](https://github.com/BA7IEE/srvf-nest-api/pull/1335) 的方案 A / 第10–13节 / 124个精确路径已合入 `main`；[#1337](https://github.com/BA7IEE/srvf-nest-api/pull/1337) 保持 Draft，待本轮提交推送后由新 SHA CI 裁决。维护者已按实际 SHA-256 `d76418fb2b837a6ff264c4d061b43e7c0b476d3b71b238480b2ce67a8aeaf51d` 重签第127条 migration；签字机器对拍 `migration-total=127` 通过。该 SQL 仅把 correction allocation 的五次重复 immutable-chain 拒绝查询收敛为一次集合聚合，并以去重输入维持既有锁序、拒绝优先级、D3/D4路径、`ptar_receipt_guard`、7秒预算和 fail-closed 边界。受控 `app_test_w98` 冷回放/126→127 非空升级及守卫/历史校验共4/4通过，完整D7-2 E2E 7/7、typecheck、lint、Harness自检和1,072项contract均通过；运行只自建并回收 w98，不触碰模板库或其他 scratch worker。上述本地结果不替代新 SHA 的 PR CI，未 Ready、合并、操作生产或启用 Gate。
