...
> **D7-2 当前状态（2026-09-18）**：[#1335](https://github.com/BA7IEE/srvf-nest-api/pull/1335) 的方案 A / 第10–13节 / 124个精确路径已合入 `main`；[#1337](https://github.com/BA7IEE/srvf-nest-api/pull/1337) 保持 Draft。远端 SHA `658d7b13` 的 PR CI 已确认：E2E shard 4 有一条 D6 旧迁移冷回放失败，根因是首次维护库连接探针与随后目标库诊断不在同一观察面；E2E shard 5 的原2,000身份 Human V3写链仍在 7,383ms 超过7,000ms事务预算（P2028）。两者均不能登记为绿。本工作树仅按本轮批准处理前者：首次非零探针从同一维护库立即复核，持续非零仍 fail-closed 拒绝并仅输出脱敏聚合诊断；不加固定等待、重试或超时。D7 的 P2028 继续只读定位，不混入本次修复。本轮未改业务代码、schema、migration、API、DTO、权限、Gate 或既有 E2E 断言；唯一获准的 `app_test_w98` 验证后才提交/推送 #1337，并对新 SHA 重新取得可信审批和 PR CI 冷跑。尚未 Ready、合并、操作生产或启用 Gate。D7-1 的 #1334 与 #1336 已合入 main；#1334 的既知2000身份审计回滚超时根因仍未定，不冒称已修复。
