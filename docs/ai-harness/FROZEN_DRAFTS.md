...
> **D7-2 当前状态（2026-09-19）**：[#1335](https://github.com/BA7IEE/srvf-nest-api/pull/1335) 的方案 A / 第10–13节 / 124个精确路径已合入 `main`；[#1337](https://github.com/BA7IEE/srvf-nest-api/pull/1337) 保持 Draft，当前 HEAD 为 `f779191d`。该 SHA 的 [CI 35449265173](https://github.com/BA7IEE/srvf-nest-api/actions/runs/35449265173) 已完成，可信扫描及维护者 `harness-review` 审批通过；其余两个 E2E 红点不能登记为已修复：A7 的两段受控 fixture cleanup 中一段继承 5 秒默认值后在 6,421ms P2028，Human V3 的 2,000 身份写链在 7 秒事务预算内于 7,889ms P2028。本轮仅将 `resetDb` 的全局受控 cleanup transaction 与 A7 的两段受控 cleanup transaction 设为 60 秒，并将 10,000 条不可变来源绑定改为每批最多 5,000 条；保留当前库校验和精确清理 SQL，不提高业务超时、不放宽断言、不改 schema、migration、API、DTO、权限或 Gate。`app_test_w98` 的原始 A7 目标用例已于 44.604 秒通过，绑定批次单测、typecheck、lint 与 harness 自检亦通过；提交推送更新 #1337 和新 SHA 的 PR CI 仍待本轮完成，未 Ready、合并、操作生产或启用 Gate。若仍需 migration，必须先有实际 SQL 的新 3b 重签。
