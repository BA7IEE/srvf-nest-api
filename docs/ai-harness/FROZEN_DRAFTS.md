...
> **D2 implementation 当前状态（2026-09-11）**：D2 方向评审 [#1317](https://github.com/BA7IEE/srvf-nest-api/pull/1317) 与精确计划 [#1318](https://github.com/BA7IEE/srvf-nest-api/pull/1318) 均已合入；当前 main 为 `c2a687bf2e9e81d1199329021f06abca40d8f8aa`，其 [CI 34597888732](https://github.com/BA7IEE/srvf-nest-api/actions/runs/34597888732) completed/success。维护者已授权 10 个精确路径，当前分支已增加考勤域内部只读 `ParticipationSegmentFacade` 并完成本地隔离验证；没有 schema/migration、双写、外部访问面、权限、审计或 Gate 变更。implementation PR 尚未创建：新增源文件使 `harness/domain-map.json` 与 `docs/ai-harness/ROUTE_AUTHZ.md` 两份受保护派生物的输入指纹过期，二者不在原 10 路径授权内，尚待维护者单独授权；D2 不因此记作已合入或已部署。
...
> **2026-09-10 C5 当前实施状态**：C4已合并#1304；C5评审/精确计划已合并#1305/#1306，本轮40路径实施已获维护者批准，两个只读报告入口已在分支实现并验证中。当前为未合并实施，不是下方历史记录中的“仅起草/未授权”；不得登记T0收口。证据见 [C5实施计划](../plans/activity-os-r3-c5-implementation-plan.md)。报告不跨活动求和，不增迁移/权限/审计，不删除业务数据。PR CI、合并、整体复审、前端/生产和Gate未完成；原紧急创建500根因未定位。
...
| 9   | Activity OS T0-A 终态合同    | P1-33 | `↔进行中` T0-A / T0-B、Release 1 A1–A8、Release 2 B1–B7、Release 3 C1–C5 已完成相应仓内实施；Release 4 的 D1-1/#1310、D1-2/#1312 与 D1-3/#1316 均已合入且 main CI 通过；D2 评审 #1317 与计划 #1318 已合入，implementation 在本地完成验证、待两份受保护派生物授权后创建 PR | 原紧急创建 500 根因未定位；整体跨模型复审、Release 4 D2–D8 及后续、前端发布、生产部署和 Gate 切换未完成                 |
...
**⑤ Activity OS T0-A —— T0-A / T0-B、Release 1 A1–A8、Release 2 B1–B7、Release 3 C1–C5 与 Release 4 D1-1/D1-2/D1-3 的仓内实施已通过；D2 评审 #1317 与计划 #1318 已合入，implementation 已完成本地验证、待受保护派生物授权后创建 PR；前端发布、生产 Gate、整体跨模型复审与 Release 4 D2–D8／后续 Release 未完成**
...
API/DTO、权限/Gate、seed、回填和生产部署均未改。
**A5 已于 2026-09-02 以 #1246 独立合并**：既有模板解析改为非空
`selectedTemplateVersionId` 按 id 精确读取、NULL 时逐字保留 legacy active fallback；已选 future
...
**A8 已于 2026-09-03 以 #1254 独立合并**：新增冻结合同点名的 `test:business:no-ai`，按结构化
AI 零依赖判据、真实 HTTP Journey、A7 Series E2E 串行验收；A7 在创建 Nest app 前显式关闭 Activity
...
**B4 已于 2026-09-04 以 #1264 独立合并**：新增只读、确定性、gate-off 的
ActivityPublishReadinessService，在一次只读事务内按固定顺序汇总结构化问题，不接入现有发布链路；A5
模板解析、Session / Position 旧字段、canonical Form / Qualification、保险 enforcement 与当前组织 / 发起人
可解析性均有覆盖。B4 不改 schema、migration、API、DTO、OpenAPI、权限、审计、Gate、快照或生产行为；PR CI
与可信红区审批均已收口。**B5 已于 2026-09-04 以 #1267 独立合并**：新 initial / change 发布审核统一生成 schemaVersion=6，
冻结当前可解析的分类 / facet、原始 selectedTemplateVersionId、ActivityPlace 本地快照和最小可见性摘要；
...
**B6 D2 已于 2026-09-05 以 #1272 独立合并**：三个 App managed POST 入口分别落实模板快速、
专业完整和紧急创建，使用独立 App DTO 与显式命令映射。快速模式复用 A6 精确 Version 与幂等物化，
地点参与请求 hash；专业模式将草稿、场次、岗位、地点、受治理 Form、资格配置、收据与安全审计同事务提交。
紧急模式校验普通创建权与专属紧急权限，草稿、紧急起源、七项事后义务、冻结受众和一次 outbox 同事务提交；
重放不重复通知，响应不返回原始名单或精确地点。义务只按现有权威事实判定，无模型支撑的事项不伪造完成。
三种创建均不直接正式发布，紧急草稿在既有发布提交和实际写入链上被拒绝，不冒充事故处置。
...
第 111 条 migration、250 权限、Audit 159 总计 / 154 活跃已随
...
三态选择、Template V3、最小 Human 模板维护及五条物化链已随 [#1282](https://github.com/BA7IEE/srvf-nest-api/pull/1282) 合入 `f5b5b226`。
...
**当前实施进展（覆盖历史待授权表述）**：C2 D1 已随 [#1289](https://github.com/BA7IEE/srvf-nest-api/pull/1289) 合入 `c2fa226d`，三个模型、第 113 条 additive migration 与纯值校验已交付；113 冷回放、112→113 非空保留、8 项数据库测试、C1/C2 55 项单测、兼容回归、contract、18 项 PR 检查及维护者 3b 重签已完成。首次 main CI 的 B6 创建 500 根因未定位；[#1290](https://github.com/BA7IEE/srvf-nest-api/pull/1290) 仅补脱敏诊断，已合入 `22c1ba8f`，15 项 PR 检查与[最终 main CI](https://github.com/BA7IEE/srvf-nest-api/actions/runs/34092294387)通过，原失败测试本轮通过，不登记为修复或误报。D1 计划标 landed，C2 总评审因 D2 尚欠保持 open。本段的“C3 未授权”是当时记录；当前 C3-1 已获独立 implementation 授权并在单独分支验证，整体跨模型复审、生产与 Gate 仍未完成。
...
**C2 D2 当前收口（覆盖上述历史未实施／未合并表述）**：[#1293](https://github.com/BA7IEE/srvf-nest-api/pull/1293) 已于 2026-09-07 squash 合入 `7f4fdbd7`；18 项 PR 检查、可信红区审批及 [main CI 34118403784](https://github.com/BA7IEE/srvf-nest-api/actions/runs/34118403784) 全部通过。人工成果完整草稿／修订、历史读面、专用收据与事务审计已交付，migration 114、权限 254、Audit events 162 总计／157 活跃；3b/4b 已重签。Authz 既有查询登记只对齐方法名，未新增组织域依赖；旧 C1 D2b 当前 seed 计数已按批准更新，历史数据与八操作断言保留。C2 D1/D2 实施完成，C2 总评审因整体跨模型复审未做保持 open；D2 实施计划标 landed。后续维护者已单独批准 C3 方案 A 与 C3-1 implementation；本段的“C3 未授权”仅保留历史时点，生产与 Gate 仍未授权。
