...
> **D7-2 当前状态（2026-09-18）**：[#1335](https://github.com/BA7IEE/srvf-nest-api/pull/1335) 的方案 A / 第10–13节 / 124个精确路径已合入 `main`；[#1337](https://github.com/BA7IEE/srvf-nest-api/pull/1337) 保持 Draft。远端当前 SHA `14eacf7e` 的 PR CI 已确认只有一条真实 P2028：E2E shard 5 的原2,000身份 Human V3写链在 7,029ms 超过7,000ms事务预算；不能登记为绿。本工作树在同一已批准范围完成第二个等价收敛：完整来源证明物化按来源最多1,000、子行最多5,000分批追加并精确计数，跨活动重叠校验改为等价 `EXISTS` 集合查询，更正账本九项 fail-closed 形状事实收敛为一次主聚合、一次冲回聚合和一次反连接复核；未改业务超时、schema、migration、API、DTO、权限、Gate 或既有 E2E 断言。唯一获准的 `app_test_w98` 由当前126条 migration 冷建后，原2,000身份 Human V3写链通过（81.287秒）；原8,192人账本规模用例在此前同一授权验证中通过（commit 1,111ms、28条SQL）。静态 typecheck、lint 与对应单测也通过。下一步仅是提交/推送 #1337、对新 SHA 重新取得可信审批并由 PR CI 冷跑；尚未 Ready、合并、操作生产或启用 Gate。D7-1 的 #1334 与 #1336 已合入 main；#1334 的既知2000身份审计回滚超时根因仍未定，不冒称已修复。
