...
> **#1339 D7-2 主干 CI 修复当前状态（2026-09-21）**：[#1338](https://github.com/BA7IEE/srvf-nest-api/pull/1338) 已 squash 合入 `main` 的 `562ee0350b67f9887de0c5edd689a8437d153c27`；后续 Draft [#1339](https://github.com/BA7IEE/srvf-nest-api/pull/1339) 的首轮 CI 只有第3组 E2E 红。失败用例为 8,192 人账本生效，未改的 `assertNoCrossActivitySegmentOverlap` 在7,318ms使7秒 member transaction 关闭，两个 fixture 清理改动不直接触及该路径。维护者已批准仅把这条重叠存在性查询改为从其他活动 committed 段出发的等价内连接：同成员、异活动、draft／committed、非 voided/replaced、闭合段及半开区间重叠条件逐字保留，member 锁、业务7秒预算、断言、schema、migration、API、DTO、权限和 Gate 均未改。获准 `app_test_w98` 的 AC-058 与规模套件共29/29通过；冷重建后的规模用例1/1通过，commit 912ms、27条语句／17条裸SQL，唯一8,192 bind仍是既有 member advisory lock。待提交推送后的新 SHA 重新经过 PR CI；#1339保持 Draft，不 Ready、不合并、不操作生产或启用 Gate，也没有新 migration/3b/4b。
