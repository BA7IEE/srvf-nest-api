...
| `270xx`-`290xx` | 未规划模块预留 | — | 训练 / 装备 / 财务 / 通知等真到时候分配(中间留缓冲) |
...
- **V2 基线预留**(历史措辞,适用于初版 v0.1 / 2026-05-07 时态):形态级几乎确定会进入 V2,段位预留不会浪费,具体字段集待 D6/D7;**当前**:`dictionaries` / `organizations` / `members` / `member_departments` / `member_profiles` / `attachments` / `permissions` / `sms` / `wechat` / `insurances` 均已实装(详见各模块"已实装"行 + [`docs/current-state.md §2`](current-state.md));仅 `270xx-290xx` + `310xx` 起仍属未规划预留
- **保险模块已实装**(2026-06-13;goal「保险模块」T2/T3):`insurances` 收口 `260xx + 261xx` 段位,6 个 BizCode 实装落地(`26001` `MEMBER_INSURANCE_NOT_FOUND` / `26002` `TEAM_INSURANCE_POLICY_NOT_FOUND` / `26003` `TEAM_INSURANCE_COVERAGE_NOT_FOUND` / `26004` `TEAM_INSURANCE_COVERAGE_ALREADY_EXISTS`〔覆盖名单 partial unique 单加重复,P2002 兜底同码〕/ `26010` `INSURANCE_COVERAGE_DATE_RANGE_INVALID` / `26030` `INSURANCE_REQUIRED`〔报名门槛,409 沿 20120/21030 报名业务态冲突家族;`requiresInsurance=false` 活动零查询不触发〕);**不开** `261xx` `FORBIDDEN_*`(沿 baseline,权限不足走通用 30100/40300)与「过期 vs 无保险」细分码(统一 26030,前端提示价值无差);未规划模块从 `270xx` 起;冻结评审稿 [`docs/archive/reviews/insurance-module-review.md §3.3`](archive/reviews/insurance-module-review.md)
