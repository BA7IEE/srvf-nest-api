...
| `220xx` + `221xx` | `attendances` | 200 | **批次 3 已实装**(v0.4.0,2026-05-11)|
...
V2 **不修改** `ARCHITECTURE.md §6` 已交付的 14 个 v1 接口的:
...
### 11.2 v1 14 接口清单(权威源 = `ARCHITECTURE.md §6`)

V2 文档不重复罗列字段集,以 `ARCHITECTURE.md §6` 为单一权威源。V2 任何修改 `auth/` / `users/` / `health/` 模块的动作,必须先回到 §6 比对兼容性。
