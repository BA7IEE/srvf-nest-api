...
| `250xx` + `251xx` | `wechat` | 200 | **微信小程序登录已实装**(2026-06-12 goal 拍板;`250xx` 段 4 BizCode〔25002/25010/25030/25031〕;**不开** `25001` `WECHAT_NOT_BOUND`〔零 throw 路径〕与 `251xx` `FORBIDDEN_*`;统一码防侧写沿 10007/24010 先例;冻结评审稿 [`docs/archive/reviews/wechat-mini-login-review.md §3.3`](archive/reviews/wechat-mini-login-review.md))|
| `260xx` + `261xx` | `insurances` | 200 | **保险模块已实装**(2026-06-13;maintainer 2026-06-12 goal 拍板;`260xx` 段 6 BizCode〔26001/26002/26003/26004/26010/26030〕;**不开** `261xx` `FORBIDDEN_*`〔权限拒绝走通用 30100/40300〕与过期细分码〔同 26030〕;冻结评审稿 [`docs/archive/reviews/insurance-module-review.md §3.3`](archive/reviews/insurance-module-review.md))|
| `270xx` | `realname` | 200 | **实名核验通道已实装**(2026-06-18;goal「招新一期」T2;`270xx` 段 2 BizCode〔27030 `REALNAME_CHANNEL_NOT_CONFIGURED` / 27031 `REALNAME_API_FAILED`〕,仅通道状态段镜像 sms 24030/24031 / wechat 25030/25031;**不开** `271xx` `FORBIDDEN_*`;「核验不匹配」不是 BizCode〔是 verify 结果驱动报名状态机〕;冻结评审稿 [`docs/archive/reviews/recruitment-phase1-review.md §3.3`](archive/reviews/recruitment-phase1-review.md))|
| `280xx` | `recruitment` | 200 | **招新报名已实装**(2026-06-18;goal「招新一期(招新前段)」T3;`280xx` 段 8 BizCode〔28001 `RECRUITMENT_CYCLE_NOT_FOUND` / 28002 `RECRUITMENT_APPLICATION_NOT_FOUND` / 28003 `RECRUITMENT_DUPLICATE_APPLICATION`〔同轮同身份证号 partial unique,P2002 兜底同码〕 / 28010 `RECRUITMENT_AGE_OUT_OF_RANGE` / 28011 `RECRUITMENT_ID_CARD_IMAGE_REQUIRED` / 28030 `RECRUITMENT_CYCLE_NOT_OPEN` / 28031 `RECRUITMENT_CYCLE_CAPACITY_FULL` / 28040 `RECRUITMENT_APPLICATION_NOT_PENDING_MANUAL`〕;**不开** `281xx` `FORBIDDEN_*`〔权限拒绝走通用 30100/40300〕;「实名核验不匹配」不是 BizCode〔verify 结果驱动报名状态机 rejected〕;冻结评审稿 [`docs/archive/reviews/recruitment-phase1-review.md §3.3`](archive/reviews/recruitment-phase1-review.md))|
| `290xx` + `291xx` | `content` | 200 | **CMS 内容发布模块已实装**(2026-06-21;goal「内容发布模块」;`290xx` 段 5 BizCode〔29001 `CONTENT_NOT_FOUND` / 29010 `CONTENT_TYPE_INVALID` / 29011 `CONTENT_VISIBILITY_INVALID` / 29012 `CONTENT_VISIBLE_ORG_INVALID` / 29030 `CONTENT_INVALID_STATUS_TRANSITION`〕;附件类错误复用 `13xxx`(经 AttachmentsService);搜索/标签非法走通用 400;**不开** `291xx` `FORBIDDEN_*`〔权限拒绝走通用 30100/40300〕;冻结评审稿 [`docs/archive/reviews/content-module-review.md §7`](archive/reviews/content-module-review.md))|
...
- **V2 基线预留**(历史措辞,适用于初版 v0.1 / 2026-05-07 时态):形态级几乎确定会进入 V2,段位预留不会浪费,具体字段集待 D6/D7;**当前**:`dictionaries` / `organizations` / `members` / `member_departments` / `member_profiles` / `attachments` / `permissions` / `sms` / `wechat` / `insurances` / `realname` 均已实装(详见各模块"已实装"行 + [`docs/current-state.md §2`](current-state.md));仅 `310xx` 起仍属未规划预留(`280xx` 招新 / `290xx` CMS content 已收口;`300xx`/`301xx` = permissions)
- **保险模块已实装**(2026-06-13;goal「保险模块」T2/T3):`insurances` 收口 `260xx + 261xx` 段位,6 个 BizCode 实装落地(`26001` `MEMBER_INSURANCE_NOT_FOUND` / `26002` `TEAM_INSURANCE_POLICY_NOT_FOUND` / `26003` `TEAM_INSURANCE_COVERAGE_NOT_FOUND` / `26004` `TEAM_INSURANCE_COVERAGE_ALREADY_EXISTS`〔覆盖名单 partial unique 单加重复,P2002 兜底同码〕/ `26010` `INSURANCE_COVERAGE_DATE_RANGE_INVALID` / `26030` `INSURANCE_REQUIRED`〔报名门槛,409 沿 20120/21030 报名业务态冲突家族;`requiresInsurance=false` 活动零查询不触发〕);**不开** `261xx` `FORBIDDEN_*`(沿 baseline,权限不足走通用 30100/40300)与「过期 vs 无保险」细分码(统一 26030,前端提示价值无差);未规划模块从 `280xx` 起;冻结评审稿 [`docs/archive/reviews/insurance-module-review.md §3.3`](archive/reviews/insurance-module-review.md)
- **实名核验通道已实装**(2026-06-18;goal「招新一期(招新前段)」T2):`realname`(第 25 模块,镜像 wechat/sms 通道层)收口 `270xx` 段位,2 个通道状态 BizCode 实装(`27030` `REALNAME_CHANNEL_NOT_CONFIGURED` / `27031` `REALNAME_API_FAILED`;仅通道段,镜像 sms 24030/24031 / wechat 25030/25031);**不开** `271xx` `FORBIDDEN_*`(权限拒绝走通用 30100/40300);「核验不匹配」不是 BizCode(是 verify 结果,驱动 T3 报名状态机 rejected);真通道休眠(DevStub 全验,真实腾讯云凭证待运维);未规划模块从 `290xx` 起(`280xx` 已由招新报名 T3 收口);冻结评审稿 [`docs/archive/reviews/recruitment-phase1-review.md §3.3`](archive/reviews/recruitment-phase1-review.md)
- **招新报名已实装**(2026-06-18;goal「招新一期(招新前段)」T3 收口):`recruitment`(第 26 模块)收口 `280xx` 段位,8 个 BizCode 实装(`28001` `RECRUITMENT_CYCLE_NOT_FOUND` / `28002` `RECRUITMENT_APPLICATION_NOT_FOUND` / `28003` `RECRUITMENT_DUPLICATE_APPLICATION`〔同轮同身份证号 partial unique,P2002 兜底同码〕 / `28010` `RECRUITMENT_AGE_OUT_OF_RANGE` / `28011` `RECRUITMENT_ID_CARD_IMAGE_REQUIRED` / `28030` `RECRUITMENT_CYCLE_NOT_OPEN` / `28031` `RECRUITMENT_CYCLE_CAPACITY_FULL` / `28040` `RECRUITMENT_APPLICATION_NOT_PENDING_MANUAL`);**不开** `281xx` `FORBIDDEN_*`(权限拒绝走通用 30100/40300);「实名核验不匹配」不是 BizCode(verify 结果驱动报名状态机 rejected);**两层身份铁律**:临时编号绑 `recruitment_applications`、**不**进 `members`(phase-2 promote 才建 User+Member,出本期范围);**`open/v1` surface 首用**(api-surface-policy §0「预留→首用」);未规划模块从 `290xx` 起;冻结评审稿 [`docs/archive/reviews/recruitment-phase1-review.md §3.3`](archive/reviews/recruitment-phase1-review.md)
- **CMS 内容发布模块已实装**(2026-06-21;goal「内容发布模块」第 28 模块):`content` 收口 `290xx` 段位,5 BizCode 实装(`29001` `CONTENT_NOT_FOUND` / `29010` `CONTENT_TYPE_INVALID` / `29011` `CONTENT_VISIBILITY_INVALID` / `29012` `CONTENT_VISIBLE_ORG_INVALID` / `29030` `CONTENT_INVALID_STATUS_TRANSITION`);附件类错误复用 `13xxx`(content-image/content-file 经 AttachmentsService 写路径,**不新增 owner 段码**);**不开** `291xx` `FORBIDDEN_*`(权限拒绝走通用 30100/40300);**`open/v1` surface 第二用**(内容公开列表/详情);未规划模块从 `310xx` 起(`300xx`/`301xx` = permissions);冻结评审稿 [`docs/archive/reviews/content-module-review.md §7`](archive/reviews/content-module-review.md)
...
- `oldPassword`
  - 2026-07-04 注记(pre-go-live readiness review v0.35.0 §4 F-1 true-up):`ChangeMyPasswordDto.oldPassword`(V2 期本人改密字段,`users.service.ts` changeMyPassword 使用)真实落表使用;§8.1 是 V1.1 已锁清单(仅 password / newPassword / passwordHash 等原始 9 项),不含 `oldPassword`,故本字段作为 V2 新增归入本节而非塞入 §8.1。`logger-options.ts` 已补录 `req.body.oldPassword` / `*.oldPassword`(#499);本节此前漏列,本次补齐。

#### 个人身份证类

- `idCard` / `idCardNumber` / `idNumber` / `nationalId`
- `documentNumber` / `realName`
  - 2026-07-04 注记(pre-go-live readiness review v0.35.0 §4 F-1 true-up):`MemberProfile.documentNumber`("MP-5 高敏感")/ `MemberProfile.realName`("MP-1")/ `RecruitmentApplication.realName` 均为真实落表字段。`logger-options.ts` 已补录 `*.documentNumber` / `*.realName`(#499);本节此前漏列,本次补齐。
...
- App 位置上报入参:`longitude` / `latitude`
- 活动打卡证据字段:`checkInLongitude` / `checkInLatitude` /
  `checkOutLongitude` / `checkOutLatitude`
- `Activity.locationLongitude` / `Activity.locationLatitude` 表达公开活动场地坐标，
  不等同于成员位置轨迹；本类不把二者混同
...
- `sex` / `nation` / `birth` / `authority` / `validDate`
  - 2026-07-04 注记(pre-go-live readiness review v0.35.0 §4 F-1 true-up):`RecruitmentOcrDetailDto` / `RealnameOcrExtendedFields` 同构兄弟字段(OCR 鉴伪版识别中间结果,顾问式回显,`sex`/`birth` 不覆盖 `genderCode` / `birthDate` 推导权威);同组 `address` 字段已被上方"地址类"`*.address` 覆盖,此处补齐其余 5 个同级字段。`birth` 是 OCR 识别中间字段,与上方"出生 / 身份信息类"的 `birthDate`(落表出生日期,推导权威)是不同字段,并列列出、不可混同。`logger-options.ts` 已补录 `*.sex` / `*.nation` / `*.birth` / `*.authority` / `*.validDate`(#499);本节此前漏列,本次补齐。

#### 第三方账号 / 凭证标识类

- `wechat` / `wechatId` / `openId` / `unionId`
  - 2026-06-12 注记(微信小程序登录 T1 落表;增量审计⑧):实际落表拼写为全小写 `User.openid`(D-W2),与本节预留拼写 `openId` 大小写不同;redact 路径匹配大小写敏感,`logger-options.ts` 已按 §8.4 同批次义务补录 `*.openid`(`openId` 预留拼写留置,字段未落表时无害)。`unionId` 未落表(D-W2 不存),届时按实际拼写同步本节与清单。
- `certificateNo` / `licenseNo` / `policyNo`(证书号 / 执照号 / 保单号)
  - 2026-07-04 注记(pre-go-live readiness review v0.35.0 §4 F-1 true-up):实际落表拼写为 `certificates.certNumber` / `insurances.policyNumber`,与本行预留拼写 `certificateNo` / `policyNo` 不同;redact 路径拼写敏感,`logger-options.ts` 已按 §8.4 同批次义务补录 `*.certNumber` / `*.policyNumber`(#499)。`certificateNo` / `policyNo` 预留拼写按 §8.4"删除保留"防御性留置不删;`licenseNo` 尚未落表,届时按实际拼写同步。
...
| v0.10 | 2026-07-19 | **§1.1 audit_logs 当前事实 true-up**(C-2 敏感读取统一落库,用户明确授权红区):保留 D6 v1.1 Q1=A/两 union 为历史起点,登记 Q1=B 已拍板、`auditPlaceholder` 退役、`AuditLogEvent` 122 项，以及普通读取/CSV/sign URL 的 fail-closed 顺序；A-17 失败操作不审计与 A-18 `audit_logs` 自身不审计继续有效。零 schema/migration/endpoint/DTO/permission/BizCode。 |
