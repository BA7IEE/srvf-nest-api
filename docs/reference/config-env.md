...
- `ACTIVITY_OS_CONTROL_PLANE_MODE` 归 `app.config.ts` 的 `activityOsControlPlane.mode`，严格取 `off|shadow|active`。development/test 缺省、空值或空白默认 `off`；production/smoke 必须显式填写，且 `active` 要求 v1.1 Gate 已为真（仅启动联锁，不自动开闸）。唯一运行时消费者 `ActivityControlPlaneGate` 只管 B6 三种 App 草稿创建及其只读状态；不影响旧泛化创建、发布或账本。`shadow` 是前端试运行展示范围，不是服务端 cohort 授权；生产模式切换与 cutover 稳定观察另行审批。
