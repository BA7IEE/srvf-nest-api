...
DELETE /api/v2/dict-types/:id
  ...
  错误码:DICT_TYPE_NOT_FOUND / DICT_TYPE_SYSTEM_PROTECTED(系统内置类型拒删)/ DICT_TYPE_IN_USE(若有 dict_items 引用 / 业务表引用)
```
...
DELETE /api/v2/dict-items/:id
  ...
  错误码:DICT_ITEM_NOT_FOUND / DICT_ITEM_SYSTEM_PROTECTED(闭集 / 国标 / 队内内置类型下的项拒删)/ DICT_ITEM_IN_USE(若有业务表引用此 code)
...
| `DICT_TYPE_SYSTEM_PROTECTED` | 120xx | 系统内置字典类型不允许删除 | 409 |
...
| `DICT_ITEM_SYSTEM_PROTECTED` | 120xx | 系统内置字典项不允许删除 | 409 |
...
- Step 2 seed 追加字典(R13 收窄后非敏感分类内置真实值;node_type 已内置真实分类〔2026-06-21 组织树 goal〕,work_nature 仍占位);**不**修改 SUPER_ADMIN 创建逻辑
...
- ❌ 真实成员 PII(姓名 / 身份证 / 手机号)+ 真实 memberNo 规则 / 样例进 git history(R13;2026-06-21 收窄,非敏感分类字典取值已可内置,见 `V2红线与复活路径.md` A-9)
- ❌ 跳过 OpenAPI 契约快照验证
