...
- **第一版前端 BizCode 翻译表唯一权威源**:覆盖 `src/common/exceptions/biz-code.constant.ts` 中**全部 124 条** BizCode(P0-D PR-3 #117 新增 `OLD_PASSWORD_INVALID=10005` / `NEW_PASSWORD_SAME_AS_OLD=10006` 后实数;P0-G 撰写时为 122 条)
...
P0-G 撰写时(基于 `main` HEAD `a240e0a`)`biz-code.constant.ts` 实数为 **122 条**;P0-D PR-3(#117,`main` HEAD `8a70573`)新增 `OLD_PASSWORD_INVALID=10005` / `NEW_PASSWORD_SAME_AS_OLD=10006` 后实数为 **124 条**(精确点数命令 `grep -cE "^  [A-Z][A-Z0-9_]+: \{" src/common/exceptions/biz-code.constant.ts`)。本文 §4 全量翻译表对应 124 条;若实数与本文记数不一致,以源码为准。
...
> 本节按段位顺序列出全部 **124 条** BizCode(2026-05-17 `main` HEAD `8a70573` 实数;P0-G 撰写时为 122 条 + P0-D PR-3 #117 新增 10005 / 10006)。
>
> 表列说明:
...
理论上不应该出现 §4 未列出的 code(已覆盖 124 条全部);但仍有几种边界场景:
