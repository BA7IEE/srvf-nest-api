...
**Current status**:
- **No dedicated `*effect.ts` class is required yet.**
- `eventPlaceholder('attendance.recorded')` remains a placeholder inside the current attendance flow(沿 [`docs/participation-bounded-context.md §4`](participation-bounded-context.md) finalApprove step 9 描述)。
- **Do not** introduce an Effect class until a real side-effect path exists(短信 / 推送 / 跨系统集成等)。
...
| Presenter | none required yet | **deferred** |
| QueryService | none required yet | **deferred** |
| Effect | none required yet | **deferred**(`eventPlaceholder` 占位) |
