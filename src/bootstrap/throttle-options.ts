...
//   - 两个 throttler 实例**物理隔离**:登录失败爆破不会消耗改密配额,反之亦然
//   - ThrottlerBizGuard.shouldSkip 默认 true:仅 @LoginThrottle() / @PasswordChangeThrottle()
//     标注的方法才走对应 throttler 的 limit/ttl 检查;按 metadata 决定走哪个 throttler
//   - setHeaders: false 完全关闭 X-RateLimit-* / Retry-After 头(任务卡 15.7 / 评审稿 §5.4)
