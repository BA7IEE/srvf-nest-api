...
export const BizCode = {
  ...
  // 只读维护态(合同 §16.4「gate 切为拒绝新写」)。与上面两个码是**第三个方向**:
  // 那两个各自表示「你走错轨了,请改走另一条」,本码表示「两条轨今天都不收写入」。
  //
  // 🔴 必须是**具名码**而不是让写路径抛 500:合同 §16.4 第 3 条要求维护态下
  //    「保留查询、任务状态、导出和审计」—— 客户端要能分辨「本实例在维护」与「服务坏了」,
  //    前者该提示稍后再试并保持读面可用,后者该报错。
  //
  // 503 而非 410:与 ACTIVITY_V11_WORKFLOW_NOT_ENABLED 同族 ——「稍后重试」,
  // 因为维护态按定义是**临时**的(合同 §16.4 第 5 条:修复版本沿相同新模型继续)。
  ACTIVITY_WORKFLOW_READONLY_MAINTENANCE: {
    code: 20158,
    message: '本实例处于活动结算只读维护态,暂不接受考勤与结算写入',
    httpStatus: HttpStatus.SERVICE_UNAVAILABLE,
  },
  ...
  ACTIVITY_TIME_POLICY_INVALID: {
    code: 20197,
    message: '时长政策请求或定义无效',
    httpStatus: HttpStatus.BAD_REQUEST,
  },
  ACTIVITY_TIME_POLICY_NOT_FOUND: {
    code: 20198,
    message: '时长政策或版本不存在',
    httpStatus: HttpStatus.NOT_FOUND,
  },
  ACTIVITY_TIME_POLICY_CODE_EXISTS: {
    code: 20199,
    message: '时长政策代码已存在',
    httpStatus: HttpStatus.CONFLICT,
  },
  ACTIVITY_TIME_POLICY_STALE: {
    code: 20023,
    message: '时长政策版本已变化，请刷新后重试',
    httpStatus: HttpStatus.CONFLICT,
  },
  ACTIVITY_TIME_POLICY_STATUS_INVALID: {
    code: 20024,
    message: '时长政策版本状态不允许此操作',
    httpStatus: HttpStatus.CONFLICT,
  },
  ACTIVITY_TIME_POLICY_COMMAND_CONFLICT: {
    code: 20025,
    message: '此操作标识已用于其他时长政策请求',
    httpStatus: HttpStatus.CONFLICT,
  },
  ACTIVITY_TIME_POLICY_RECEIPT_INVALID: {
    code: 20026,
    message: '时长政策收据或版本锚校验失败',
    httpStatus: HttpStatus.CONFLICT,
  },
  ACTIVITY_TIME_POLICY_VERSION_LIMIT: {
    code: 20027,
    message: '时长政策版本号已达上限',
    httpStatus: HttpStatus.CONFLICT,
  },
