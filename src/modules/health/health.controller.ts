...
// 健康检查分层(详见 ARCHITECTURE.md §11.1 + docs/deployment.md「健康检查分层」)
//
// 三个端点全部 @Public(),全部走 ResponseInterceptor 包装,响应外层始终是
//   { code: 0, message: 'ok', data: { status: 'ok', ... } }
// 详见 docs/reference/response-pagination-errors.md §4 + ARCHITECTURE.md §11.1。
//
// 设计选择(15.5 与用户决策):
...
//   docs/reference/response-pagination-errors.md §5 规定
//   "HTTP status 由 BizCode 的 httpStatus 决定",而
//   BizCode.INTERNAL_ERROR.httpStatus = 500。历史 V1.1 资料中的 "503"
//   不作当前权威 → 走 HTTP 500。后续若需要标准 503,应单独
...
//   成功则返回 { status, info, error, details }。
