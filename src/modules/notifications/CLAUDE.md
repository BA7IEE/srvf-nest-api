...
**定向通知 replay(F2 / SF2)**:`NotificationOutboxService.replayDirectedWecomDelivery(notificationId)` 是系统定向通知**唯一**的重发入口(admin 广播继续走 unpublish + publish)。它建**新 child id + 新 eventKey**(v1 定向键允许追加 `:r{n}` nonce),已 SENT 者返 `already-sent`、在途 attempt 返 `active-attempt-exists`。⚠️ **暂态失败的 delivery 行绝不能占 `intent.id`** —— 占了就会被 `deliverWecom` 开头的幂等判据短路,"重试 8 次"退化成"第一次网络抖动即永久放弃"且现场看起来一切正常。上线/回滚见 [`runbook`](../../../docs/ops/wecom-message-channel-rollout.md);⚠️ 旧 worker 不认识 `notification.wecom-*` 会误判 terminal dead,开 `messageEnabled` 前必须确认 fleet 只剩新版本。
