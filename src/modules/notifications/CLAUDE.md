...
- **统一通知 S1 站内信渠道**(2026-06-25):admin 撰写/发布面(`NotificationAdminController` 8 端点)+ 会员 app 拉取面(`NotificationAppController` 4 端点);`Notification` 广播 + `NotificationRead` 已读;**站内 = pull 零发送**;可见性**复用 `content.visibility`**(去 public = 4 档)。`formal_member` 由 ACTIVE Member + 共享 `isFormalMemberGradeCode()` 决定，`activeOrgIds` 只服务 department；management 仅认 SUPER_ADMIN 或当前有效 GLOBAL `notification.read.record`，Role.ADMIN 不天然放行；feed/detail/mark-read/unread 与 SMS/WeChat 广播共用该真值，directed 仍只认收件人本人。
...
- 报名审批、候补递补与自助取消 producer 同样在业务 transaction 内 enqueue；同一状态版本复用稳定 eventKey，reopen 后新审核生成新 eventKey。gate=true 的自助取消缺 ACTIVE owner 时不入队且只记安全告警，禁止回退 publisher。
...
- 考勤 `firstReturn`/`finalReturn`/`finalApprove` 同样在 Sheet 状态、audit 的业务 transaction 内 enqueue；退回收件人从 active attendance assignment/提交人解析，终审逐 record snapshot，enqueue 失败整体回滚，worker/provider 失败不得重放业务写。
