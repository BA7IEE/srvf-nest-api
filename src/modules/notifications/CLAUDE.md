...
- **企业微信应用消息(T5B,2026-08-02;第四条渠道,默认关)**:`wecom` ≠ `wechat` —— 前者是企业微信应用消息(身份键 `corpId + wecomUserId`),后者专指微信小程序订阅消息(`openid`)。**两条链全程分家**:独立 outbox 事件(`notification.wecom-broadcast` / `notification.wecom-delivery`)、独立 active-slot partial unique(第 69 migration,谓词按 `eventType` 分域 —— 合并索引会让同一通知同一人无法同时收两个渠道)、独立 reasonCode 组、独立 errcode 段(360xx vs 25xxx)。受众判定**消费 T5A 两入口**,零第二份可见性口径。**两层开关判据缺一不可**:`NotificationService.transition` 在 publish 前问 `resolveChannelReadiness()`(关着则不建 root intent),`NotificationWecomDispatchService.authorizeDurableRecipient` 在 Provider 前**用 `FOR SHARE` 锁后复读** `wecom_settings`(root 之后被关掉 ⇒ 终态 `skipped/channel-disabled`,**不迟到补发**)。最终闸锁序在 T5A 之后**追加两段**:`wecom_identities`(FOR SHARE)→ `wecom_settings`(FOR SHARE),放尾部是因为 settings 写路径只锁自己那一行、不可能与本链成环。`wecomUserId` **只从最终闸的事务内快照产生**,不入 payload、不入日志、不入 Audit;`NotificationDelivery.recipientRef` 只存掩码。回执分类:`errcode=0` **仍必须**逐条查 `invaliduser`/`unlicenseduser`,81013 一律不记 SENT;45009 与 invalidparty/invalidtag **直接 dead 等人工 replay**(重试只会延长官方拦截),其余暂态走既有退避。⚠️ **暂态失败的 delivery 行绝不能占 `intent.id`** —— 占了就会被 `deliverWecom` 开头的幂等判据短路,"重试 8 次"退化成"第一次网络抖动即永久放弃"且现场看起来一切正常。上线/回滚见 [`runbook`](../../../docs/ops/wecom-message-channel-rollout.md);⚠️ 旧 worker 不认识 `notification.wecom-*` 会误判 terminal dead,开 `messageEnabled` 前必须确认 fleet 只剩新版本。
