...
describe('RecruitmentPromotionService · promote 超时硬化(bcrypt 移出事务 + 显式 timeout)', () => {
  ...
  it('bcrypt 在事务之外预算 n 个、事务回调内零 bcrypt;$transaction 传显式 timeout', async () => {
    ...
    // 招新域的 mainland_id 不能直接泄漏到 MemberProfile；建档边界写字典真值 id_card。
    const profileCalls = txMock.memberProfile.create.mock.calls as Array<
      [{ data: { documentTypeCode: string } }]
    >;
    expect(profileCalls).toHaveLength(n);
    expect(profileCalls.map(([{ data }]) => data.documentTypeCode)).toEqual(
      Array<string>(n).fill('id_card'),
    );
  });
