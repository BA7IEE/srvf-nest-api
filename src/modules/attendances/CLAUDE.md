...
- **第 6 批离线模型边界**：migration 88 已建立 `OfflinePackage`、`OfflinePackageParticipant`、`OfflinePunchReviewItem` 和 PunchEvent 离线锚/数据库闭集；它们不是可自行调用的 production path。当前补充合同尚未给出 package issue/revoke/upload/review 的精确 HTTP wire，故不得以模型存在为由私造 controller、DTO 或第二套 PunchEvent writer。
