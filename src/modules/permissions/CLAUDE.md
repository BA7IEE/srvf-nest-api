...
- **控制面授码 + 内置角色删保护**:role-permissions assign 对非 SUPER_ADMIN 禁止分配上述任一控制面码,整批返 `30103`;[`protected-role-codes.ts`](protected-role-codes.ts) 是 API 删除保护的 15 角色唯一清单（9 个既有角色 + v0.61.0 六个活动责任角色），任何身份含 SUPER_ADMIN 删除均返 `30104`,自定义角色删除逻辑不变。seed-rbac e2e 是 protected/system-managed/固定权限映射的漂移哨兵;**禁止**在 service / 测试 / 文档另造代码清单
