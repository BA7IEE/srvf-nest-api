...
- **控制面授码 + 内置角色删保护**:role-permissions 的 **assign 与 revoke 对称** —— 非 SUPER_ADMIN 既不得分配、也不得撤销上述任一控制面码,两侧统一返 `30103`,共用同一个 `assertNoControlPlaneCodesOrThrow()`,**禁止只给一侧挂闸**(E-B2:revoke 曾一个控制面判定都没有,ops-admin 授不了却撤得掉,可把某角色的 `rbac.*` 撤空)。assign 收 codes,故能在 Permission 存在性查询前拦(未 seed 的保留码也返 `30103`);revoke 收 permissionId,必须先查库取 code 再判 —— 这是签名决定的唯一次序差,不是漏拦。**新增任何会改写角色权限映射的公开方法(如 RBAC 终态方案 PR 4 的原子 `PUT`)必须同样挂闸**,由 [`role-permissions-control-plane-gate.spec.ts`](role-permissions-control-plane-gate.spec.ts) 强制:该判据按 AST **动态现取**本类所有会写 `rolePermission` 的公开方法(含把写操作藏进私有 helper 的形态),逐个要求能到达控制面谓词,漏一个即红并点名;**刻意不写死方法名** —— 写死名单时新方法与它漏掉的闸会一起不在名单里,判据当场变摆设且全绿。整批返 `30103`;[`protected-role-codes.ts`](protected-role-codes.ts) 是 API 删除保护的 15 角色唯一清单（9 个既有角色 + v0.61.0 六个活动责任角色），任何身份含 SUPER_ADMIN 删除均返 `30104`,自定义角色删除逻辑不变。seed-rbac e2e 是 protected/system-managed/固定权限映射的漂移哨兵;**禁止**在 service / 测试 / 文档另造代码清单
...
- **错误码段位**:`PERMISSION_NOT_FOUND/30001` / `RBAC_ROLE_NOT_FOUND/30003` / `INVALID_PERMISSION_CODE_FORMAT/30008` / `PERMISSION_CODE_ALREADY_EXISTS/30009` / `LAST_OPS_ADMIN_PROTECTED/30101`(含 role-binding/user-role 撤权及禁用/软删最后 ops-admin 持有人) / `CANNOT_ASSIGN_HIGHER_ROLE/30102` / `PERMISSION_RESERVED_SUPER_ADMIN_ONLY/30103` / `PROTECTED_ROLE_DELETE_FORBIDDEN/30104` / `SEED_PERMISSION_DELETE_FORBIDDEN/30105` / `PERMISSION_CODE_NOT_IN_SEED_CATALOG/30106`;`RBAC_FORBIDDEN=30100` 是判权失败统一码
- **Permission 物理删**(D4 v1.0);`RolePermission` FK Cascade 自动联级清理
- **🔴 权限目录护栏(P1-32 PR1,2026-08-22)**:`Permission` 三条写路径按 [`seed-permission-codes.ts`](seed-permission-codes.ts) 这**一个**谓词分流 —— 闭包内的码**删返 `30105`**(任何身份含 SUPER_ADMIN)、闭包外的码**造返 `30106`**;`PATCH` 仍只写 `description`(既有 DTO 白名单,本次只补回归锁,**不得**顺手放开 module/action/resourceType)。立项事实:`RolePermission.permission` 是 `onDelete: Cascade`,删一个码 = 一次性撤销所有角色对它的授权 —— 实测删 `member.read.record` 打掉 4 个角色(`role_permissions` 337→333),重跑 seed 只补得回内置角色那部分,**自定义角色那条永久丢失**。⚠️ 三条路径**必须共用同一个谓词**,各判各的会出现「删挡住了、造没挡住」这种半边闸;⚠️ 护栏**只**管闭包内的码,闭包外的历史惰性码删除行为一字不变(顺手全禁会把唯一清理入口也堵死);⚠️ 结果上 `POST /permissions` 不再存在成功路径(闭包内已存在→`30002`,闭包外→`30106`),这是**刻意的** —— 权限码只能改代码并发版
