...
- **闭包清单为什么在 `src/` 又存了一份**:`src/` 够不到 seed 事实闭包(`prisma/seed.ts` 不进 `dist/`,且它自己 import [`rbac-seed-facts.ts`](rbac-seed-facts.ts) ⇒ 反向 import 是循环)。[`seed-permission-codes.ts`](seed-permission-codes.ts) 是**唯一**允许的那份副本,与 [`protected-role-codes.ts`](protected-role-codes.ts) 同范式,靠三段链钉死:清单 == 桶并集 == typed-AST 全集([`seed-permission-codes.spec.ts`](seed-permission-codes.spec.ts),薄跑红区判据 `scripts/check-permission-catalog-closure.ts`)、清单 == seed 后的 DB 行(`test/e2e/seed-permission-catalog-runtime.e2e-spec.ts`,**连 DB**;源码侧闭包判据对运行时漂移结构性失明)。**禁止**在 service / 测试 / 文档再造第三份权限码清单
