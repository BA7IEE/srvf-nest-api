...
// 沿 D7 v1.1 §5.2.3 + 用户拍板。
//
// **入参字段**:沿 D7 §5.2.3 锁定 `permissionCodes: string[]`(权限点 code 字符串数组,
// 不用 permissionIds);Service 内部用 code 查 permission.id 后批量写入。
//
// ⚠️ **P1-32 PR 8(2026-08-24)删掉了两个入参 DTO**:`AssignRolePermissionsDto`
//    (旧 `POST` 批量增量授权)与 `RevokeRolePermissionParamDto`(旧 `DELETE` 精确撤权)——
//    两条端点已退役,写入参只剩下面这一个 `ReplaceRolePermissionsDto`
//    (`PUT` 与 `POST /preview` **共用同一个类**,见本文件下方 4b 段的说明)。

// P1-32 PR 4a(2026-08-23):整集替换入参。
//
// **与已退役的 `AssignRolePermissionsDto` 的两处刻意差异**(留档,别再造回增量入参):
//   ① `@ArrayMinSize` **没有** —— 空数组是合法目标集(= 清空该角色的全部权限点)。
//      整集替换若不许传空,「把权限收干净」就只能靠逐条 DELETE,那正是 4a 要收掉的多写入口。
...
// ⚠️ `RevokeRolePermissionParamDto`(旧 `DELETE /:permissionId` 的双 cuid 路径校验)
//    随 P1-32 PR 8 一并删除 —— 本 controller 现在只剩 `:id` 一个路径参数,直接用 `IdParamDto`。
