...
describe('C1 D2b template catalogue query scope and pagination', () => {
  ...
  it('omitted schema filter includes V1–V5 and still excludes legacy/non-global/inactive Families', async () => {
  ...
  it('detail lookup applies the same Family and V1–V5 schema boundary after explicit read authorization', async () => {
    const { service, tx, client, assertAccess } = setup();
    ...
    expect(tx.activityTemplate.findFirst).toHaveBeenCalledWith({
      where: { id: 'missing', family: visibleFamily, schemaVersion: { in: [1, 2, 3, 4, 5] } },
      include: { family: true },
    });
    expect(assertAccess.mock.invocationCallOrder[0]).toBeLessThan(
