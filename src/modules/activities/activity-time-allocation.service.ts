...
import {
  ...
  type ActivityTimeAllocationSettlementProof,
...
export class ActivityTimeAllocationService {
    ...
    try {
      ...
      return await this.prisma.$transaction(
        async (tx) => {
          ...
          return await this.recognizeSourceInTx(
            tx,
            activityId,
            command,
            meta,
  ...
  /**
   * Explicit D4 bridge only. Caller holds actor/operation, Activity and Run locks, and resolves
   * proof after checking the current sealed draft. SQL repeats proof validation. No nested tx.
   * Replays deliberately precede draft-proof resolution, but never current authorization.
   */
  async recognizeSealedDraftInTx(
    ...
    resolveProof: () => Promise<ActivityTimeAllocationSettlementProof>,
  ): Promise<ActivityTimeAllocationReceiptResult> {
    let actor = await reauthorize();
    const receiptIdentity = {
    ...
    if (receipt) {
      if (receipt.activityId !== activityId || receipt.requestHash !== requestHash)
        throw new BizException(BizCode.ACTIVITY_TIME_ALLOCATION_COMMAND_CONFLICT);
      if (
        receipt.allocationRevision.id !== receipt.allocationRevisionId ||
        receipt.allocationRevision.activityId !== activityId ||
        receipt.allocationRevision.createdByUserId !== actor.id ||
        !receipt.allocationRevision.settlementDraftVersionId
      )
        throw new BizException(BizCode.ACTIVITY_TIME_ALLOCATION_INVALID);
      actor = await reauthorize();
      return parseActivityTimeAllocationReceipt(
        receipt.resultJson,
        activityId,
        receipt.allocationRevisionId,
  ...
  private async recognizeSourceInTx(
    tx: Prisma.TransactionClient,
    activityId: string,
    command: ActivityTimeAllocationCommand,
    meta: AuditMeta,
    requestHash: string,
    receiptIdentity: { actorUserId: string; operationCode: string; operationKey: string },
    reauthorize: () => Promise<void>,
    actor: () => CurrentUserPayload,
    proof?: ActivityTimeAllocationSettlementProof,
  ): Promise<ActivityTimeAllocationReceiptResult> {
    const currentSegments = await this.segments.readActivityCurrentSegmentsTrusted(tx, activityId);
    const source = currentSegments.find((segment) => segment.id === command.sourceSegmentId);
    if (
      !source ||
      source.statusCode !== (proof ? 'draft' : 'committed') ||
    ...
    let pointer: { policyId: string; versionId: string; definitionHash: string } | null = null;
    try {
      ...
      if (activityTimePolicySelectionHash(document) !== selection.selectionHash)
        return policyUnavailable();
      const targets = historicalTimePolicySelectionTargetsFromRuleSnapshot(snapshot.resolvedConfig);
      ...
      if (error instanceof TypeError || error instanceof BizException) return policyUnavailable();
      throw error;
    }
    ...
    await this.audit.log(
      ...
      meta,
      result,
      proof ? 'recognize_settlement_time_allocation' : undefined,
    );
    return result;
  }
}
