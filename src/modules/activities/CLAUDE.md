...
第 116 条 migration、隔离库 116 条冷回放、115→116 非空升级，以及 C3 专属 21 条 HTTP／数据库、9 条锁等待、20 条迁移约束用例已在本分支的 app_test_w98 通过。
真实更正 prepare／失败回滚／commit、现场提前离场及现场作废的 Activity 锁交错已有验证；离线 package／review、结算重投影和其余 writer 的并发覆盖、PR CI、可信审批、合并、生产 deploy 与整体跨模型复审仍未发生。
