...
- `InsuranceEligibilityEvidence` 的 PR4 约束与第 82 migration 的 revision 兼容地基已交付，但尚未生产 deploy：保留 `ownerKind=activity_registration` 与 header FK，nullable `activityRegistrationRevisionId` 通过 `(activityRegistrationId,activityRegistrationRevisionId)` 复合 FK 保证同头；旧 header-only evidence 零回填，legacy/revision 分别唯一。producer/approval 本刀零行为变化，仍按 header 写读；runtime C 刀才切当前 revision。DB 终态为 8 CHECK、3 个 owner partial unique、既有 member-match 与 immutable 双 trigger。
