...
- `InsuranceEligibilityEvidence` 的 PR4 约束与第 82 migration 的 revision 兼容地基已交付，但尚未生产 deploy：保留 `ownerKind=activity_registration` 与 header FK，nullable `activityRegistrationRevisionId` 通过 `(activityRegistrationId,activityRegistrationRevisionId)` 复合 FK 保证同头；旧 header-only evidence 零回填，legacy/revision 分别唯一。producer/approval 本刀零行为变化，仍按 header 写读；上线须先 drain 独立 deploy D82，再由 runtime C 整齐切换，禁新旧版本混跑。若 revision writer 与旧 approval 并存，后者按 header 读到多份 evidence 会 fail-closed 26030，必须由 C 刀切换处理，不得 fallback。DB 终态为 8 CHECK、3 个 owner partial unique、既有 member-match 与 immutable 双 trigger。
...
- 保险生命周期 PR-A 复用同一 gate：Activity update 仅在受保护字段真实变化时查 live 非 cancelled 报名并复用 20030；approve 的保险 exact-source 重验仍只在 gate=true + Activity.requiresInsurance=true 启用。PR-E 起所有 approve（含 gate=false/无需保险）都在 claim Registration 前锁并重读 Member；不存在/软删→15001，inactive→17030。insured self 锁序为 Activity→Member lifecycle→MemberInsurance，team 为 Activity→Policy→Coverage→Member lifecycle；只有 evidence/source 失配仍统一 26030，禁止 fallback。
