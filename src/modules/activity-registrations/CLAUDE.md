...
- `activity-registrations.service.ts` **1454L**(god-service,沿 CODEMAP);`activity-registration-state-machine.ts`(105L)/ `activity-registration-audit-recorder.ts`(198L)/ `app-my-registrations.service.ts`(290L)已抽离；候补另以纯函数 promotion engine + QueryService 隔离
...
- Capacity:create 在全部既有闸通过后按 passCount/capacity 分流 pending|waitlisted；`assertCapacityNotExceeded` 仅保留于 approve，其 Activity `FOR UPDATE` + count 块不动。取消 pass 递补 1 人；capacity 调大递补 delta，改 null 递补全部，缩容不递补
...
- **候补并发锁序**:`promoteActivityWaitlist` 由调用方透传 tx，固定锁 Activity，按 `registeredAt ASC,id ASC` 逐行 claim CAS。pass 取消在 claim registration 前先锁 Activity，与 approve / check-in 统一 Activity → Registration 锁序，禁止恢复会触发 PostgreSQL 40P01 的反向锁序
- **候补排位**:`activity-registration-waitlist-query.service.ts` 批量按活动计算，列表禁止 N+1；App 列表/详情与 Admin 列表非 waitlisted 返 null
...
- ❌ **不**主动拆 `activity-registrations.service.ts`(1454L,沿 [`/docs/current-state.md §4 P2`](../../../docs/current-state.md))
