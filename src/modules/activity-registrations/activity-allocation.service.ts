...
import {
  assertProjectedReservationsExact,
  firstComeWaitlistRank,
  lockApplicationProjections,
  lockBatchWaitlistHead,
  lockFirstComeWaitlistHead,
  readReservationAnchors,
} from './activity-allocation-locks';
import {
  asObject,
  assertPendingSource,
  assertPreparingCandidates,
  assertVoidLiveFacts,
  initialPreferencePositions,
  decimalString,
  readReceiptBatchStatusCode,
  targetProjection,
...
export class ActivityAllocationService {
  ...
  async applyFirstComeAfterSubmissionInTransactionTrusted(
    ...
    for (const source of sources.sort(
      ...
      const waitlistRank = allocated
        ? null
        : await firstComeWaitlistRank(tx, {
            activityId: input.activity.id,
            sessionId: source.sessionId,
  ...
  async promoteAfterCancellationInTransactionTrusted(
    ...
    for (const slot of allocationSlots.sort(
      ...
      const candidate =
        slot.allocationBatchId === null
          ? modeCode === 'first_come'
            ? await lockFirstComeWaitlistHead(tx, input.activityId, slot)
            : null
      ...
      const anchors = await readReservationAnchors(
        tx,
  ...
  async commit(
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        assertPreparingCandidates(batch, candidates);
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (batch.statusCode === 'committed') {
          const projections = await lockApplicationProjections(tx, batch.id);
          const sources = await this.lockCandidateSources(
            tx,
            activityId,
            batch.sessionId,
          ...
          assertVoidLiveFacts({ batch, candidates, projections, sources });
          await assertProjectedReservationsExact(tx, activityId, projections);
  ...
  private async reserveInitialPreference(
    ...
    const positions = initialPreferencePositions(input.preferenceSnapshot, input.batchPositionId);
  ...
  private async freezeQualification(input: {
    ...
    const projection = targetProjection(evaluation, input.sessionId, input.positionId);
  ...
  private async findReplay(
    ...
    const receiptBatchStatusCode = readReceiptBatchStatusCode({
      activityId,
