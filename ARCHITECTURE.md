...
- **登录限流**:`@nestjs/throttler` 内存 storage;仅作用于 `POST /api/auth/v1/login`,默认 `5 次 / 60 秒 / per IP`;超限抛 `BizException(BizCode.TOO_MANY_REQUESTS)`(`code: 42900` / HTTP 429);**不暴露**阈值数字、剩余配额、重置时间到 message
