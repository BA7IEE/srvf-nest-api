...
| 限流 | **@nestjs/throttler** | PostgreSQL shared storage | 10 个命名 throttler 以 `(throttlerName,key)` 物理隔离,多实例共享配额,DB 异常 fail-closed |
| 健康检查 | **@nestjs/terminus** | — | `/api/system/v1/health` / `/live` / `/ready` 三端点(详 §11) |
...
- **登录限流**:`@nestjs/throttler` PostgreSQL shared storage(10 个命名实例以 `(throttlerName,key)` 隔离,多实例共享配额,DB 异常 fail-closed);登录实例仅作用于 `POST /api/auth/v1/login`,默认 `5 次 / 60 秒 / per IP`;超限抛 `BizException(BizCode.TOO_MANY_REQUESTS)`(`code: 42900` / HTTP 429);**不暴露**阈值数字、剩余配额、重置时间到 message 或响应头
- **健康检查分层**:`@nestjs/terminus`;`/api/system/v1/health/live`(进程存活)+ `/api/system/v1/health/ready`(DB 连通)+ `/api/system/v1/health`(根端点,等同 `/live`);三者都 `@Public()`,都走统一响应包装
