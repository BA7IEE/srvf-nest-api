...
| `authz/` | S | **统一判权大脑**(终态 scoped-authz PR8,2026-07-02):AuthzService(三源 grant 推导 + covers + ActionConstraint〔同人开关 PR9 起经 ctx 从 app.config env 注入〕)+ ResourceResolver(11 类归属解析,fail-close);0 controller / 0 端点 | 🔴 **PR9(2026-07-02)起首个消费者 = attendances 终审两方法**(自审/同人约束现网生效),其余业务面判权仍走 rbac.can(逐面迁移 PR12);无 ref 判权逐字等价 rbac.can(等价矩阵 e2e 行为锁);R5 副职零推导 / BD-2 不 hardcode 部门(`bd2-department-literal-gate.spec` CI 闸) | [`CLAUDE.md`](src/modules/authz/CLAUDE.md) · [`docs/reviews/org-position-scoped-authz-terminal-design-review.md §5`](docs/reviews/org-position-scoped-authz-terminal-design-review.md) |
