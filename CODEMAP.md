...
| `seed.ts` | 默认 super admin + bootstrap 授予(PR6 起写 global RoleBinding)+ 内置 6 角色(ops-admin/member/biz-admin + PR7 org-admin/group-manager/org-supervisor〔后 3 者纯定义零持有,PR8 才推导〕)+ 3 条职务→角色默认映射(仅正职;🔴 R5 副职零行 + 运行时断言) | 生产环境强校验启动(`SUPER_ADMIN_*` / `JWT_SECRET` / `APP_CORS_ORIGIN`) | [`docs/deployment.md`](docs/deployment.md) |
