...
| `authz/` | S | **统一判权大脑**(终态 scoped-authz PR8,2026-07-02):AuthzService(三源 grant 推导 + covers + ActionConstraint〔同人开关 PR9 起经 ctx 从 app.config env 注入〕)+ ResourceResolver(11 类归属解析,fail-close);**PR10(2026-07-02)+ explain 端点** = 模块第一个 controller(`AuthzController`,`POST admin/v1/authz/explain` 1 端点 + 1 码 `authz.explain.decision` 绑 ops-admin;薄编排 `AuthzExplainService`:调用者 rbac.can → 目标用户加载〔不存在/软删 10001〕→ 委托 AuthzService.explain 原样返;**deny 是 200 数据非错误**〔resource_not_found 亦 decision reason〕;reason 11 值枚举入 OpenAPI 契约锁;无 audit) | 🔴 **PR9(2026-07-02)起首个消费者 = attendances 终审两方法**(自审/同人约束现网生效),其余业务面判权仍走 rbac.can(逐面迁移 PR12);无 ref 判权逐字等价 rbac.can(等价矩阵 e2e 行为锁);R5 副职零推导 / BD-2 不 hardcode 部门(`bd2-department-literal-gate.spec` CI 闸);explain 是只读诊断出口,**不改判权语义** | [`CLAUDE.md`](src/modules/authz/CLAUDE.md) · [`docs/reviews/org-position-scoped-authz-terminal-design-review.md §5`](docs/reviews/org-position-scoped-authz-terminal-design-review.md) |
