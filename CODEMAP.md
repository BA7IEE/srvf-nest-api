...
| `team-join/` | M (~1039L,T2) | 招新三期(入队:志愿者→队员;T2):入队轮 CRUD + 标 gate(8 通用 + 4 条件性专业队;完成日→有效期;dept-assessment 可延长期)+ 综合评估单一人工闸 + 贡献值只读汇总(approved sheet,checkInAt<入队年 3-31)+ 状态机(joining→pending_evaluation→approved→joined/rejected);**两层身份**:入队才赋部门 + 级别 level-1 | gate 全过 + 贡献值≥5 自动推进;专业队 = node_type code 约定识别;贡献值直读 attendance_records 只读汇总 Decimal 精度;app 自助面(发起/查进度)T3、一键入队(enrollment 单事务)T4 | [`docs/archive/reviews/recruitment-phase3-review.md`](docs/archive/reviews/recruitment-phase3-review.md)(冻结评审稿) |
