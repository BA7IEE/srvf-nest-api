...
>
> 本节与 [`AGENTS.md`](AGENTS.md) 同步;冲突按 `ARCHITECTURE.md` > `CLAUDE.md` / `AGENTS.md` > 本节 顺序处理。
...
> **SRVF v0.13.0 注**:此处"refresh token 在 V1.1 阶段仍然不做"是 V1.1 历史段陈述。SRVF 已进入 P0-* 阶段,refresh token / logout / logout-all 沿 P0-E 评审稿 v1 在**独立 P0-E 通道**解锁(铁律见 §9 P0-E 子节),**不通过** V1.1 工程加固通道实现;两者通道边界明确,不冲突。
...
- 禁止安装新依赖(包括看似"早晚要装"的 `@nestjs/event-emitter` / `cls-rs` / 任何字典缓存库)
...
- 禁止在草案文档中画"最终 ER 图"或写完整 Prisma DSL(草案是"形态级"讨论,不是"实现级"锁定)
...
任何 V2 草案 / 开发动作开始前,**必须**读取并遵守 [`docs/srvf-foundation-baseline.md`](./docs/srvf-foundation-baseline.md)(自 commit `16876fe` 起锁定)。该文档承载 13 项 A 档基线规范(BizCode 段位、命名、响应包装、DTO 白名单、模块结构、错误码命名、配置归属、日志屏蔽、Guard、软删除、v1 兼容性、时区、验收门槛),与本节(§18)是**互补关系**:
...
### 18.5 V2 调研期 Claude Code 工具链约束

- **TodoWrite**:V2 调研类任务允许使用,但内容只能是"读 / 写 / 评审 / 提问"类动作,**禁止**出现"实现 / 创建 / migration / 安装"等执行词
- **Plan(ExitPlanMode)**:任何**疑似越界**到执行的动作(改 schema、新建模块、安装依赖)**必须**先出 Plan 经用户确认,且默认应当被否决
- **Agent(subagent)**:允许用 Explore / general-purpose 调研既有代码与社区实践,产出必须落到文档,**不得**直接产出代码 / schema
- **Skill**:V2 调研期不调用任何会修改仓库代码 / schema 的 skill;`gsd-*` 中带"execute / migrate / scaffold"语义的命令默认不启用
- **Commit**:V2 设计阶段每次 commit 仅含文档变更,commit message 前缀建议 `v2-design: <章节> <简述>`;**禁止**把文档与代码 / schema / 依赖变更混进同一 commit
...
| 调研字典模式时顺手装 `i18next` / 字典缓存包 | 违反 §18.1 + 研究文档 §3.12 不主动加缓存 |
| 讨论审计时顺手在 `PrismaService` 加全局 hook | 违反 §18.1;审计写入策略是 [待调研],不允许预实现 |
