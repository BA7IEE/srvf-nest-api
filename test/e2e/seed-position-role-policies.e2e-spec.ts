...
//      (org-admin 56 = biz-admin 72〔2026-07-03 摘码微刀后已不含终审两码〕- 敏感 1
...
//      ops-admin 94(F3「C 组」起)/ member 9 / biz-admin 72(2026-07-03 摘码微刀起)绑定数不变;
...
// org-admin 56 码(独立期望集;= biz-admin 72〔2026-07-03 摘码微刀后终审两码已不在 biz-admin,
// seed 侧排除项转为防御性 no-op〕过滤 recruitment-application.read.sensitive〔§4.2 敏感分级〕+
// recruitment-* 8 + team-join-* 7〔招新/入队中央流程不随组织业务下放〕)。
...
// 既有 3 角色绑定数零漂移基线(seed-rbac 94 / seed-attachment 9 / seed-biz-admin 72 同口径;
...
const EXPECTED_BIZ_ADMIN_BINDING_COUNT = 72;
...
describe('prisma/seed.ts — PR7 position role policies + PR9 final reviewer(内置角色 7)', () => {
  ...
  it('5. 零指派 + 零漂移:3 新角色无任何持有者;ops-admin 94(F3「C 组」起)/ member 9 / biz-admin 72(摘码微刀起)不变;保留码不绑', async () => {
