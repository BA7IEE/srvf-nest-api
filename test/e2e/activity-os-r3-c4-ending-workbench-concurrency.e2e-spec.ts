...
describe('C4 independent-pool lock waits', () => {
  ...
  let roleId: string;
  const key = () => `c4lock_${randomUUID().replaceAll('-', '').slice(0, 20)}`;
  beforeAll(async () => {
    ...
    roleId = (await prisma.rbacRole.create({ data: { code: key(), displayName: 'C4并发读' } })).id;
  ...
  it.each(['selection', 'grant', 'initiator'] as const)(
    ...
    async (mode) => {
      ...
      const result = request(httpServer(peer))
        .get(`/api/app/v1/my/managed-activities/${row.id}/ending-workbench`)
      ...
      try {
        await waitFor(
          ...
          { timeoutMs: 5000, message: 'C4 did not wait on the held Activity lock' },
        );
        unlock();
        await holder;
        ...
        if (mode === 'selection') {
          ...
          expect(response.body.data.metricRequirementCode).toBe('not_required');
          expect(response.body.data.metricSelectionRevision).toBe(1);
          expect(response.body.data.notices).toEqual([]);
