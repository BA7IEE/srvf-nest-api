// Independent real-database fixture retained locally to keep the approved write set closed.
// The original D7-1 suite remains unchanged and is also run independently.
...
describe('D7-1 pairing index probe migration regression', () => {
  let f: D13Fixture;
    ...
    async (population) => {
      ...
      try {
        ...
        for (const seconds of [1800, 900, 1200]) {
          ...
          await verifyNonemptyFunctionUpgrade(f.db, prepared.newPostingBatchId, population * 4);
          const reportUrl =
...
/** Exercise both full function definitions against this spec's own nonempty real chain.
 * DDL is transactional and always rolls back; the deployed migration stays authoritative.
 */
async function verifyNonemptyFunctionUpgrade(
  db: PrismaClient,
  batchId: string,
  ...
  const oldSql = readFileSync(
    join(migrationRoot, '20260915120000_activity_os_r4_d7_time_correction', 'migration.sql'),
    'utf8',
  );
  const match = oldSql.match(/CREATE FUNCTION ptc_assert_complete[\s\S]*?\n\$\$;/u);
  if (!match) throw new Error('Missing historical complete function');
  const oldFunction = match[0].replace('CREATE FUNCTION', 'CREATE OR REPLACE FUNCTION');
  const next = readFileSync(
    join(migrationRoot, '20260915170000_activity_os_r4_d7_pairing_index_probe', 'migration.sql'),
    'utf8',
  );
  const normalized = next
    .slice(next.indexOf('CREATE OR REPLACE FUNCTION'))
    .replace(
      /LEFT JOIN LATERAL \(\s*SELECT e\.\* FROM "ParticipationTimeCorrectionEntry" e\s*WHERE e\."manifestId" = m\."(id|predecessorManifestId)" AND e\."rootEntryId" = root\."id" AND e\."entryTypeCode" = '(reversal|credit)'\s*OFFSET 0\s*\) (reversal|credit|prior) ON TRUE/gu,
      (_whole, manifest: string, type: string, alias: string) =>
        'LEFT JOIN "ParticipationTimeCorrectionEntry" ' +
        alias +
        ' ON ' +
        alias +
        '."manifestId" = m."' +
  ...
  // Everything outside the three equivalent complete-key lookups must be byte-identical.
  expect(normalized.trim()).toBe(oldFunction.trim());
  const rollback = new Error('pairing migration fixture rollback');
  await expect(
    db.$transaction(
      async (tx) => {
        const digest = async () => {
          const rows = await tx.$queryRaw<{ digest: string }[]>`
        SELECT md5(string_agg(payload, '' ORDER BY payload)) AS digest FROM (
          SELECT row_to_json(m)::text AS payload FROM "ParticipationTimeCorrectionManifest" m
          UNION ALL SELECT row_to_json(e)::text FROM "ParticipationTimeCorrectionEntry" e
          UNION ALL SELECT row_to_json(r)::text FROM "ParticipationTimeCorrectionCommitReceipt" r
          UNION ALL SELECT row_to_json(l)::text FROM "ParticipationTimeLedgerEntry" l
        ) snapshots
      `;
          return rows[0].digest;
        };
        const counts = await tx.participationTimeCorrectionEntry.count();
        expect(counts).toBeGreaterThanOrEqual(expectedRoots * 2);
        const before = await digest();
        await tx.$executeRawUnsafe(oldFunction);
        await tx.$queryRaw`SELECT ptc_assert_complete(${batchId})::text`;
        expect(await digest()).toBe(before);
        await tx.$executeRawUnsafe(next);
        await tx.$executeRawUnsafe('SET LOCAL enable_hashjoin = off');
        await tx.$queryRaw`SELECT ptc_assert_complete(${batchId})::text`;
        expect(await digest()).toBe(before);
        const manifest = await tx.participationTimeCorrectionManifest.findUniqueOrThrow({
          where: { postingBatchId: batchId },
        });
        const request = await tx.attendanceCorrectionRequest.findUniqueOrThrow({
          where: { id: manifest.correctionRequestId },
          select: { requestedChangeJson: true },
        });
        const change = request.requestedChangeJson;
        if (!change || typeof change !== 'object' || Array.isArray(change))
          throw new Error('Expected a correction request object');
        const correction = change.timeCorrection;
        if (
          !correction ||
          typeof correction !== 'object' ||
          Array.isArray(correction) ||
          !Array.isArray(correction.items)
        )
          throw new Error('Expected complete correction items');
        // Explain the exact complete mismatch query from the deployed candidate, not a
        // simplified count-only prototype. The full function is called independently above.
        const begin = next.indexOf('WITH paired AS MATERIALIZED (');
        const end = next.indexOf('\n    )\n    OR EXISTS', begin);
        expect(begin).toBeGreaterThan(0);
        expect(end).toBeGreaterThan(begin);
        const pairing = next
          .slice(begin, end)
          .replaceAll('m."id"', '$1::text')
          .replaceAll('m."predecessorManifestId"', '$2::text')
          .replaceAll('m."rootManifestId"', '$3::text')
          .replaceAll('approved_items', '$4::jsonb');
        const plans = await tx.$queryRawUnsafe<{ 'QUERY PLAN': Array<{ Plan: ProbePlan }> }[]>(
          'EXPLAIN (ANALYZE, FORMAT JSON, TIMING OFF) ' + pairing,
          manifest.id,
          manifest.predecessorManifestId,
          manifest.rootManifestId,
          JSON.stringify(correction.items),
        );
        const plan = plans[0]['QUERY PLAN'][0].Plan;
        expect(plan['Actual Rows']).toBe(0);
        const probes = flattenProbePlan(plan).filter(
          (node) => node['Index Name'] === 'ptce_manifest_root_type_key',
        );
        // A literal null predecessor has no possible prior row and may be folded away.
        expect(probes).toHaveLength(manifest.predecessorManifestId === null ? 2 : 3);
        for (const probe of probes) {
          expect(probe['Actual Loops']).toBe(expectedRoots);
          expect(probe['Actual Rows']).toBeLessThanOrEqual(1);
          for (const key of ['manifestId', 'rootEntryId', 'entryTypeCode'])
            expect(probe['Index Cond']).toContain(key);
        }
        const stored = await tx.$queryRaw<{ body: string }[]>`
      SELECT prosrc AS body FROM pg_proc
      WHERE oid = 'ptc_assert_complete(text)'::regprocedure
    `;
        expect((stored[0].body.match(/OFFSET 0/gu) ?? []).length).toBe(3);
        // Record a stable SQL fingerprint, never fixture contents.
        expect(createHash('sha256').update(next).digest('hex')).toMatch(/^[a-f0-9]{64}$/u);
        throw rollback;
      },
      { timeout: 120000 },
    ),
...
function flattenProbePlan(plan: ProbePlan): ProbePlan[] {
  return [plan, ...(plan.Plans ?? []).flatMap(flattenProbePlan)];
}
