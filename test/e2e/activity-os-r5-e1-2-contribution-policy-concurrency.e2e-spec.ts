...
import { ActivityContributionPolicyService } from '../../src/modules/activities/activity-contribution-policy.service';
import { ActivityContributionPolicyCommand } from '../../src/modules/activities/activity-contribution-policy-command';
import { ActivityContributionPolicyCatalogueQueryService } from '../../src/modules/activities/activity-contribution-policy-catalogue-query.service';
...
const meta = { requestId: 'contribution-policy-race', ip: null, ua: null };
...
describe('E1-2 real PostgreSQL command races', () => {
  let app: INestApplication;
  let db: PrismaService;
  let service: ActivityContributionPolicyService;
  let user: CurrentUserPayload;
  let n = 0;
  const prefix = 'cpr_' + randomBytes(5).toString('hex');
  ...
  beforeAll(async () => {
    ...
    service = app.get(ActivityContributionPolicyService);
  ...
  it('measures real application SQL against the approved budget', async () => {
    const reader = await db.permission.findUniqueOrThrow({
      where: { code: 'contribution-policy.read.catalog' },
    });
    ...
    try {
      ...
      const commands = new ActivityContributionPolicyCommand(observed, new RbacService(observed));
      const measured = new ActivityContributionPolicyService(
        commands,
        app.get(ActivityContributionPolicyAuditRecorder),
      );
      const query = new ActivityContributionPolicyCatalogueQueryService(observed, commands);
      ...
      console.info('E1-2 measured application SQL', counts);
  ...
  it('same key concurrent creation is one policy, receipt and audit', async () => {
    ...
    expect(await db.contributionPolicy.count({ where: { code: command.code } })).toBe(1);
    expect(
      await db.contributionPolicyCommandReceipt.count({ where: { policyId: results[0].policyId } }),
    ).toBe(1);
  ...
  it('rolls back an expired transaction without an automatic retry', async () => {
    ...
    try {
      ...
      expect(await db.contributionPolicy.count({ where: { code: command.code } })).toBe(0);
      expect(
  ...
  it('same key different input has exactly one winner', async () => {
    ...
    expect(results.find((r) => r.status === 'rejected')).toMatchObject({
      reason: { biz: BizCode.ACTIVITY_CONTRIBUTION_POLICY_COMMAND_CONFLICT },
    });
  });
  it('different keys cannot reuse the stable policy code', async () => {
    ...
    expect(results.filter((r) => r.status === 'fulfilled')).toHaveLength(1);
    expect(results.find((r) => r.status === 'rejected')).toMatchObject({
      reason: { biz: BizCode.ACTIVITY_CONTRIBUTION_POLICY_CODE_EXISTS },
    });
  });
  it('different keys allocate monotone distinct versions under the policy row lock', async () => {
    ...
    expect(
      await db.contributionPolicyVersion.findMany({
  ...
  it('concurrent activation cannot emit two transitions or audits', async () => {
    ...
    expect(results.filter((r) => r.status === 'fulfilled')).toHaveLength(1);
    expect(results.find((r) => r.status === 'rejected')).toMatchObject({
      reason: { biz: BizCode.ACTIVITY_CONTRIBUTION_POLICY_STALE },
    });
    expect(await db.auditLog.count({ where: { resourceId: { in: [p.policyId, vid] } } })).toBe(3);
  });
  ...
  it.each(['command-revoke', 'command-disable', 'policy-revoke', 'version-revoke'] as const)(
    ...
    async (mode) => {
      ...
      const blocker = db.$transaction(
        async (tx) => {
          if (mode.startsWith('command'))
            ...
            await tx.$queryRaw(
              Prisma.sql`SELECT "id" FROM "ContributionPolicy" WHERE "id" = ${p.policyId} FOR UPDATE`,
            );
          else
            await tx.$queryRaw(
              Prisma.sql`SELECT "id" FROM "ContributionPolicyVersion" WHERE "id" = ${vid} FOR UPDATE`,
            );
          locked();
          await release;
        },
      ...
      try {
        await waitFor(
          async () => {
            ...
            AND (query LIKE '%pg_advisory_xact_lock%' OR query LIKE '%ContributionPolicy%FOR UPDATE%')`;
      ...
      expect(await db.contributionPolicyVersion.count({ where: { policyId: p.policyId } })).toBe(1);
      expect(
        await db.contributionPolicyCommandReceipt.count({ where: { policyId: p.policyId } }),
      ).toBe(2);
      expect(await db.auditLog.count({ where: { resourceId: { in: [p.policyId, vid] } } })).toBe(2);
      expect(
        (await db.contributionPolicyVersion.findUniqueOrThrow({ where: { id: vid } })).statusCode,
      ).toBe('draft');
    },
  );
});
