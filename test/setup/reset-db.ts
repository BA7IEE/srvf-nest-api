...
//
// 活动评价 F1 追加(2026-07-16):activity_feedbacks(物理小写名,goal 指定 `@@map`);
// 两条 FK 均 Restrict:
//   - activity_feedbacks.activityId → Activity.id
//   - activity_feedbacks.memberId → Member.id
// 作为参与域子表放在 Activity / Member 之前，防跨 spec 残留评价污染聚合与资格断言。
//
// 活动岗位与时段 F1 追加(2026-07-16):activity_positions(物理小写名,goal 指定 `@@map`);
// 两条 FK 均 Restrict:
//   - activity_positions.activityId → Activity.id
//   - ActivityRegistration.activityPositionId → activity_positions.id(nullable)
// 放在 ActivityRegistration 之后、Activity 之前；虽 CASCADE 可兜底，仍显式列出以防跨 spec
// 残留岗位污染后续容量、重名与删除守卫断言。
//
...
//
// D-Throttle(2026-07-18):throttler_buckets 无 FK、承载全部 10 个命名 throttler 的共享额度；
// 必须显式清理，否则跨 spec/application 遗留 bucket 会污染 limit/block 断言。
...
// Activity OS R2 / B1:ActivityPlace 同时引用 Activity / 可选 ActivitySession / 可选 PlacePreset；
// PlacePreset 自身可独立存在，不得期待清 Activity 时的 CASCADE 间接清掉它。顺序固定
// ActivityPlace → PlacePreset，避免未来关系调整后跨 spec 残留地点预设或活动地点快照。
// Activity OS R2 / B6 D1:紧急后续项 → 紧急起源 → 创建收据均以 Restrict 关联 Activity / User。
// 即使 CASCADE 当前可兜底，也显式按子到父清掉，避免后续关系调整让跨 spec 的命令收据或
// pending 义务残留，污染幂等、唯一或清单状态断言。
