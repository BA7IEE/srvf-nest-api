...
D7-min 决议(§5.2 V2-D7)将 5 个模型延后到 V2.x;复活触发条件如下,**任一即可启动对应延后模型的 V2.x 立项**:
...
| `audit_logs` | **作为 V2.x 第一个增量启动**(无独立触发条件,跟随 V2.x 启动节奏);接入 V2 第一阶段已交付 4 模型的关键写操作(管理员状态变更 / 删除 / 角色变更 / 部门归属变更等) | 直接进入 V2.x 开发立项,无需重走 D7 |
