...
| **V2-D5** | 调研访谈与资料收集(用户主导) | ⏸️ **暂停**(2026-05-07) | V2-D1..D4 |
| **V2-D6** | 输出 `docs/srvf-foundation-data-model-draft.md`(候选模型草案) | ⏳ 待启动 | V2-D5 阶段性产出 |
...
| **A2** | 日志屏蔽清单代码侧扩展(`src/bootstrap/logger-options.ts` `redact` 配置) | ⏸️ 暂缓 | 等首个敏感字段进入 schema 时同 commit 联动 |
...
- **暂停说明**(2026-05-07):
  - `docs/srvf-foundation-research-questions.md` 已起草完整 9 节(共 82 题),作为**工具文档**保留为 Untracked,**不进** git history
  - 暂停原因:用户决定优先推进 §5.5 A 档快车道,避免调研周期阻塞基建落地
  - **重启条件**(任一即可):
    - (a) §5.5 A 档第一批完成且需要业务调研支撑下一步
    - (b) 用户主动恢复访谈
    - (c) V2-D6 草案启动前补足关键调研项(研究文档 §4.1 / §4.5 / §4.6 / §4.8 / §4.9)
  - 重启时**不**重新起草问卷 — 复用现有 `research-questions.md` 直接进入访谈作答阶段
...
- **状态**:⏸️ 暂缓
- **范围**:把 baseline §8.2 列出的 V2 敏感字段名加进 `src/bootstrap/logger-options.ts` 的 `redact` 配置
- **暂缓原因**:
  - 触碰 v1 已交付 `src/bootstrap/`(`CLAUDE.md §18.1` 列入禁止区,需用户单独许可)
  - 字段尚未落表,屏蔽规则无命中目标,实质价值低
  - baseline §8.4 修订纪律明确:屏蔽规则与字段进入 schema 的 commit **同批次或先于**才有意义
- **解锁条件**:首次有敏感字段(如 `members.idCard` / `member_profiles.phone` 等)进入 schema 时,**与该 schema 变更同 commit** 完成屏蔽扩展;由 D8 决议进入开发后处理
- **不独立立项**:本任务严格搭车于业务任务,不单独 commit
...
A2 / A3 解锁的统一规则:**与 v1 src/ 改动同 commit 才有实质价值**。

| 触发场景 | 联动任务 |
|---|---|
| 首个 V2 模块的 schema 引入敏感字段 | A2 联动(同 commit 扩展 redact 清单) |
| 首次新增 V2 模块的 BizCode | A3 联动(同 commit 加段位注释) |
| 其他"顺手做"反模式 | 暂缓,等独立任务 |

A1 是已唯一"独立可做且无副作用"的 A 档任务;A2-A5 全部要求**搭车**于业务任务,**不**独立立项。
