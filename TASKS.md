...
| **V2-D6** | 输出 `docs/srvf-foundation-data-model-draft.md`(候选模型草案) | 🟡 v0.2 含 D5 结论(commit `92d7512`,**非评审通过版**) | V2-D5 阶段性产出 |
...
- **状态**:🟡 **v0.2 含 D5 结论**(commit `92d7512`,**非评审通过版**)
...
- **演进轨迹**(2026-05-07):
  ...
  - **v0.2 含 D5 结论**:commit `92d7512` 同步 D5 访谈结论(基于 commit `17486fe` research.md §4 回填),改动 +344 / -16,文件总行数 1342:
    ...
    - §7.1 v0.2 版本行
- **当前 D6 状态**:🟡 **起草中 / 非评审通过版**
  - 9 个候选模型全部从"骨架级"演进到"形态级有阶段性结论"
  - 所有"当前倾向"标签**未经 D7 评审锁定**
  - **不**等于"已确认进入开发"
- **下一步**:
  - **人工审阅** v0.2 草案(用户主导)
  - 进入 **D7 模型评审**:对 9 个候选模型逐一拍板"进入 V2 / 延后到 V2.x / 砍掉";对 4 个跨模型模式拍板单一方案
  - 通过 D7 → 草案进入 v1.0 锁定状态 → 触发 D8 进入开发立项决策
- **明确禁止**(承接草案 §0.3 / §7.3 / §3.x.9 各模型尾注):
  - **不得**据此直接开发 schema / migration / API / V2 业务模块 controller / service / dto
  - **不得**因 v0.2 落地就把"当前倾向"视为"已拍板开发"
  - **不得**绕过 D7 评审 / D8 决策直接进入实施层
  - 开发阶段以 D8 决议为准
- **特别提示**(D5 结论中带条件的项):
  - **member_profiles**:F1 身份证 / F2 紧急联系人 / F3 医疗信息进入 D6 候选范围,但 **D7/D8 前必须补合规依据**(永久保留依据 / 数据最小化说明 / 退队最小化处理 / 医疗用途更具体表述);未补齐前禁止进入最终 schema / migration / API
  - **events / event_participants**:回退档升级为"全做",但仍受 D7/D8 关卡保留;D5 已确认"兼容人工录入 / 报名考勤维持弱关联"
  - **dictionaries**:类别总数 + 异质性仍待补,可在 D7 前后补齐
