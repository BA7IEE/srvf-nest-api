...
- 根据 #1339 同一 SHA 的两次冷跑证据，关闭剩余全部同形状缺口：14 份旧 E2E 的 15 处 `withTimeLedgerFixtureCleanup` 外层事务统一显式设置 60 秒上限。全仓 31 处调用现已全部具名设置 30 或 60 秒；本轮只延长测试夹具清理事务，不改 Jest 总时限、业务事务预算、断言、生产代码或测试基础设施。本地串行冷跑 13 份完整 E2E 为 13/13 suites、255/255 tests 通过；B3 仅运行命中本次改动的 Form materialization 分组并 3/3 通过，固定使用未授权 w95 的 migration rehearsal 留给 PR CI 冷跑。
